...
function EmptyAppointments({ onAction }) {
  return (
    <div className="flex flex-col items-center justify-center p-8 border border-dashed border-[var(--color-border)] rounded-2xl bg-[var(--color-surface-2)]/10 text-center space-y-4 w-full">
      ...
      {onAction && (
        <button onClick={onAction} className="px-3.5 py-1.5 rounded-xl bg-[var(--color-primary)] hover:bg-[var(--color-primary)]/90 text-white text-[9px] font-semibold transition-all shadow cursor-pointer active:scale-95">
          Agendar Turno
        </button>
      )}
    </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        {professionals.map((prof) => {
          ...
          return (
            ...
            >
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-mono font-bold text-xs shrink-0 ${isSelected ? 'bg-[var(--color-primary)] text-white' : 'bg-zinc-800 text-zinc-400'}`}>
                {prof.avatar}
              </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={() => setIsAdding(!isAdding)} className="text-[9px] font-semibold text-[var(--color-primary)] hover:text-white transition-colors cursor-pointer">
      ...
      {isAdding ? (
        <div className="p-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-2)]/20 space-y-2">
          ...
          <button type="button" onClick={handleSave} className="w-full py-1.5 rounded-xl bg-[var(--color-primary)] text-white text-[9px] font-semibold transition-all cursor-pointer">
            ...
            return (
              <button
                ...
                className={`py-1.5 rounded-xl border text-[10px] font-mono font-bold transition-all text-center flex flex-col justify-center items-center h-10 ${
                  isSelected ? 'bg-[var(--color-primary)] text-white border-[var(--color-primary)]' : slot.isAvailable ? 'bg-[var(--color-surface-2)]/30 border-[var(--color-border)] text-zinc-300 hover:border-zinc-600 cursor-pointer' : 'bg-zinc-950/45 border-transparent text-zinc-700 cursor-not-allowed opacity-40'
                }`}
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-2)]/40 backdrop-blur-md">
      <div className="flex items-center space-x-2.5">
        <div className="flex p-0.5 rounded-lg bg-zinc-950 border border-zinc-900">
          ...
          <button onClick={onToday} className="px-2.5 py-1 rounded text-[9px] font-semibold text-zinc-300 hover:text-white cursor-pointer">Hoy</button>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button onClick={onAddAppointment} className="px-3 py-1.5 rounded-xl bg-[var(--color-primary)] text-white text-[9px] font-semibold transition-all active:scale-[0.98] cursor-pointer flex items-center space-x-1">
          <span>+ Cita</span>
        </button>
      </div>
    </div>
  ...
  return (
    <div onClick={onClick} className="group relative p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)]/20 hover:bg-[var(--color-surface-2)]/50 transition-all cursor-pointer flex flex-col justify-between space-y-2.5 active:scale-[0.99] select-none text-left">
      ...
      {appointment.status !== 'Completada' && appointment.status !== 'Cancelada' && (
        <button onClick={handleQuickComplete} className="opacity-0 group-hover:opacity-100 w-full py-1 rounded bg-emerald-950/20 border border-emerald-900/30 text-emerald-400 hover:text-white hover:bg-emerald-600 text-[8px] font-bold transition-all text-center cursor-pointer">
          Marcar Completada
        </button>
      )}
    </div>
            ...
            <div key={day.date.toISOString()} className="flex flex-col min-h-[320px] group/col">
              <div className={`p-2.5 border-b border-zinc-900/60 flex items-center justify-between ${day.isToday ? 'bg-[var(--color-primary)]/5' : 'bg-zinc-950/20'}`}>
                <div className="flex items-center space-x-1.5">
                  ...
                  <span className={`w-4.5 h-4.5 rounded-full flex items-center justify-center font-mono text-[9px] font-bold ${day.isToday ? 'bg-[var(--color-primary)] text-white' : 'text-zinc-300'}`}>{dayNum}</span>
                </div>
      ...
      <div className="grid grid-cols-7 divide-y divide-x divide-zinc-900/60 bg-zinc-950/5">
        {monthDays.map((day, idx) => {
          ...
          return (
            ...
            >
              <span className={`w-4 h-4 rounded-full flex items-center justify-center font-mono text-[8px] font-bold ${day.isToday ? 'bg-[var(--color-primary)] text-white' : 'text-zinc-400'}`}>
                {day.date.getDate()}
              </span>
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(hours); }} className="space-y-4 text-left">
      ...
      <div className="flex justify-end space-x-2 pt-2 border-t border-[var(--color-border)]">
        ...
        <button type="submit" className="px-3.5 py-1.5 rounded-xl bg-[var(--color-primary)] text-white text-xs font-semibold shadow cursor-pointer">Guardar Horarios</button>
      </div>
    </form>
  );
}
...
function AppointmentForm({ initialData, clients, professionals, services, appointments, businessHours, onSave, onClose, onCreateClient }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 text-left">
      ...
      <div className="flex justify-end space-x-2 pt-2 border-t border-[var(--color-border)]">
        <button type="button" onClick={onClose} className="px-3.5 py-1.5 rounded-xl bg-zinc-900 text-xs text-zinc-300 font-semibold border border-zinc-800 cursor-pointer">Cancelar</button>
        <button type="submit" disabled={!clientId || !serviceId || !professionalId || !date || !time} className="px-3.5 py-1.5 rounded-xl bg-[var(--color-primary)] text-white text-xs font-semibold disabled:bg-zinc-800 disabled:text-zinc-600 cursor-pointer">Confirmar</button>
      </div>
    </form>
  );
}
  ...
  return (
    <div className="space-y-4 text-left">
      <div className="p-3.5 rounded-2xl border border-[var(--color-border)] bg-zinc-950/40 relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-3 mt-3 pl-1.5 border-t border-zinc-900/60 pt-2.5">
          <div>
      ...
      <div className="pt-2 border-t border-[var(--color-border)] space-y-3">
        ...
        <div className="flex justify-between items-center pt-1.5">
          <button onClick={() => onDelete(appointment.id)} className="px-3 py-1.5 rounded-xl bg-rose-950/15 border border-rose-900/30 text-rose-500 hover:bg-rose-600 hover:text-white text-[9px] font-bold cursor-pointer">Eliminar Cita</button>
          <div className="flex space-x-1.5">
            ...
            <button onClick={() => onEdit(appointment)} className="px-3 py-1.5 rounded-xl bg-[var(--color-primary)] text-white text-xs font-semibold cursor-pointer">Editar / Reagendar</button>
          </div>
        </div>
      </div>
    </div>
...
function AppointmentConfirmationModal({ appointment, onClose }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-3 text-center space-y-3.5">
      ...
      <button onClick={onClose} className="w-full py-2 rounded-xl bg-[var(--color-primary)] text-white text-[10px] font-semibold cursor-pointer">Volver al Calendario</button>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4 text-center">
      ...
      <div className="flex justify-end space-x-2 pt-2 border-t border-[var(--color-border)]">
        ...
        <button onClick={() => onConfirm(appointment.id)} className="px-3.5 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-semibold cursor-pointer">Eliminar Cita</button>
      </div>
    </div>
  );
}
