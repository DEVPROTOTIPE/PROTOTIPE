...
* **`/Prototipe-CLI/`**: Herramienta de línea de comandos (CLI) interactiva para automatizar el aprovisionamiento, copiado de plantillas, inyección de variables de entorno y bootstrapping de nuevos proyectos del ecosistema en `D:\PROTOTIPE\Instancias Clientes\` mediante spinners interactivos `ora`.
  ...
  * **`server.js`**: Servidor local Express que expone la API Bridge con el dev-dashboard, integrando telemetría de tests Playwright, logs en tiempo real y arranque del worker de creación.
  * **`generator.js`**: Motor de aprovisionamiento de proyectos que gestiona copias de plantillas, configuración HSL, autogeneración de reglas de Storage/Firestore y copiado/redimensionamiento de logos.
  ...
  * **`sync_clients.js`**: Script de sincronización selectiva downstream para propagar de forma segura parches del core hacia las instancias de clientes, verificando el build de integridad.
...
* **`/src/pages/admin/AdminSettings.jsx`**: Panel de Ajustes y Configuración del Administrador. Cuenta con diseño responsivo prémium (split-screen en PC con barra de navegación lateral y contenido persistente, y vista lineal basada en tarjetas en móviles). Integra Herramientas de Administrador (Clientes y Gastos/Pagos), Empleados y Capa de Optimización. Se purgó físicamente todo el código e interfaces del módulo obsoleto de mesas y autoservicio QR (TableQRModal y AdminTablesCRUD).
