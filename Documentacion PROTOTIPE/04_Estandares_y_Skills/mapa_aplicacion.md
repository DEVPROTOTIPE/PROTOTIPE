...
* **`/LandingPage/Index.html`**: Portal web y landing page corporativa oficial de PROTOTIPE, implementando variables HSL de tema, tipografías Outfit/Inter, fondos radiales animados, glassmorphism, acordeón de FAQ interactivo y optimización SEO.
...
* **`/Prototipe-CLI/`**: Herramienta de línea de comandos (CLI) interactiva para automatizar el aprovisionamiento, copiado de plantillas, inyección de variables de entorno y bootstrapping de nuevos proyectos del ecosistema en `D:\PROTOTIPE\Instancias Clientes\{coreType}\` mediante spinners interactivos `ora`.
  ...
  * **`generator.js`**: Motor de aprovisionamiento de proyectos que gestiona copias de plantillas estructuradas por tipo de core, configuración HSL, autogeneración de reglas de Storage/Firestore, copiado y redimensionamiento inteligente de logos con Jimp y arranque síncrono inmediato del script de mapa de IA.
  ...
  * **`templates/template-core-seed/`**: Plantilla base (Core Seed) genérica y desacoplada de negocio, equipada con telemetría, facturación multi-modo, alerta remota de pago y ping test interactivos, y un script de generación automática de mapa semántico para IA (`scratch/generate_ia_map.js`).
