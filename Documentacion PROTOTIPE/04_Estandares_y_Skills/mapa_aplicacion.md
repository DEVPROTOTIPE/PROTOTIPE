...
* **`/git_backup.ps1`**: Script de PowerShell para automatizar copias de seguridad (Git Backup) completas de todo el ecosistema (Maestro), deteniendo servidores de desarrollo para liberar bloqueos y forzando la rama `develop` al finalizar.
* **`/subproject_backup.ps1`**: Script de PowerShell para realizar respaldos Git y commits en subproyectos, forzando la rama `develop` para componentes base (Core/Dashboard) y respetando las ramas de cliente al concluir.
* **`/menu_backup.ps1`**: Script de PowerShell con menú interactivo por consola para simplificar copias de seguridad de forma manual en el entorno local.
...
* **`/src/services/billingService.js`**: Lógica de cálculo, acumulación y desglose de comisiones mensuales e históricas multi-modo (porcentaje, valor fijo por servicio, pago mensual fijo). Escucha y expone en tiempo real la señal `triggerTelemetryReport` emitida por el panel de control central.
