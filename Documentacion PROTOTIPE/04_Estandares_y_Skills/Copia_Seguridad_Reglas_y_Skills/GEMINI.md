...
- CONTROL Y BITÁCORA DE TAREAS (CRÍTICO - OBLIGATORIO):
  * TODO cambio en el estado de una tarea (nueva, en progreso, completada o modificada) debe registrarse inmediatamente en el archivo general `D:\PROTOTIPE\Documentacion PROTOTIPE\02_Tareas_Roadmap\tareas_pendientes.md`. **Si el cambio pertenece a un subproyecto o Core específico (ej: dev-dashboard, App Ventas, App Servicios, etc.), se debe registrar también obligatoriamente en su tareas_pendientes.md local (ej: D:\PROTOTIPE\Central PROTOTIPE\Documentacion dev-dashboard\tareas_pendientes.md, o D:\PROTOTIPE\Plantillas Core\App Ventas\Documentacion App Ventas\tareas_pendientes.md).**
  * SINCRONIZACIÓN DEL MAPA: Ante cualquier modificación, creación, eliminación o refactorización que altere la estructura física, lógica o de datos de los archivos, se debe actualizar obligatoriamente el mapa de la aplicación general en `D:\PROTOTIPE\Documentacion PROTOTIPE\04_Estandares_y_Skills\mapa_aplicacion.md`. **Si la modificación pertenece a un subproyecto o Core específico, se debe actualizar además obligatoriamente su mapa_aplicacion.md local.**
  ...
  * REGISTRO EN BITÁCORA DE CAMBIOS: Todo cambio técnico, corrección de bugs, refactorizaciones e implementaciones de módulos debe registrarse obligatoriamente en la bitácora general `D:\PROTOTIPE\Documentacion PROTOTIPE\03_Auditorias_y_Faro_Core\bitacora_cambios.md` en el mismo paso que se realiza el cambio. **Si el cambio pertenece a un subproyecto o Core específico, se debe registrar también obligatoriamente en su bitacora_cambios.md local.**
  ...
  * DISPARADOR RÁPIDO DE INTEGRIDAD Y DOCUMENTACIÓN DE APLICACIONES A LA MEDIDA: Siempre que el usuario escriba la palabra **`@postchange`** (o sus variantes), la IA debe ejecutar la compilación local (`npm run build`) del proyecto correspondiente y registrar los cambios, mapas y tareas en el proyecto destino:
    ...
    - **REGLA DE DOCUMENTACIÓN CRUZADA (CRÍTICO):** Cualquier cambio realizado desde este chat sobre un proyecto (como dev-dashboard, App Ventas, App Servicios, etc.) obliga a documentar la bitácora, mapas y tareas tanto dentro del directorio de documentación general del Ecosistema como localmente en la subcarpeta de documentación específica de dicho subproyecto para asegurar la consistencia y trazabilidad leída por otras IAs.
