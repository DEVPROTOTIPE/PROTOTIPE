...
*   **Monitoreo y Telemetría Centralizada:** Las métricas de error, deudas y comisiones de las instancias de clientes se reportan de forma asíncrona hacia una base de datos centralizada de telemetría a través del Bridge local (redirigiendo `VITE_DEVELOPER_TELEMETRY_ENDPOINT` a `http://localhost:3001`), o de forma directa mediante escrituras del SDK secundario `centralFirebaseService.js` en Firestore Central.
