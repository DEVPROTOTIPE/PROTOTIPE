...
* **Backdrop Blur:** Úsalo en cabeceras fijas (`sticky`), barras de navegación inferiores y overlays de modales para lograr un acabado elegante:
  ...
  ```
* **Sombras Suaves:** Limita el uso de sombras grandes y toscas. Utiliza elevaciones finas (`shadow-sm`) o gradientes de contorno:
  ```css
  shadow-sm border border-slate-100/40
  ```
...
* **Tipografía de Títulos (Outfit, Inter, Roboto):** Debe ser de peso fuerte, espaciado compacto y tamaño responsivo.
  ```css
  text-xl md:text-2xl font-black text-slate-800 tracking-tight
  ```
* **Tipografía de Contenido:** Debe ser legible, con peso regular y un color suave para evitar la fatiga visual.
  ```css
  text-sm font-medium leading-relaxed text-slate-500
  ```
