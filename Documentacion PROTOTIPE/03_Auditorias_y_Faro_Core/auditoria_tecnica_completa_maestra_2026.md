...
* **Funciones Clave:**
  * `getFileHash(filePath)`: Calcula hashes MD5 de archivos para verificaciones rápidas.
...
#### 📄 `vite.config.js`, `package.json`, `index.html`, `firebase.json`, `firestore.rules`
* **Comportamiento:** Estructura estándar de aplicación Vite + React + Firebase Hosting. La configuración de `firestore.rules` contiene vulnerabilidades de auditoría.

#### 📄 `src/main.jsx`
* **Comportamiento:** Bootstrap del frontend de React, montando el árbol en el DOM.

#### 📄 `src/firebase.js`
* **Comportamiento:** Inicializa el SDK de Firebase Web App para auth y base de datos del cockpit.
...
#### 📄 `initialize_cores_docs.js`
* **Comportamiento:** Script CommonJS (`require`) que escribe la estructura base de carpetas y los 12 archivos de documentación Markdown en los directorios de Cores de desarrollo.
* **Estado:** Candidato a eliminación (funcionalidad absorbida por Express en `/api/register-core`).
...
### 3.2 Bugs Lógicos y del Sistema
