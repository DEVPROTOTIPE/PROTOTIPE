...
### [BUG-022] ReferenceError: Cannot access 'selectedRecomendations' before initialization
- **Fecha:** 2026-07-02
- **Severidad:** Crítico
- **Área:** UI/UX / Lógica
- **Archivos Afectados:**
  - [`Central PROTOTIPE/dev-dashboard/src/App.jsx`](file:///d:/PROTOTIPE/Central%20PROTOTIPE/dev-dashboard/src/App.jsx)
- **Síntoma Observable:** Pantalla blanca al cargar el Dashboard Central de desarrollo. La consola arroja el error: `Uncaught ReferenceError: Cannot access 'selectedRecomendations' before initialization`.
- **Causa Raíz:** Se inyectaron los hooks de LocalStorage (`hasLoadedDraft` y sus `useEffect` correspondientes) en la línea 1830, pero el hook de estado de React para `selectedRecomendations` (al cual acceden los effects de persistencia) se declaraba más abajo en la línea 2333, violando el orden de declaración temporal de variables.
- **Solución Aplicada:** Se reubicaron los hooks de persistencia del borrador de LocalStorage y sus efectos asociados debajo de todas las declaraciones de estado iniciales (justo después del effect de `libraryList` y la declaración de `selectedRecomendations`).
- **Estado:** ✅ Resuelto
