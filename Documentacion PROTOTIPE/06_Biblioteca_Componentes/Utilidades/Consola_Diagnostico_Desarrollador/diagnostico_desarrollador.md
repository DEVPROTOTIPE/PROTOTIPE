...
El componente `DeveloperDiagnosticsModal` proporciona un panel en tiempo real dentro de la aplicación para que los ingenieros de soporte o desarrolladores verifiquen la salud de la infraestructura de Firebase local, pings a servidores centrales, configuraciones del entorno (`.env.local`) y soporte de notificaciones push (VAPID/FCM).

### Casos de Uso principales:
1. **Verificación de Entorno:** Permite inspeccionar las variables globales cargadas (`VITE_FIREBASE_PROJECT_ID`, `VITE_DEVELOPER_EMAIL`, etc.) sin acceder a la terminal de compilación.
2. **Ping Local a Firestore:** Mide la latencia de lectura y escritura en la base de datos de Firebase realizando transacciones temporales que registran el tiempo transcurrido en milisegundos.
3. **Ping Central de Telemetría:** Prueba la conexión con el panel central de reportes o la conectividad externa para diagnosticar bloqueos de red o problemas de DNS.
4. **Pruebas de Notificaciones Push (VAPID):** Evalúa el estado del permiso del navegador para notificaciones, la validez del token FCM y la disponibilidad del SDK de mensajería de Firebase.
...
A continuación, se presenta la implementación completa del modal de diagnóstico:

```jsx
import React, { useState, useEffect } from 'react';
import { 
  X, Activity, Database, Network, Key, AlertTriangle, 
  CheckCircle, RefreshCw, Terminal, Cpu, Bell, ShieldCheck, Mail
} from 'lucide-react';
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className="flex-1 h-11 bg-primary text-[var(--color-text)] text-xs font-bold rounded-xl flex items-center justify-center gap-2 hover:opacity-90 active:scale-95 disabled:opacity-50 transition-all cursor-pointer"
                >
          ...
          {activeTab === 'vapid' && (
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full h-11 bg-primary text-[var(--color-text)] text-xs font-bold rounded-xl flex items-center justify-center gap-2 hover:opacity-90 active:scale-95 disabled:opacity-50 transition-all cursor-pointer"
              >
...
1. **Estado del Permiso de Notificaciones:**
   Al inicializarse en la pestaña VAPID, el componente verifica `Notification.permission` de forma proactiva y lo actualiza tras la interacción del usuario cuando éste concede el permiso.
2. **Generación del Token FCM:**
   Si el navegador soporta el protocolo PWA Push de Firebase y la llave `VITE_VAPID_KEY` está presente en la instancia local, el botón invoca la función asíncrona `getToken(messaging, { vapidKey })`, recuperando el identificador único para inyectar mensajes directos en las pruebas técnicas de QA.
3. **Escritura y Borrado Firestore:**
   Para prevenir acumulaciones de datos basuras, la prueba local de base de datos corre secuencialmente: primero escribe en la colección interna `_diagnostics` un documento temporal con timestamps, después verifica su existencia exacta por su ID generado, y finalmente lo elimina de la nube para mantener una base de datos limpia de telemetrías basuras.
...
sequenceDiagram
    autonumber
    actor Desarrollador
    participant UI as DeveloperDiagnosticsModal
    participant FS as Firestore Local
    participant FCM as Firebase Messaging (VAPID)

    Desarrollador->>UI: Clic en pestaña "Pings Firebase"
    Desarrollador->>UI: Clic en "Ping Firestore Local"
    UI->>UI: Registra log inicial e inicia temporizador
    UI->>FS: addDoc(collection(db, "_diagnostics"))
    FS-->>UI: Retorna ID de documento creado
    UI->>FS: getDoc(doc(db, "_diagnostics", ID))
    FS-->>UI: Retorna datos del documento
    UI->>FS: deleteDoc(doc(db, "_diagnostics", ID))
    FS-->>UI: Confirma remoción del documento
    UI->>UI: Detiene temporizador y calcula latencia
    UI-->>Desarrollador: Imprime logs e indicador en verde con los ms totales
```
