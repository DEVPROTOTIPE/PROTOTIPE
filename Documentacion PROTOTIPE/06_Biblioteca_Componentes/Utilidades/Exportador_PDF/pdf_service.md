...
* **Generación de Reportes Financieros:** Exporta el listado de pedidos exitosos (completados) en un rango de fechas con desglose y sumatoria de métodos de pago (efectivo, transferencia, crédito) y métricas clave (ticket promedio).
* **Análisis de Rotación e Inventario:** Genera reportes de stock consolidando las ventas contra los umbrales de alerta física de los productos para dar recomendaciones inteligentes ("Surtir Urgente", "Descontinuar / Liquidar").
* **Recibos de Comisión del Desarrollador:** Genera recibos formales de comisiones mensuales que incluyen datos de cobro, desglose de pedidos y un área para la firma digital (canvas).

---

## 2. Especificación Operativa (Lógica Pura)
Este componente opera como una utilidad JS pura sin maquetado visual, exponiendo métodos funcionales para crear documentos PDF consistentes y estilizados con una paleta de colores homogénea. Todas las dependencias e integraciones específicas de negocio (como formatters o almacenes de estado) son inyectables por parámetros, logrando un desacoplamiento total.

---

## 3. Código Javascript Completo y 100% Funcional (Decoupled & Generic)
...
/**
 * Standard currency formatter fallback if none is provided.
 */
...
export async function exportSalesReportPDF({
  ...
  const {
    ...
    orderCompletedState = 'COMPLETED',
    paymentMethods = { CASH: 'CASH', TRANSFER: 'TRANSFER', CREDIT: 'CREDIT' },
  ...
  const filtered = orders.filter(o => {
    if (o.estado !== orderCompletedState) return false
    const fecha = toLocalDate(o.createdAt)
    if (!fecha) return false
    if (!creditsEnabled && o.metodoPago === paymentMethods.CREDIT) return false
  ...
  filtered.forEach(o => {
    if (o.metodoPago === paymentMethods.CASH) {
      cashTotal += o.total
    } else if (o.metodoPago === paymentMethods.TRANSFER) {
      transferTotal += o.total
    } else if (o.metodoPago === paymentMethods.CREDIT) {
      creditTotal += o.total
    }
  })
  ...
  const tableData = orderCalculations.map(item => {
    ...
    const pago = o.metodoPago === paymentMethods.CASH 
      ? 'Efectivo' 
      : o.metodoPago === paymentMethods.TRANSFER 
      ? 'Transferencia' 
      : 'Credito'
 ...
 */
export async function exportRotationReportPDF({
  dateFrom,
  dateTo,
  orders,
  products,
 ...
 */
export async function exportDeveloperReceiptPDF({
  signatureDataUrl,
  orders,
  config = {},
  billingMetrics = {},
...
* **Módulo Autónomo:** Opera como funciones puras desacopladas del ciclo de vida de React, recibiendo todo el estado necesario mediante parámetros estructurados.
...
```mermaid
sequenceDiagram
    participant Componente as Vista Admin
    participant Servicio as pdfService.js
    participant jsPDF as jsPDF Engine
    participant autoTable as AutoTable Plugin
    
    Componente->>Servicio: exportSalesReportPDF({ dateFrom, dateTo, orders })
    Servicio->>Servicio: Filtrar pedidos por fecha y estado COMPLETADO
    Servicio->>Servicio: Calcular sumatoria por métodos de pago y ticket promedio
    Servicio->>jsPDF: Instanciar documento (new jsPDF)
    Servicio->>jsPDF: Pintar cabecera y rectángulos de resumen
    Servicio->>autoTable: Formatear tabla striped (autoTable)
    Servicio->>jsPDF: Guardar archivo (doc.save)
```
