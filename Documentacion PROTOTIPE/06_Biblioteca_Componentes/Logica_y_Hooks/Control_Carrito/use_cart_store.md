<!--
{
  ...
  "targetPath": "src/features/cart/hooks/useCartStore.js",
  "dependencies": {
    "npm": {},
    "internal": []
  },
...
export default function CartBadge() {
  ...
  return (
    <button onClick={openCart} className="relative p-2 bg-[var(--color-surface)] rounded-full">
      🛒 Carrito
      {count > 0 && (
        <span className="absolute -top-1.5 -right-1.5 bg-indigo-500 text-[var(--color-text)] rounded-full text-[10px] w-5 h-5 flex items-center justify-center font-bold">
          {count}
        </span>
      )}
    </button>
