<!--
{
  "resource": "ListaDiagnosticoFallas",
  "technicalName": "ListaDiagnosticoFallas",
  "targetPath": "src/components/common/ListaDiagnosticoFallas.jsx",
  "dependencies": {
    "npm": {
...
Este componente proporciona un árbol de diagnóstico interactivo para identificar fallas comunes en unidades de aire acondicionado, detallando su nivel de severidad y el costo estimado de la reparación técnica.

## 1. Propósito y Casos de Uso
* **Auto-Diagnóstico de Clientes:** Permite a los usuarios identificar qué le pasa a su aire acondicionado (no enfría, tira agua, hace ruidos extraños) antes de solicitar la visita del técnico.
* **Control de Asistencia Técnica:** Asiste a los despachadores en la categorización de urgencias y en la asignación del perfil de técnico correcto.

## 2. Especificación Visual y Estilos (Tailwind CSS)
* **Cuadrícula de Síntomas:** Botones interactivos con hover de elevación y glows HSL de marca blanca.
* **Chips de Severidad Semánticos:** Badges con colores de alerta (`text-red-500` / `text-amber-500` / `text-green-500`) según el tipo de daño detectado.
* **Consumo de CustomSelect:** Reemplazo de selectores nativos para filtrar por síntomas generales.
...
export default function ListaDiagnosticoFallas({
...
* **Actualización en runtime:** Cambia la lista de causas probables y severidades reactivamente al seleccionar un síntoma a través de `CustomSelect`.
* **Despacho del Diagnóstico:** Invoca a `onSelectDiagnostic` propagando la causa elegida para iniciar el flujo de solicitud de cotización formal.

## 5. Flujo Operativo y Secuencia de Interacción

```mermaid
sequenceDiagram
    participant Cliente as Usuario en App
    participant UI as ListaDiagnosticoFallas
    participant Parent as Formulario Servicio

    Cliente->>UI: Selecciona síntoma "Gotea agua"
    UI->>UI: Actualiza severidad a "Moderada"
    UI->>UI: Carga causas de drenaje obstruido
    Cliente->>UI: Hace clic en la causa "Limpieza de Drenaje"
    UI->>Parent: Propaga onSelectDiagnostic con datos de la falla y costo
```
