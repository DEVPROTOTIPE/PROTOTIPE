<!--
{
  "resource": "CalculadoraCargaBTU",
  "technicalName": "CalculadoraCargaBTU",
  "targetPath": "src/components/common/CalculadoraCargaBTU.jsx",
  "dependencies": {
    "npm": {
...
Este componente permite calcular los BTUs y toneladas de refrigeración necesarios para aclimatar un área basándose en sus dimensiones físicas, ocupación habitual y nivel de exposición al sol.

## 1. Propósito y Casos de Uso
* **Dimensionamiento en Venta:** Ayuda a los usuarios a saber qué capacidad de aire acondicionado comprar (9000, 12000, 18000, 24000 BTU) para su recámara o local comercial.
* **Presupuestador de Servicio Técnico:** Utilizado por instaladores HVAC para realizar levantamientos de carga rápidos directamente desde el móvil.

## 2. Especificación Visual y Estilos (Tailwind CSS)
* **Sliders Interactivos:** Deslizadores premium HSL para ajustar el largo, ancho y altura de la habitación.
* **Chips de Exposición Solar:** Selector estructurado mediante `CustomSelect` para elegir entre exposición alta (sol constante) o baja.
* **Dial de Capacidad (Marca Blanca):** Indicador visual que destaca la capacidad recomendada con colores semánticos en base al cálculo final.

## 3. Código React Completo

```jsx
import React, { useState, useMemo } from 'react';
import { ShieldCheck, HelpCircle, Compass, ThermometerSun } from 'lucide-react';
import CustomSelect from '../ui/CustomSelect';

export default function CalculadoraCargaBTU({
  onCalculate = null,
  sunExposureOptions = [
  ...
  const calculatedLoad = useMemo(() => {
    // Cálculo base de carga frigorífica simplificada (ASHRAE adaptado a climas latinos)
    const volume = width * length * height;
    
    // Carga base por volumen: 200 BTU por metro cúbico
    let baseBtu = volume * 200;

    // Carga por personas: 500 BTU por persona extra (más de 1)
    const peopleBtu = Math.max(0, people - 1) * 500;

    // Carga por electrodomésticos: 600 BTU por aparato
    const appliancesBtu = appliances * 600;

    // Incremento por alta exposición solar (20% de incremento)
    let totalBtu = baseBtu + peopleBtu + appliancesBtu;
    if (exposure === 'high') {
      totalBtu = totalBtu * 1.2;
    }

    // Convertir a toneladas de refrigeración (1 Tonelada = 12000 BTU/h)
    const tons = totalBtu / 12000;

    // Determinar equipo comercial recomendado
