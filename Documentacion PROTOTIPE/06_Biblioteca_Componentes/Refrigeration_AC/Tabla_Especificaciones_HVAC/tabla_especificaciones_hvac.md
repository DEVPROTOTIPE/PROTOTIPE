<!--
{
  "resource": "TablaEspecificacionesHVAC",
  "technicalName": "TablaEspecificacionesHVAC",
  "targetPath": "src/components/common/TablaEspecificacionesHVAC.jsx",
  "dependencies": {
    "npm": {
...
Este componente proporciona una tabla técnica comparativa de compresores y unidades condensadoras, detallando amperajes de arranque/marcha, frigorías, tipo de refrigerante y aceite recomendado.

## 1. Propósito y Casos de Uso
* **Especificación de Taller:** Ayuda al técnico a validar los valores nominales de amperaje de un equipo para constatar si está sobrecargado o defectuoso.
* **Filtro de Compra B2B:** Para contratistas que necesitan cotizar compresores de repuesto específicos.

## 2. Especificación Visual y Estilos (Tailwind CSS)
* **Tabla Estructurada:** Rejilla responsiva con scroll horizontal y padding vertical anti-clipping en hover.
* **Tipografía Monospace:** Valores numéricos de amperajes y BTUs estilizados con fuente monoespaciada para facilitar la comparación rápida.
* **Control Select Premium:** Consumo obligatorio de `CustomSelect` para filtrar los registros por capacidad (de 9000 a 24000 BTU).
...
export default function TablaEspecificacionesHVAC({
  selectedModel = '',
  onChange = null,
  filterOptions = [
...
* **Filtros en useMemo:** Filtra en memoria los compresores listados al cambiar la opción del desplegable `CustomSelect`, evitando re-renders innecesarios.
* **Ruteo de Selección:** Almacena la fila activa localmente y emite los metadatos completos del modelo seleccionado hacia el controlador superior.

## 5. Flujo Operativo y Secuencia de Interacción

```mermaid
sequenceDiagram
    participant Tecnico as Ingeniero HVAC
    participant UI as TablaEspecificacionesHVAC
    participant State as useMemo State

    Tecnico->>UI: Filtra por "12,000 BTU"
    UI->>State: Setea filtro a '12k'
    State->>UI: Filtra y renderiza compresores de 12k
    Tecnico->>UI: Clic en fila "Compresor Inverter 12k"
    UI->>UI: Resalta fila con fondo HSL activo
    UI->>Tecnico: Expone RLA de 4.4A y LRA de 18.0A
```
