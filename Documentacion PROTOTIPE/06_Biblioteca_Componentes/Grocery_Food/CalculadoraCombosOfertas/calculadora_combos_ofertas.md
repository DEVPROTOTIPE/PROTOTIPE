<!--
{
  "resource": "CalculadoraCombosOfertas",
  "technicalName": "CalculadoraCombosOfertas",
  "targetPath": "src/components/common/CalculadoraCombosOfertas.jsx",
  "type": "component",
  "niches": ["grocery_food"],
  "dependencies": {
    "npm": {
...
Permite visualizar, simular y armar combos de productos promocionales cruzados (ej. arroz + aceite, granos + pasta), calculando interactivamente el ahorro monetario neto al comparar la compra en paquete contra la compra de artículos individuales.

## 1. Propósito y Casos de Uso
* **Promociones en Supermercado:** Impulsar la venta cruzada de artículos de la misma categoría o complementarios.
* **Carrito de Compra del Cliente:** Mostrar alertas de combos completados en tiempo real si el usuario tiene los artículos requeridos en su canasta.
* **Simulador Comercial de Ofertas:** Permitir al administrador configurar y ajustar el margen de descuento de combos promocionales.

## 2. Especificación Visual y Estilos
* **Desglose de Artículos del Combo:** Lista con checkbox para indicar la inclusión/exclusión o selección de variantes.
* **Visualizador de Ahorro:** Barra de progreso u indicador visual del ahorro neto y el porcentaje descontado.
* **Estado de Combo Completado:** Animación y badge destacado con destello de color si se cumplen los requisitos del combo.

## 3. Código React Completo

```jsx
import React, { useState, useEffect, useMemo } from 'react';
import { Tag, Sparkles, Check, Info, Plus, Minus, Gift, ShoppingBag, ArrowRight } from 'lucide-react';
...
const COMBO_TEMPLATES = [
  {
    ...
    comboPrice: 13500 // Suma individual: 16100 (Ahorro: 2600)
  },
  {
    id: 'C02',
    ...
    comboPrice: 22000 // Suma individual: 26600 (Ahorro: 4600)
  },
  {
    id: 'C03',
    ...
    comboPrice: 28500 // Suma individual: 33650 (Ahorro: 5150)
  }
];

export default function CalculadoraCombosOfertas({
  onAddCombo = () => {}
}) {
  const [activeComboId, setActiveComboId] = useState(COMBO_TEMPLATES[0].id);
  const [selectedOptions, setSelectedOptions] = useState({}); // Mapeo de item.id -> opción seleccionada
  ...
  // Inicializar opciones seleccionables para el combo activo
  useEffect(() => {
    const initialOpts = {};
  ...
  // Calcular precio sumado individual
    ...
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl shadow-xl w-full max-w-4xl mx-auto p-6 text-[var(--color-text)]">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-7 bg-[var(--color-surface-2)] border border-[var(--color-border)]/50 rounded-xl p-5 flex flex-col justify-between">
          ...
          {/* Comparador de Ahorro y Totales */}
          <div className="mt-6 border-t border-[var(--color-border)] pt-4">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-[var(--color-primary)] text-[var(--color-text)] hover:bg-[var(--color-primary)]/90 py-2.5 rounded-xl text-xs font-bold shadow-sm transition"
            >
...
* El componente inicializa dinámicamente las opciones configurables mediante un `useEffect` disparado al cambiar el combo seleccionado (`activeComboId`).
* Computa en tiempo real los totales y porcentajes de descuento a través de `useMemo` para evitar re-cálculos redundantes, manteniendo una excelente fluidez.

## 5. Secuencia de Interacción
```mermaid
sequenceDiagram
    participant User as Cajero / Cliente
    participant Combo as Componente Combo
    participant Memory as Estado Local

    User->>Combo: Cambia opción de Papa Pastusa a Papa Criolla
    Combo->>Memory: Llama a handleOptionChange("I03", "Papa Criolla")
    Memory->>Combo: Actualiza opción en pantalla sin alterar precio del combo
    User->>Combo: Incrementa cantidad de Combos a 2
    Combo->>Memory: Llama a setQuantity(2)
    Memory->>Combo: Recalcula total final del combo (ej: $13,500 * 2 = $27,000)
    User->>Combo: Hace clic en "Llevar Combo Promocional"
    Combo->>User: Emite callback con desglose del combo y opciones elegidas
```
