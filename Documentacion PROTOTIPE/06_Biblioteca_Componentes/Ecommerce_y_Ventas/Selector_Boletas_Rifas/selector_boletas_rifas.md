...
export default function RaffleNumberSelector({
  ...
  return (
    <div className="w-full max-w-xl mx-auto bg-[var(--color-surface)]/60 backdrop-blur-xl border border-white/10 p-5 rounded-3xl text-[var(--color-text)] shadow-2xl relative overflow-hidden">
      {/* Cabecera */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-[10px] text-[var(--color-text-muted)]/80 mt-0.5">
          ...
          <div className="flex flex-wrap items-center gap-3 self-end">
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                className="w-10 px-1.5 py-1 text-center text-xs font-bold bg-[var(--color-bg)]/60 border border-white/10 rounded-lg text-[var(--color-text)] placeholder-slate-600 focus:outline-none focus:border-indigo-500 transition"
              />
              <button
      ...
      >
        {numbersList.map((num) => {
          ...
          if (status === 'sold') {
            cellClass = "bg-[var(--color-surface-1)]/40 border-[var(--color-border)] text-slate-600 cursor-pointer opacity-50";
          } else if (status === 'reserved') {
            cellClass = "bg-amber-500/10 border-amber-500/30 text-amber-400 animate-pulse cursor-pointer";
          } else if (status === 'selected') {
            cellClass = "border-transparent text-[var(--color-text)] font-black scale-105";
            cellStyle = {
            ...
            cellClass = "border-transparent text-[var(--color-text)] font-black scale-110";
            cellStyle = {
              background: 'linear-gradient(135deg, #ec4899, #8b5cf6)',
              animation: 'elasticPop 150ms infinite'
            };
          } else {
            cellClass = "bg-[var(--color-surface)]/40 border-white/5 text-[var(--color-text-muted)] hover:border-white/20 hover:scale-105 transition-all duration-200 cursor-pointer";
          }
      ...
      {/* Leyenda */}
      <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 mt-5 text-[10px] font-medium text-[var(--color-text-muted)]/80 border-t border-white/5 pt-4">
        <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded bg-[var(--color-surface-1)]/40 border border-[var(--color-border)] opacity-50 relative overflow-hidden">
    ...
    <div className="w-full p-4 bg-[var(--color-surface)] min-h-screen">
      ...
      {selectedNumbers.length > 0 && (
        <div className="mt-4 p-4 max-w-xl mx-auto bg-[var(--color-bg)]/50 border border-white/5 rounded-2xl flex items-center justify-between animate-fade-in">
          <div>
            <h5 className="text-xs font-bold text-[var(--color-text)] uppercase tracking-wider">Boletas Seleccionadas</h5>
            <p className="text-lg font-black text-indigo-400 mt-1.5 flex gap-1.5 flex-wrap">
              {selectedNumbers.sort().map((n) => (
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-0.5 bg-indigo-500/20 hover:bg-red-500/20 border border-indigo-500/30 hover:border-red-500/30 rounded-lg text-xs text-[var(--color-text)] hover:text-red-200 transition duration-200 group cursor-pointer"
                >
                  <span>{n}</span>
          ...
          <div className="flex flex-col gap-2 min-w-[160px] self-center">
            <button
              ...
              className="relative overflow-hidden px-4 py-2.5 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-500 hover:via-purple-500 hover:to-pink-500 rounded-xl text-xs font-black text-[var(--color-text)] uppercase tracking-wider transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-purple-500/25 hover:shadow-purple-500/50 cursor-pointer text-center"
            >
              Confirmar {selectedNumbers.length} boleta(s)
            </button>
            <button
              onClick={() => setSelectedNumbers([])}
              className="px-3 py-1.5 bg-[var(--color-bg)]/40 hover:bg-red-500/10 border border-white/5 hover:border-red-500/20 rounded-lg text-[10px] font-bold text-[var(--color-text-muted)]/80 hover:text-red-400 tracking-wider uppercase transition-all duration-200 cursor-pointer text-center"
            >
              Limpiar Selección
            </button>
          </div>
