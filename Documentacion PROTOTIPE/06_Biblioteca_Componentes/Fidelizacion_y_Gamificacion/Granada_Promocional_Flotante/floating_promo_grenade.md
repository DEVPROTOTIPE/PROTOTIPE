<!--
{
  "resource": "FloatingPromoGrenade",
  "technicalName": "FloatingPromoGrenade",
  "targetPath": "src/components/common/FloatingPromoGrenade.jsx",
  "type": "component",
  "niches": ["retail_clothing", "grocery_food", "licores-cocteleria", "coleccionismo-geek"],
  "dependencies": {
    "npm": {
...
# Granada Promocional Flotante (`FloatingPromoGrenade`)

El componente `FloatingPromoGrenade` es un desencadenador interactivo flotante gamificado de alta conversión. Presenta una granada flotante animada que, al hacer clic, simula un efecto de ignición/explosión mediante Framer Motion y `canvas-confetti` para luego revelar un modal glassmorphic con ofertas y códigos de cupón dinámicos.

## 1. Propósito y Casos de Uso
* **Llamado a la Action de Alto Impacto:** Ideal para campañas de ventas flash, promociones de primera compra o cupones de carrito abandonado.
* **Gamificación del eCommerce:** Reemplaza los popups tradicionales estáticos e invasivos por una micro-interacción lúdica impulsada por la curiosidad.
* **Fidelización y Retención:** Ocupa un espacio no intrusivo en el viewport móvil (bottom-24) que coexiste perfectamente con la barra inferior.

---

## 2. Especificación Visual y Estilos (Tailwind CSS)
El componente consume variables cromáticas de diseño HSL nativas para adaptarse dinámicamente a la marca del cliente:
* **Fondo del Modal:** `bg-[var(--color-surface)]` con efecto backdrop-blur y sombra `shadow-soft-2xl`.
* **Botones e Indicadores:** `bg-[var(--color-primary)]` y `text-[var(--color-primary)]` para el branding.
* **Bordes y Divisores:** `border-[var(--color-border)]` para una separación sutil.
* **Halo de Levitación:** Emplea la animación `animate-ping` con opacidad reducida (`opacity-40`) en una capa inferior (`-z-10`) para inducir al clic.
* **Corte Limpio (Glassmorphism):** Un gradiente superior interno `from-[var(--color-primary)]/15` otorga un acabado holográfico de alta calidad.

---

## 3. Código React Completo
Crea este archivo en [FloatingPromoGrenade.jsx](file:///d:/PROTOTIPE/Plantillas%20Core/App%20Ventas/src/components/common/FloatingPromoGrenade.jsx):
      ...
      <AnimatePresence>
        {phase === 'floating' && (
          <motion.button
            ...
            className="fixed bottom-24 right-6 z-40 flex items-center justify-center w-14 h-14 rounded-full bg-[var(--color-primary)] text-white shadow-soft-xl border border-white/20 active:scale-95 transition-transform"
          >
...
* **Ignición y Sincronía Temporal:** El método `handleIgnite` cambia el estado a `ignited` activando la animación de expansión masiva en Framer Motion (`scale: [1, 1.3, 0.8, 2, 40]`). Un timer asíncrono de `600ms` detona el confeti físico y transiciona a la fase `revealed` para dar la sensación de que el modal emerge a través de la explosión.
* **Salida de AnimatePresence:** La animación de transición permite desmontar los elementos de manera fluida y suave sin cortes bruscos.
...
sequenceDiagram
    autonumber
    actor Usuario
    participant G as FloatingPromoGrenade (UI)
    participant C as canvas-confetti
    
    Note over G: Fase 'floating': Granada levitando
    Usuario->>G: Clic en Granada
    Note over G: Fase 'ignited': Animación Shake/Inflación
    G->>C: Detonar explosión de partículas (600ms)
    G->>G: Fase 'revealed': Abrir modal con efecto spring
    Usuario->>G: Clic en Aplicar Cupón u ocultar modal
    G->>G: Volver a Fase 'floating'
```
