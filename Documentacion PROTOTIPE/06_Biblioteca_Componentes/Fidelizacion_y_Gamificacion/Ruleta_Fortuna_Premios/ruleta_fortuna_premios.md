...
- **Fidelización en Checkout:** Incentiva al cliente final a completar su pedido en restaurantes o comercios otorgándole un giro al azar para obtener descuentos en su próxima compra.
- **Promociones Estacionales:** Módulo configurable desde Firestore para campañas especiales (ej: Día de las Madres, Black Friday).

---

## 2. Especificación Visual y Estilos (Tailwind CSS)
- **SVG Responsivo:** Renderiza las divisiones de la rueda con cálculos trigonométricos dinámicos para ajustarse a cualquier número de segmentos (CSV) con separadores blancos translúcidos (`rgba(255,255,255,0.2)`).
- **Aro Exterior Dorado Metálico:** Dos aros concéntricos con degradado radial y lineal dorado y 24 bombillas LED circulares animadas que simulan luces de casino parpadeantes.
- **Pointer Superior Premium:** Indicador estilizado en 3D vectorial con degradados metálicos y una gema rubí roja en el centro.
- **Textos Radiales:** Alineación al exterior (`textAnchor="end"`) y rotación sincronizada con la bisectriz del sector para evitar que se solapen con el botón central de giro.
...
export default function RaffleWheelOfFortune({
  prizesText = '10% OFF, Bebida Gratis, Postre Gratis, Café Gratis, Reintenta, Descuento $5k',
  onWin = () => {},
  duration = 5000,
  spinsCount = 6,
  ...
  const spin = () => {
    ...
    const targetRotation = (spinsCount * 360) - (winningIndex * angle) - 90;
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-4 select-none text-[var(--color-text)]">
      <style>{`
        @keyframes bulbGlow {
      ...
      <div className="relative w-64 h-64 flex items-center justify-center">
        ...
        <div
          style={{
            ...
            transition: isSpinning ? `transform ${duration}ms cubic-bezier(0.1, 0.8, 0.1, 1)` : 'none'
          }}
        ...
        >
          <svg viewBox="0 0 200 200" className="w-full h-full">
            ...
            {/* Centro Decorativo */}
            <circle cx="100" cy="100" r="28" fill="url(#wheelCenter)" stroke="url(#goldMetallic)" strokeWidth="2.5" />
          </svg>
        </div>
        ...
        <button
          ...
          className={`absolute z-30 w-14 h-14 rounded-full flex flex-col items-center justify-center text-[10px] font-black uppercase shadow-[0_6px_20px_rgba(217,119,6,0.3)] transition-all duration-300 cursor-pointer ${
            isSpinning
              ...
              : 'bg-gradient-to-tr from-amber-500 via-rose-500 to-violet-600 text-[var(--color-text)] hover:scale-105 active:scale-95 hover:shadow-[0_6px_24px_rgba(217,119,6,0.5)] border border-white/20'
          }`}
      ...
      {prizeResult && (
        <div className="w-full max-w-xs bg-[var(--color-bg)]/60 border border-white/10 rounded-2xl p-3 text-center space-y-2 animate-fade-in shadow-xl">
          <span className="text-[9px] font-bold text-[var(--color-text-muted)]/80 uppercase tracking-widest">¡Felicidades, ganaste!</span>
...
1. **Inercia con Bezier:** Utilizamos una curva cúbica `cubic-bezier(0.1, 0.8, 0.1, 1)` para lograr una aceleración instantánea y una desaceleración amortiguada de 5 segundos.
2. **Confeti Dinámico:** El script carga `'canvas-confetti'` de forma diferida mediante un `import()` dinámico para no sobrecargar el bundle inicial de las páginas.
