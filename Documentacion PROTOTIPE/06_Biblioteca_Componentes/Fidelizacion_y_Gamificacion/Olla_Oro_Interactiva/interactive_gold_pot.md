<!-- { "name": "InteractiveGoldPot", "technicalName": "InteractiveGoldPot", "category": "Fidelizacion_y_Gamificacion", "targetPath": "src/components/common/InteractiveGoldPot.jsx", "tags": ["alcancía", "ahorro", "fidelización", "olla de oro", "framer motion", "gamificación", "abonos"] } -->

# 🪙 InteractiveGoldPot (Olla de Oro / Ahorro Interactivo)

## 1. Propósito y Casos de Uso
El componente `InteractiveGoldPot` (Olla de Oro Interactiva) es un módulo gamificado premium diseñado para simular abonos y metas de ahorro. Utilizando una olla de oro en SVG vectorial hecha a mano y animaciones de impacto coordinadas con Framer Motion, incentiva visualmente al usuario a saldar deudas o realizar aportes financieros de manera dinámica.
Ideal para módulos de fiados, e-commerce, y la sección de abonos de clientes (`ClientCredits.jsx`).

---

## 2. Especificación Visual y Estilos
- **Olla de Oro en SVG:** Diseño vectorial metálico con gradientes de slate, boca con borde de oro y monedas doradas que rebosan en la cima.
- **Escalamiento Proporcional Gradual:** El contenedor exterior del objeto se expande proporcionalmente a la meta de ahorro (`scale` dinámico de `1.0` a `1.35` según el progreso de ahorro), simbolizando que la olla "se llena y se agranda".
- **Efecto Squash & Stretch:** Animación de compresión y estiramiento del cuerpo de la olla (`scale: [1, 1.18, 0.82, 1.06, 1]`) gatillada al caer la moneda de dólar.
- **Física de Moneda:** Trayecto vertical de gravedad coordinado con `AnimatePresence` y giro de 180° antes de insertarse en la boca de la olla.
- **Resplandor Semántico:** Halo ambiental brillante (`bg-amber-500/25 blur-xl`) y celebración con confeti de alta fidelidad cuando el acumulado iguala o supera la meta.
...
|---|---|---|---|
| `initialAmount` | `number` | `150000` | Cantidad inicial ya acumulada en la olla. |
| `goalAmount` | `number` | `500000` | La meta total de ahorro o deuda a cubrir. |
| `onGoalReached` | `function` | `() => {}` | Callback gatillado una vez se alcanza la meta de ahorro. |
