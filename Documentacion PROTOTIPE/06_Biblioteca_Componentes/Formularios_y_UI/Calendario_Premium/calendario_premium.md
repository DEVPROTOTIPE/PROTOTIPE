...
export default function DatePickerPremium({
  ...
  return (
    <div ref={containerRef} className={`relative w-full max-w-sm ${className}`}>
      ...
      {/* Calendario Popover */}
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 z-[999] bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row max-w-full max-w-[31.25rem]">
                ...
                return (
                  <button
                    ...
                    className={`relative aspect-square flex items-center justify-center text-[10px] rounded-lg cursor-pointer select-none font-bold transition-all focus:outline-none ${
                      ...
                      isSelected
                        ? 'bg-indigo-600 dark:bg-indigo-500 text-[var(--color-text)] shadow-md hover:bg-indigo-500 rounded-lg scale-105 z-10'
                        : isInRange
