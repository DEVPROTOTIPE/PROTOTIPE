...
* **Gestión de Catálogo Comercial:** Administración de las categorías asociadas a productos en sistemas POS o plataformas de E-commerce.
* **Interfaz Autocontenida:** Proporciona un selector visual de íconos integrados categorizados para enriquecer la estética del menú de productos.
* **Búsqueda Avanzada de Íconos:** Permite filtrar íconos mediante palabras clave (tags) para facilitar la asignación.

---

## 2. Especificación Visual y Estilos (Tailwind CSS)
* **Diseño Adaptativo (Mobile-First):** Grid auto-ajustable para la rejilla de selección de íconos.
* **Paleta de Colores HSL y Modo Oscuro:**
  * Contenedor: `bg-surface border-app shadow-sm`
  * Campos y Botones Secundarios: `bg-surface-2 border-app text-app`
  * Elemento Seleccionado/Acción Primaria: `bg-primary text-white border-primary`
  * Botones de Acción Rápida (Edición/Eliminación): Estados de hover dinámicos y color de error (`text-error hover:bg-red-50 dark:hover:bg-red-900/20`).
...
// Diccionario de íconos SVG autocontenidos (Cero dependencias externas)
const SVG_ICONS = {
  Plus: (props) => (
          ...
          {editingId ? (
            <div className="flex gap-2">
              <button
                type="submit"
                disabled={isUpdating}
                className="flex-1 sm:flex-none h-11 px-5 bg-primary text-[var(--color-text)] rounded-xl font-bold transition-all active:scale-95 disabled:opacity-50 text-xs sm:text-sm"
              >
                Guardar
              </button>
              <button
            ...
            <button
              ...
              className="w-full sm:w-auto h-11 px-5 bg-primary text-[var(--color-text)] rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50 text-xs sm:text-sm"
            >
                ...
                return (
                  <button
                    ...
                    className={`w-9 h-9 rounded-lg flex items-center justify-center border transition-all active:scale-95 shrink-0 ${
                      iconName === name
                        ? 'bg-primary text-[var(--color-text)] border-primary shadow-sm scale-105'
...
| :--- | :--- | :--- | :--- |
| `categories` | `Array` | `[]` | Listado de categorías a desplegar. Formato: `[{ id: string, nombre: string, iconName: string }]`. |
| `isLoading` | `Boolean` | `false` | Indica si se está ejecutando la lectura inicial de las categorías. |
| `isCreating` | `Boolean` | `false` | Estado de carga al crear una nueva categoría. Deshabilita el botón de agregar. |
| `isUpdating` | `Boolean` | `false` | Estado de carga al actualizar una categoría existente. |
| `onCreate` | `Function` | `() => {}` | Callback gatillado al registrar una categoría. Recibe `{ nombre: string, activa: true, iconName: string }`. |
| `onUpdate` | `Function` | `() => {}` | Callback gatillado al guardar cambios. Recibe `(id, { nombre: string, iconName: string })`. |
| `onDelete` | `Function` | `() => {}` | Callback gatillado al eliminar. Recibe `(id)`. |

---

## 5. Lógica de Estado y Ciclo de Vida
* **Estado Local del Formulario:** Maneja independientemente los inputs de texto, selección y término de búsqueda del ícono (`nombre`, `iconName`, `editingId`, `searchTermIcon`).
* **Ciclo de Edición:** Al hacer clic en editar, se carga temporalmente el ID de la categoría y sus valores en el estado local. Al cancelar o guardar con éxito, se restablece el formulario al estado inicial.
...
sequenceDiagram
    participant Usuario
    participant Componente as CategoryManager (UI)
    participant Padre as Componente Padre / Adaptador
    
    Usuario->>Componente: Escribe nombre y selecciona ícono
    Usuario->>Componente: Clic en "Agregar"
    Componente->>Padre: onCreate({ nombre, activa: true, iconName })
    Padre->>Componente: Nuevas 'categories' por props
    Componente->>Usuario: Actualiza lista visualmente
```

---

## 7. Ejemplo de Uso (Integración con Firebase)

```jsx
import React from 'react';
import CategoryManagerUI from '@/components/ui/CategoryManager';
import { useCategories, useCreateCategory, useDeleteCategory, useUpdateCategory } from '@/hooks/useInventory';

export function AdminCategoryWrapper() {
...
## 8. Origen, Fecha y Versión
* **Origen:** [CategoryManager.jsx](file:///D:/PROTOTIPE/App%20Ventas/src/components/admin/inventory/CategoryManager.jsx)
* **Fecha:** 2026-05-29
* **Versión:** 2.0 (White-label HSL, stateless y con SVG fallback).
