<!--
{
  "technicalName": "SeguimientoPedido",
  "targetPath": "src/components/ui/SeguimientoPedido.jsx",
  "dependencies": {
    "npm": {},
    "internal": []
  }
          ...
          <div className="bg-[var(--card-bg)] border border-slate-100 dark:border-[var(--color-border)]/80 rounded-3xl p-6 shadow-xl text-center">
            ...
            <form onSubmit={handleSearchSubmit} className="flex gap-2">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-6 py-3 rounded-2xl bg-primary text-[var(--color-text)] font-bold text-sm shadow-md flex items-center gap-2 hover:opacity-90 active:scale-95 transition-all"
              >
                <I.Search />
                Buscar
              </button>
                  ...
                  return (
                    <div key={idx} className="flex md:flex-col items-center gap-4 md:text-center w-full">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center border text-xs font-bold transition-all ${
                        isCompleted ? 'bg-primary border-primary text-[var(--color-text)] shadow-md' : 'border-slate-200 dark:border-[var(--color-border)] text-[var(--text-secondary)]'
                      }`}>
            ...
            <div className="flex flex-col sm:flex-row gap-2 pt-2">
              ...
              {whatsappSupport && (
                <a
                  ...
                  className="flex-1 py-3 rounded-2xl bg-emerald-600 text-[var(--color-text)] font-bold text-sm text-center"
                >
