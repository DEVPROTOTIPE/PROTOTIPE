            ...
            {[
              ...
              >
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-[var(--color-text-muted)]/80 leading-relaxed">{filter.desc}</p>
                </div>
        ...
        <div className="border-t border-slate-100 pt-6">
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-xs text-[var(--color-text-muted)]/80 mt-0.5">
          ...
          <div className="space-y-3.5">
            {filters.customAttributes?.map((attr, index) => (
              ...
              >
                ...
                <div className="flex-1 w-full">
                  <label className="block text-[10px] font-bold text-[var(--color-text-muted)]/80 uppercase mb-1.5">Nombre del filtro</label>
                  <input 
                    type="text" 
                ...
                <div className="w-full sm:w-auto shrink-0">
                  <label className="block text-[10px] font-bold text-[var(--color-text-muted)]/80 uppercase mb-1.5">Tipo de entrada</label>
                  <div className="flex bg-white border border-slate-200 rounded-xl overflow-hidden h-10 p-0.5">
                    <button 
                      ...
                      className={`px-4.5 rounded-lg text-xs font-extrabold transition-all cursor-pointer ${attr.type === 'text' ? 'bg-emerald-500 text-[var(--color-text)] shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
                    >
                      Texto
                    </button>
                    <button 
                      ...
                      className={`px-4.5 rounded-lg text-xs font-extrabold transition-all cursor-pointer ${attr.type === 'select' ? 'bg-emerald-500 text-[var(--color-text)] shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
                    >
                      Opciones
                    </button>
                  </div>
                ...
                {attr.type === 'select' && (
                  <div className="flex-[1.5] w-full">
                    <label className="block text-[10px] font-bold text-[var(--color-text-muted)]/80 uppercase mb-1.5">Opciones del filtro</label>
                    <input 
                      type="text" 
                    ...
                    />
                    <p className="text-[9px] text-[var(--color-text-muted)]/80 mt-1 px-1">Separa las opciones con comas.</p>
                  </div>
                )}
                ...
                <div className="sm:self-end">
                  <button 
                    ...
                    className="w-full sm:w-10 h-10 flex items-center justify-center shrink-0 rounded-xl text-[var(--color-text-muted)]/80 hover:bg-rose-50 hover:text-rose-500 border border-slate-200 hover:border-rose-100 transition-all cursor-pointer"
            ...
            {(!filters.customAttributes || filters.customAttributes.length === 0) && (
              <div className="text-center py-8 text-[var(--color-text-muted)]/80 text-xs border border-dashed border-slate-200 bg-slate-50/20 rounded-2xl">
      ...
      {onSave && (
        <div className="p-5 border-t border-slate-100 bg-slate-50/30 flex justify-end">
          <button 
            ...
            onClick={onSave}
            disabled={isSaving}
            className="w-full sm:w-auto h-11 px-6 bg-emerald-500 hover:bg-emerald-600 text-[var(--color-text)] rounded-xl text-xs font-extrabold transition-all duration-200 active:scale-95 flex items-center justify-center gap-2 shadow-sm disabled:opacity-50 shrink-0 cursor-pointer"
          >
            {isSaving ? (
