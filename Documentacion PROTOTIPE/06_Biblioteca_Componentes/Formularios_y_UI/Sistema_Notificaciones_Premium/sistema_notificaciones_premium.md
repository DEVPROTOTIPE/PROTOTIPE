        ...
        <AnimatePresence>
          {unread > 0 && (
            <motion.span
              ...
              className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 flex items-center justify-center bg-red-500 text-[var(--color-text)] text-[9px] font-black rounded-full shadow-lg shadow-red-500/30 pointer-events-none"
            >
              {unread > 99 ? '99+' : unread}
            </motion.span>
          )}
      ...
      {isTrayOpen && (
        <>
          ...
          <motion.div
            ...
            style={style}
            className="w-full max-w-[23.75rem] max-h-[520px] flex flex-col bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl shadow-2xl shadow-[var(--color-shadow)] overflow-hidden"
          >
            {/* Header */}
...
function ToastItem({ toast, index, total, onDismiss }) {
  ...
  return (
    <motion.div
      ...
      className="pointer-events-auto w-full max-w-[21.25rem] max-w-[calc(100vw-2.5rem)]"
    >
      <div
        className={`relative overflow-hidden rounded-2xl border ${cfg.border} bg-[var(--color-surface)] shadow-xl shadow-[var(--color-shadow)]`}
