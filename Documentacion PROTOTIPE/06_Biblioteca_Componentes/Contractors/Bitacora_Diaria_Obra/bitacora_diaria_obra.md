<!--
{
  "resource": "BitacoraDiariaObra",
  "technicalName": "BitacoraDiariaObra",
  "targetPath": "src/components/common/BitacoraDiariaObra.jsx",
  "dependencies": {
    "npm": {
...
Este componente modular permite llevar la bitácora técnica de obra diaria, reportar novedades (incidentes, clima, entregas de material), gestionar el control de asistencia rápida de las cuadrillas y simular la subida de evidencia fotográfica con progreso visual.

---

## 💎 Propósito y Casos de Uso
El control físico de las obras requiere reportes inmediatos para prevenir retrasos y justificar imprevistos. Este componente centraliza:
1. **Control de Asistencia de Cuadrilla**: Check rápido del personal operativo contratado para la jornada.
2. **Reporte de Novedades y Nivel de Clima**: Registro del clima diario (Lluvioso, Soleado) e incidentes que retrasen el avance.
3. **Archivo de Evidencia de Avance**: Simula la carga de fotografías del frente de trabajo, generando un registro histórico colapsable cronológico.

---

## 🎨 Especificación Visual y Estilos (Tailwind CSS)
* **Formulario Principal**: Panel responsivo con división clara de inputs de texto, asistencia rápida y drop-zone de fotos.
* **Carga de Archivos**: Zona de arrastre interactiva con borde punteado de foco y barra de progreso animada `bg-[var(--color-primary)]`.
* **Histórico de Hojas**: Lista ordenada verticalmente con tarjetas colapsables `bg-[var(--color-surface-2)]/30` y tags según la gravedad de la novedad (Normal, Alerta, Crítico).
...
export default function BitacoraDiariaObra({
  onSaveReport,
  confirmAction,
  personalInicial = [
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          <div className="bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] p-5 rounded-xl flex flex-col gap-4">
            ...
            <button
              ...
              className="w-full py-2.5 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-[var(--color-text)] font-bold rounded-xl flex items-center justify-center gap-2 transition-colors active:scale-[0.99]"
            >
              Registrar Jornada
            </button>
          </div>
              ...
              {personal.map(p => (
                ...
                >
                  ...
                  <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${
                    p.presente ? 'bg-emerald-500 border-transparent text-[var(--color-text)]' : 'border-[var(--color-border)]'
                  }`}>
...
1. **`clima`**: String que mapea la selección activa del clima (Soleado, Nublado, Lluvioso).
2. **`personal`**: Array de objetos de personal con flags de asistencia interactivos.
3. **`registros`**: Histórico local de hojas de bitácora ordenados en orden cronológico descendente.
4. **`cargandoImagen` / `progresoCarga`**: Controlan la barra de progreso animada para emular la carga de evidencias fotográficas asíncronas.
...
sequenceDiagram
    ...
    U->>B: Activa o desactiva checks de asistencia
    B->>B: Actualiza estado reactivo de personal
    U->>B: Selecciona archivo fotográfico
    B->>B: Inicia bucle temporizado de progreso (0% a 100%)
    B->>U: Renderiza miniatura de foto cargada
    U->>B: Clic en "Registrar Jornada"
    B->>B: Inserta nuevo registro al inicio del array
    U->>B: Clic en borrar bitácora histórica
    B->>AC: Pregunta de confirmación destructiva
    AC-->>B: Acepta
    B->>B: Remueve registro y actualiza UI
```
