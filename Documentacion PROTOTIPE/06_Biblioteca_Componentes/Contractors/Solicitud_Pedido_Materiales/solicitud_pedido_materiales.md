<!--
{
  "resource": "SolicitudPedidoMateriales",
  "technicalName": "SolicitudPedidoMateriales",
  "targetPath": "src/components/common/SolicitudPedidoMateriales.jsx",
  "dependencies": {
    "npm": {
...
Este componente implementa un formulario y carrito interno para que los directores de obra gestionen pedidos de materiales (varillas, sacos de cemento, arena, ladrillos) al almacén central o a proveedores homologados, indicando cantidades, prioridades y fechas de entrega críticas.

---

## 💎 Propósito y Casos de Uso
El desabastecimiento de materiales paraliza frentes de trabajo completos. Este componente agiliza:
1. **Petición Ágil de Insumos**: Formulario interactivo con agregación rápida de ítems a una lista de despacho en lote.
2. **Priorización por Urgencia**: Mapea visualmente niveles de urgencia (Normal, Urgente, Crítico) para optimizar el orden de entrega logística.
3. **Manejo de Unidades de Medida**: Selector de unidades normalizadas según el tipo de material (sacos, metros cúbicos, toneladas, unidades).

---

## 🎨 Especificación Visual y Estilos (Tailwind CSS)
* **Tabla de Requerimientos**: Rejilla responsiva con scroll horizontal protegido por padding (`py-4`) para previsualizar el carrito del pedido.
* **Badges de Prioridad**: Chips con colores HSL coherentes (`bg-red-500/10` para crítico, `bg-blue-500/10` para normal).
* **Confirmación de Envío**: Uso de confirmaciones asíncronas mediante `useAlertConfirm` al intentar purgar la lista del pedido en curso.
...
export default function SolicitudPedidoMateriales({
  onSubmitRequest,
  confirmAction,
  materialesCatalogo = [
    ...
    if (existeIndex > -1) {
      ...
      setItems([
        ...items,
        {
          ...
          cantidad,
          fechaEntrega: fechaEntrega ? fechaEntrega.toLocaleDateString('es-CO', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '',
          prioridad
        }
      ]);
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 flex flex-col gap-4">
          ...
          <button
            ...
            className="w-full py-2 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-[var(--color-text)] font-medium rounded-xl flex items-center justify-center gap-2 transition-colors mt-2"
          >
          ...
          <div className="bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] p-4 rounded-xl flex flex-col h-full justify-between">
            ...
            {items.length > 0 && (
              <div className="flex gap-3 pt-3 border-t border-[var(--color-border)] mt-4">
                ...
                <button
                  onClick={handleSend}
                  className="flex-1 py-2.5 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-[var(--color-text)] font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-[var(--color-primary)]/20 transition-all active:scale-[0.98]"
                >
...
1. **`items`**: Colección de insumos constructivos (varillas, cemento) agregados a la orden activa.
2. **`material`**: Identificador activo del selector para configurar la unidad de medida correspondiente.
3. **`cantidad` / `fechaEntrega` / `prioridad`**: Parámetros de validación en formulario para agregar nuevos requerimientos a la solicitud.
...
sequenceDiagram
    ...
    participant SPM as FormularioPedido
    participant AC as useAlertConfirm

    U->>SPM: Completa campos del material y clic en "Agregar"
    SPM->>SPM: Agrega objeto a items
    SPM->>U: Limpia formulario y actualiza lista de despacho
    U->>SPM: Clic en "Cancelar Pedido"
    SPM->>AC: Solicita confirmación
    AC-->>SPM: Confirma cancelar
    SPM->>SPM: Limpia array de items y vacía la interfaz
```
