<!--
{
  "resource": "ChecklistSeguridadEPP",
  "technicalName": "ChecklistSeguridadEPP",
  "targetPath": "src/components/common/ChecklistSeguridadEPP.jsx",
  "dependencies": {
    "npm": {
...
Este componente implementa un formulario interactivo y lista de chequeo de seguridad de EPP (Equipo de Protección Personal) obligatorio, adaptado dinámicamente al tipo de labor a realizar (excavación, soldadura, alturas, electricidad), garantizando el cumplimiento antes del ingreso a obra.

---

## 💎 Propósito y Casos de Uso
La prevención de accidentes laborales es obligatoria por ley y previene incidentes graves. Este componente centraliza:
1. **Pautas de Seguridad Dinámicas**: Los requisitos cambian según la tarea (ej: exige arnés de seguridad para alturas, careta para soldadura).
2. **Validación de Firma Digital**: Un simulador de aceptación de declaración juramentada de seguridad que valida que todos los checks críticos estén activos.
3. **Control de Ingreso Directo**: Genera un pase de acceso aprobado (con código de barra/QR simulado) si el contratista cumple con todo su equipamiento.

---

## 🎨 Especificación Visual y Estilos (Tailwind CSS)
* **Contenedor Premium**: Card estructurado `bg-[var(--color-surface)]` con contorno fino.
* **Badges y Chips**: Indicadores de criticidad HSL (`text-amber-500` para advertencias).
* **Confirmación de Seguridad**: Uso obligatorio del hook `useAlertConfirm` para reiniciar los checks de seguridad.
...
export default function ChecklistSeguridadEPP({
  onValidate,
  confirmAction,
  laboresConfig = [
    {
  ...
  const laborActiva = useMemo(() => {
    return laboresConfig.find(l => l.value === labor) || laboresConfig[0];
  }, [labor, laboresConfig]);

  const toggleCheck = (id) => {
    if (aprobado) return; // Si ya firmó, no cambiar
    setCheckedIds({
      ...checkedIds,
      [id]: !checkedIds[id]
    });
  ...
  // Validar si cumple con todos los requerimientos críticos
  const validacionEPP = useMemo(() => {
    const faltantes = [];
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-4 h-4 rounded border flex items-center justify-center ${
                        isChecked ? 'bg-[var(--color-primary)] border-transparent text-[var(--color-text)]' : 'border-[var(--color-border)]'
                      }`}>
            ...
            {!aprobado ? (
              <div className="flex flex-col gap-4 pt-3 border-t border-[var(--color-border)]">
                ...
                <button
                  ...
                  className="w-full py-3 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] disabled:bg-[var(--color-text-muted)]/20 disabled:text-[var(--color-text-muted)] disabled:cursor-not-allowed text-[var(--color-text)] font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg transition-all active:scale-[0.98]"
                >
...
1. **`labor`**: Indica la especialidad del frente (alturas, soldadura, etc.) para filiar los insumos requeridos.
2. **`checkedIds`**: Objeto clave-valor que mapea cuáles implementos de seguridad ha verificado físicamente el usuario.
3. **`aprobado`**: Flag de control de pase de acceso. Cuando se firma digitalmente, la interfaz congela la edición para evitar manipulaciones de seguridad.
...
sequenceDiagram
    ...
    participant CS as Checklist
    participant AC as useAlertConfirm

    U->>CS: Selecciona tipo de labor
    CS->>CS: Restablece checks y cambia listado de EPP requerido
    U->>CS: Marca ítems EPP en la lista
    CS->>CS: Valida si todos los de tipo "Crítico" están activos (useMemo)
    CS->>U: Habilita botón de firma e ilustra barra verde
    U->>CS: Rellena nombre y clica "Firmar Declaración"
    CS->>U: Bloquea edición y dibuja pase de acceso aprobado (código de barras)
    U->>CS: Clic en "Reiniciar Checklist"
    CS->>AC: Solicita confirmación
    AC-->>CS: Confirma
    CS->>CS: Limpia nombre, firma y checks
```
