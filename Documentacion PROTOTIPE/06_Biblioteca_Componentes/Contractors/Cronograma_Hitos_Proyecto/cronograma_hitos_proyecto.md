<!--
{
  "resource": "CronogramaHitosProyecto",
  "technicalName": "CronogramaHitosProyecto",
  "targetPath": "src/components/common/CronogramaHitosProyecto.jsx",
  "dependencies": {
    "npm": {
...
Este componente proporciona una interfaz visual e interactiva tipo Gantt/Cronograma simplificado para gestionar y supervisar el avance de las fases críticas de un proyecto de construcción, permitiendo actualizar progresos, fechas e hitos clave.

---

## 💎 Propósito y Casos de Uso
El seguimiento de plazos en obra evita penalizaciones por entregas tardías. Este componente resuelve:
1. **Línea de Tiempo Interactiva**: Visualización rápida del estado de avance (Completado, En Proceso, Retrasado, Pendiente) por fase constructiva.
2. **Control de Avance Físico**: Slider integrado para actualizar de forma táctil el porcentaje de avance de la fase seleccionada de forma bidireccional.
3. **Alertas de Retraso**: Los directores pueden cambiar el estado a "Retrasado" de forma manual, lo que tiñe visualmente la fase de color rojo o naranja en la línea de tiempo.

---

## 🎨 Especificación Visual y Estilos (Tailwind CSS)
* **Contenedor Gantt**: Diseño de rejilla responsiva que simula una línea de tiempo horizontal en pantallas grandes y se transforma en un listado vertical limpio en móviles.
* **Barra de Progreso Integrada**: Progreso porcentual dinámico que cambia de color según el estado (`bg-emerald-500` para completado, `bg-red-500` para retrasado).
* **Confirmación Destructiva**: Integración obligatoria de `useAlertConfirm` al resetear las fechas de cronograma al estado de planificación inicial.
...
export default function CronogramaHitosProyecto({
  onSaveSchedule,
  confirmAction,
  fasesIniciales = [
    ...
    { id: 'f3', nombre: 'Estructuras y Concreto Armado', inicio: '2026-07-26', fin: '2026-08-20', progreso: 0, estado: 'pendiente' },
  ...
  const [selectedFaseId, setSelectedFaseId] = useState(fasesIniciales[1].id); // Excavación por defecto
          ...
          {faseActiva ? (
            <div className="bg-[var(--color-surface-2)]/40 border border-[var(--color-border)] p-5 rounded-xl flex flex-col gap-5 sticky top-6">
              ...
              <button
                ...
                className="w-full py-2.5 bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] text-[var(--color-text)] font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-[var(--color-primary)]/20 transition-all active:scale-[0.98]"
              >
...
1. **`fases`**: Array conteniendo las fases del proyecto constructivo con sus respectivos progresos, plazos e indicadores de estado.
2. **`selectedFaseId`**: Clave de selección activa para controlar los inputs y el slider en la tarjeta derecha.
3. **`faseActiva`**: Objeto memorizado (`useMemo`) que devuelve los metadatos de la fase actualmente seleccionada.
...
sequenceDiagram
    ...
    participant G as Gantt/Cronograma
    participant AC as useAlertConfirm

    U->>G: Clic en fase para controlarla
    G->>G: Actualiza selectedFaseId
    G->>U: Carga información en tarjeta lateral
    U->>G: Desplaza slider de progreso
    G->>G: Actualiza progreso y reevalúa estado
    U->>G: Clic en "Reiniciar Plazos"
    G->>AC: Solicita confirmación
    AC-->>G: Acepta
    G->>G: Restaura plazos originales y renderiza UI
```
