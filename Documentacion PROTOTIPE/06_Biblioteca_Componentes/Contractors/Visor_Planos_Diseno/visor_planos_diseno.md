<!--
{
  "resource": "VisorPlanosDiseno",
  "technicalName": "VisorPlanosDiseno",
  "targetPath": "src/components/common/VisorPlanosDiseno.jsx",
  "dependencies": {
    "npm": {
...
Este componente visualiza planos técnicos de obra (Arquitectura, Estructuras, Eléctricos, Sanitarios) organizados por especialidad, con soporte interactivo para simular zoom, consultas de escalas de dibujo, versiones de archivo e historial de revisiones del proyecto.

---

## 💎 Propósito y Casos de Uso
En obra, trabajar con versiones desactualizadas de planos causa demoliciones y retrabajo muy costosos. Este visor premium centraliza:
1. **Organización por Especialidad**: Filtros rápidos para alternar entre arquitectura, estructuras e instalaciones.
2. **Historial de Versiones**: Indica claramente qué versión está activa (ej: V2.1) y quién aprobó la última revisión.
3. **Simulador de Zoom y Grid Técnico**: Un visor táctil con botones de lupa y una retícula matemática de fondo que facilita la lectura de cotas.

---

## 🎨 Especificación Visual y Estilos (Tailwind CSS)
* **Retícula Técnica de Visualización**: Fondo negro translúcido con una cuadrícula tipo blueprint en color cian/azul (`bg-[var(--color-bg)]` con líneas de retícula azuladas).
* **Controles Flotantes**: Botones con estética de vidrio (`backdrop-blur bg-white/10`) superpuestos sobre el plano.
* **Selectores**: Uso de `CustomSelect` para alternar la disciplina o especialidad del plano.
...
export default function VisorPlanosDiseno({
  onArchivePlan,
  confirmAction,
  planosIniciales = [
    {
        ...
        <div className="lg:col-span-8 flex flex-col gap-4">
          {planoActivo ? (
            <div className="flex flex-col gap-4">
              ...
              <div className="relative w-full h-[320px] bg-[var(--color-bg)] border border-[var(--color-border)] rounded-xl overflow-hidden flex items-center justify-center select-none shadow-inner">
                ...
                <div className="absolute bottom-4 right-4 flex items-center gap-2 bg-[var(--color-surface)]/80 backdrop-blur border border-slate-700 p-2 rounded-xl">
                  <button
                    ...
                    className="p-1.5 hover:bg-[var(--color-surface-1)] rounded-lg text-cyan-400 transition-colors"
                  >
              ...
              <div className="p-4 bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-xl grid grid-cols-1 sm:grid-cols-2 sm:grid-cols-4 gap-4">
...
1. **`disciplinaFiltro`**: Filtro activo de disciplina (Arquitectura, Estructuras, Instalaciones) para el listado lateral.
2. **`zoom`**: Estado porcentual del zoom del blueprint (de 50% a 250% en saltos de 25%).
3. **`planos`**: Estado de array que almacena la lista actual de planos vigentes, permitiendo archivarlos de forma segura.
...
sequenceDiagram
    ...
    participant V as VisorPlanos
    participant AC as useAlertConfirm

    U->>V: Clic en plano del listado
    V->>V: Actualiza planoActivo y restablece zoom a 100%
    V->>U: Renderiza imagen de plano con cuadrícula técnica
    U->>V: Clic en zoom in/out
    V->>V: Modifica la escala de transformación CSS
    U->>V: Clic en "Archivar Plano"
    V->>AC: Solicita confirmación
    AC-->>V: Confirma archivar
    V->>V: Filtra plano del array y actualiza UI
```
