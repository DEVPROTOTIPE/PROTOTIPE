<!--
{
  "resource": "FormularioSolicitudRectificacion",
  "technicalName": "FormularioSolicitudRectificacion",
  "targetPath": "src/components/common/FormularioSolicitudRectificacion.jsx",
  "dependencies": {
    "npm": {
...
Este componente proporciona una interfaz premium para registrar solicitudes de rectificación, pulido o reconstrucción de piezas desgastadas o dañadas con carga de imágenes y descripción del fallo.

## 1. Propósito y Casos de Uso
* **Servicio de Reparación de Piezas:** Para que los clientes detallen el estado de un eje, bloque o molde que necesita rectificación en prensa o rectificadora tangencial.
* **Inspección de Recepción:** Permite a los técnicos detallar daños detectados al recibir una pieza en taller.

## 2. Especificación Visual y Estilos (Tailwind CSS)
* **Formulario Estructurado:** Rejilla responsiva con variables HSL del tema (`bg-[var(--color-surface)]`).
* **Zona de Carga de Evidencia:** Área de arrastre con borde discontinuo (`border-dashed border-[var(--color-border)]`) para subir fotos de la pieza dañada.
* **Control Select Premium:** Consumo obligatorio de `CustomSelect` para indicar el tipo de daño y material de la pieza.
...
export default function FormularioSolicitudRectificacion({
  onSubmit = null,
  materialOptions = [
    ...
    <div className="w-full max-w-xl mx-auto bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-5 shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {images.map((img, idx) => (
              <div key={idx} className="relative w-14 h-14 border border-[var(--color-border)] rounded-lg overflow-hidden group">
                ...
                <button
                  ...
                  className="absolute inset-0 bg-red-500/80 text-[var(--color-text)] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 cursor-pointer"
                >
                  <Trash2 size={12} />
                </button>
              </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            type="submit"
            className="flex-1 h-10 bg-[var(--color-primary)] hover:opacity-90 active:scale-95 text-[var(--color-text)] font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm"
          >
...
* **Manejador de Confirmación Taller:** Utiliza el hook promesificado `useAlertConfirm` en el botón de limpieza para mitigar pérdidas accidentales de inputs extensos de cotización.
* **Control de Imágenes Locales:** Libera referencias temporales de URL (`URL.revokeObjectURL`) al subir u omitir imágenes para resguardar la memoria del navegador.

## 5. Flujo Operativo y Secuencia de Interacción

```mermaid
sequenceDiagram
    participant User as Inspector / Cliente
    participant UI as FormularioSolicitudRectificacion
    participant Dialog as useAlertConfirm Modal

    User->>UI: Clic en "Limpiar Formulario"
    UI->>Dialog: Dispara confirm()
    Dialog->>User: Muestra modal de confirmación
    alt Confirma
        User->>Dialog: Clic en "Sí, limpiar"
        Dialog->>UI: Devuelve true
        UI->>UI: Limpia estados e imágenes temporales
    else Cancela
        User->>Dialog: Clic en "Cancelar"
        Dialog->>UI: Devuelve false
        UI->>UI: Preserva inputs ingresados
    end
```
