<!--
{
  "resource": "SelectorLotesVolumen",
  "technicalName": "SelectorLotesVolumen",
  "targetPath": "src/components/common/SelectorLotesVolumen.jsx",
  "dependencies": {
    "npm": {
...
Este componente proporciona una visualización interactiva de precios escalonados (tiers) por volumen de piezas mecanizadas, resaltando dinámicamente el rango activo en base a la cantidad ingresada.

## 1. Propósito y Casos de Uso
* **Venta al por Mayor:** Permite a los clientes ver cómo se reduce el costo unitario de mecanizado al aumentar el tamaño del lote.
* **Presupuestador CNC:** Asiste en la selección del tamaño de lote óptimo para justificar los costos de seteo de máquina.

## 2. Especificación Visual y Estilos (Tailwind CSS)
* **Resaltado Activo (Glow HSL):** La fila/tarjeta correspondiente al tier seleccionado se resalta con bordes de color de marca HSL (`border-[var(--color-primary)]`), fondo activo y un check de confirmación.
* **Barra de Progreso de Ahorros:** Muestra visualmente la progresión hacia el siguiente nivel de descuento.
* **Diseño Responsivo:** Visualización flexible que cambia de cuadrícula de tarjetas en móvil a tabla estructurada en pantallas más grandes.
...
export default function SelectorLotesVolumen({
  quantity = 1,
  onChange = null,
  tiers = [
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-5 h-5 rounded-md flex items-center justify-center ${
                  isActive ? 'bg-[var(--color-primary)] text-[var(--color-text)]' : 'bg-[var(--color-surface)] text-[var(--color-text-muted)] border border-[var(--color-border)]'
                }`}>
...
* **Detección de Tiers Dinámica (`useMemo`):** Asigna el índice de la tabla de precios comparando la cantidad (`qty`) contra los límites inferiores y superiores de cada rango.
* **Cálculo de Descuentos Cruzados:** Compara el costo del lote actual contra el costo inicial del siguiente tier, advirtiendo al usuario si comprar más piezas resulta en un desembolso menor debido a la reducción del precio por volumen.

## 5. Flujo Operativo y Secuencia de Interacción

```mermaid
sequenceDiagram
    participant User as Comprador B2B
    participant UI as SelectorLotesVolumen
    participant Calc as useMemo

    User->>UI: Modifica cantidad a 48 unidades
    UI->>Calc: Recalcula activeTierIndex (detecta Tier 2: 10-49)
    UI->>Calc: Calcula costo a 48 * $98.00 = $4704
    UI->>Calc: Compara contra Tier 3 (50 * $75.00 = $3750)
    UI->>UI: Detecta que Tier 3 es más barato
    UI->>User: Muestra tip de ahorro destacando $954 USD de ahorro
```
