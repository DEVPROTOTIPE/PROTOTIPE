<!--
{
  ...
  "targetPath": "src/components/common/SelectorTratamientoAcabado.jsx",
  "dependencies": {
    "npm": {
      ...
      <div className="flex justify-between items-center mb-1 px-1">
        ...
        <div className="flex gap-1">
          <button
            type="button"
            onClick={() => scroll('left')}
            className="w-6 h-6 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-2)]/50 text-[var(--color-text-muted)] flex items-center justify-center hover:bg-[var(--color-primary)] hover:text-[var(--color-text)] transition-all duration-300 cursor-pointer shadow-sm"
          >
            <ChevronLeft size={14} />
          </button>
          <button
            ...
            >
              <div className="flex justify-between items-start">
                <div className={`p-1.5 rounded-lg ${
                  isSelected ? 'bg-[var(--color-primary)] text-[var(--color-text)]' : 'bg-[var(--color-surface)] text-[var(--color-text-muted)]'
                }`}>
                  <Icon size={14} />
                </div>
