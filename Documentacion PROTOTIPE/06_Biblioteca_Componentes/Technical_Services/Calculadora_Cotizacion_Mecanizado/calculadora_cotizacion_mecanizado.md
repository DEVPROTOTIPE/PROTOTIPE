<!--
{
  "resource": "CalculadoraCotizacionMecanizado",
  "technicalName": "CalculadoraCotizacionMecanizado",
  "targetPath": "src/components/technical-services/CalculadoraCotizacionMecanizado.jsx",
  "dependencies": {
    "npm": {
...
Este componente calcula el precio estimado para trabajos de mecanizado de precisión en base al tipo de metal, las dimensiones físicas de la pieza y la tolerancia requerida.

## 1. Propósito y Casos de Uso
* **Cotizaciones en Caliente:** Para que clientes e ingenieros conozcan costes estimados de fabricación al instante.
* **Control de Costos de Taller:** Ayuda a los torneros a desglosar el coste de material, máquina y mano de obra.

## 2. Especificación Visual y Estilos (Tailwind CSS)
* **Rejilla Estilizada:** Grid responsivo de inputs estilizados con variables HSL del tema (`border-[var(--color-border)]`).
* **Visualización de Desglose:** Tarjeta con animación suave de carga y distribución porcentual del coste (material vs mano de obra).
* **Control Select Premium:** Uso obligatorio de `CustomSelect` para asegurar la coherencia estética con el dev-dashboard.
...
export default function CalculadoraCotizacionMecanizado({
  onCalculate = null,
  materialOptions = [
...
* **Cálculo Reactivo (`useMemo`):** Recalcula peso, tiempo de máquina, descuentos por volumen y costos finales en tiempo real cada vez que cambia el material, la tolerancia, la cantidad o las dimensiones.
* **Propiedades Dinámicas:** Permite inyectar opciones de tolerancias y densidades específicas de materiales.

## 5. Flujo Operativo y Secuencia de Interacción

```mermaid
sequenceDiagram
    participant User as Ingeniero/Tornero
    participant UI as CalculadoraCotizacionMecanizado
    participant Parent as Componente Padre

    User->>UI: Modifica dimensiones (Largo a 120mm)
    UI->>UI: Dispara recálculo de volumen (useMemo)
    UI->>UI: Actualiza peso y costo de material
    UI->>UI: Incrementa horas máquina estimadas
    UI->>UI: Refleja nuevo total en UI
    User->>UI: Hace clic en "Solicitar Presupuesto"
    UI->>Parent: Emite onCalculate(totalCost)
```
