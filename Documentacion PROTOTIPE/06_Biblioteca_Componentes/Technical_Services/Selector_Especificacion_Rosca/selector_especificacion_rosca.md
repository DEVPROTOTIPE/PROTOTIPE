<!--
{
  "resource": "SelectorEspecificacionRosca",
  "technicalName": "SelectorEspecificacionRosca",
  "targetPath": "src/components/technical-services/SelectorEspecificacionRosca.jsx",
  "dependencies": {
    "npm": {
...
Este componente proporciona dropdowns técnicos especializados para seleccionar roscas normalizadas (Métrica, Whitworth BSP, NPT) con sus respectivos diámetros nominales y pasos de rosca.

## 1. Propósito y Casos de Uso
* **Configuración de Roscado en Torno:** Para que los clientes definan exactamente el tipo de acoplamiento roscado de su pieza.
* **Broca de Diámetro de Núcleo:** Informa automáticamente el diámetro de la broca preliminar requerida antes de pasar el macho de roscar.

## 2. Especificación Visual y Estilos (Tailwind CSS)
* **Formulario Adaptativo:** Rejilla responsiva de inputs HSL de marca blanca.
* **Visualización de Fórmulas:** Bloque de información con tipografía monoespaciada para mostrar equivalencias de paso e hilos por pulgada.
* **Control Select Premium:** Consumo obligatorio del componente `CustomSelect` del dev-dashboard.

## 3. Código React Completo

```jsx
import React, { useState, useMemo } from 'react';
import { Settings, HelpCircle, Compass, CheckCircle } from 'lucide-react';
import CustomSelect from '../ui/CustomSelect';

export default function SelectorEspecificacionRosca({
  selectedRosca = null,
  onChange = null,
  threadSystems = [
...
* **Sincronización de Cascadas (`useMemo`):** Filtra reactivamente los tamaños e hilos disponibles al cambiar el sistema principal (`system`).
* **Actualización Segura de Estados:** Restablece la medida por defecto al cambiar la norma para evitar desajustes en el índice.

## 5. Flujo Operativo y Secuencia de Interacción

```mermaid
sequenceDiagram
    participant Eng as Ingeniero/Tornero
    participant UI as SelectorEspecificacionRosca
    participant State as React States

    Eng->>UI: Cambia Norma a "Whitworth"
    UI->>State: Setea system a 'whitworth'
    State->>UI: Filtra y actualiza availableSizes
    UI->>UI: Auto-selecciona primer valor (G 1/8")
    UI->>Eng: Actualiza diámetro de broca (8.8 mm)
```
