# Control de Creación y Checklist de Componentes (230 Componentes)

Este documento registra y realiza el seguimiento persistente de la inyección de los 230 componentes comerciales (10 por vertical) diseñados para expandir el ecosistema **PROTOTIPE**. 

---

## 📊 Progreso de Implementación
* **Total Planificado:** 230
* **Completados:** 40
* **Restantes:** 190
...
- [x] **`TablaEspecificacionesHVAC`**: Tabla comparativa de especificaciones de compresores y consumo de amperaje.
- [x] **`SelectorTramosTuberia`**: Selector de metros de tubería de cobre y aislamiento necesarios.
...
- [ ] **`CalendarioRangoAlquiler`**: Calendario interactivo de rango de fechas para verificar disponibilidad.
- [ ] **`CalculadoraTarifasAlquiler`**: Calculadora que desglosa tarifas por día, semana o mes.
- [ ] **`ChecklistInspeccionMaquinaria`**: Formulario digital para registrar niveles, horómetro y daños.
- [ ] **`TarjetasOperadoresAutorizados`**: Selector de conductores con licencias certificadas.
- [ ] **`CalculadoraFletesTransporte`**: Selector de tipo de cama baja y kilómetros de traslado.
- [ ] **`SelectorAccesoriosMaquinaria`**: Selector de complementos para excavadoras o minicargadores.
- [ ] **`MonitorHorasAlertas`**: Tarjeta que muestra las horas de uso y proximidad de mantenimiento.
- [ ] **`SelectorSeguroDanos`**: Checkbox interactivo para seguro de exención de daños.
- [ ] **`DeslizadorCapacidadTonelaje`**: Deslizador para filtrar por toneladas de capacidad o alcance.
- [ ] **`TarjetaLogisticaDespacho`**: Ficha que detalla conductor del flete, ruta y entrega.

## 🪚 Vertical 6: Carpinterías y Muebles (`carpentry`)
- [ ] **`OptimizadorCorteTableros`**: Utilidad para visualizar cómo cortar piezas en un tablero minimizando el desperdicio.
- [ ] **`SelectorMaderaAcabado`**: Muestrario de maderas y acabados de superficie.
- [ ] **`CalculadoraMueblesMedida`**: Configurar dimensiones de muebles con actualización de precio.
- [ ] **`SelectorHerrajesAccesorios`**: Selector de bisagras, rieles, jaladeras y organizadores.
- [ ] **`TablaDespieceMateriales`**: Tabla interactiva con cantidad de piezas y dimensiones exactas.
- [ ] **`AgendamientoTomaMedidas`**: Formulario para programar la visita de toma de medidas a domicilio.
- [ ] **`SelectorModulosCocina`**: Selector gráfico de módulos para armar cocinas modulares.
- [ ] **`GaleriaRendersMuebles`**: Galería con sliders interactivos de imágenes "Antes / Después".
- [ ] **`SelectorAperturaPuertas`**: Diagrama interactivo para elegir el sentido de apertura.
- [ ] **`CalculadorTarifaInstalacion`**: Tarifador dinámico según piso y complejidad del ensamble.

## 🧺 Vertical 7: Lavanderías y Tintorerías (`laundry`)
- [ ] **`SelectorTipoPrendaLavado`**: Lista visual de prendas con métodos de lavado sugeridos.
- [ ] **`CalculadoraLavadoKilos`**: Barra interactiva para estimar costo ingresando los kilogramos.
- [ ] **`ProgramadorRutasDomicilio`**: Formulario para agendar recolección y entrega de prendas.
- [ ] **`FichaReporteManchas`**: Formulario digital para marcar manchas específicas en una silueta.
- [ ] **`SelectorFraganciaSuavizante`**: Selector de aromas e indicador de detergente hipoalergénico.
- [ ] **`TarjetaSesionAutoservicio`**: Interfaz para escanear QR de lavadora/secadora física y ver tiempo restante.
- [ ] **`BuscadorPercherosRopa`**: Buscador de ubicación de percheros numerados para entrega rápida.
- [ ] **`SelectorVelocidadServicio`**: Selector de velocidad de entrega (Express, Mismo día, Normal) con recargo.
- [ ] **`SaldoPuntosFidelizacion`**: Tarjeta con saldo de lavadas, puntos y cupones.
- [ ] **`CuadriculaPrendasOlvidadas`**: Galería de prendas sin etiqueta con buscador descriptivo.

## 🛋️ Vertical 8: Tapicería y Restauración de Muebles (`furniture_repair`)
- [ ] **`SelectorTelasTexturas`**: Muestrario de telas clasificadas por tipo con especificaciones de resistencia.
- [ ] **`CalculadoraMetrajeTela`**: Estima metros de tela requeridos según el tipo de mueble.
- [ ] **`SelectorDensidadEspuma`**: Selector de densidad de espuma y estado de resortes.
- [ ] **`CargadorFotosRestauracion`**: Subir fotos del mueble desgastado desde diferentes ángulos.
- [ ] **`SelectorAcabadoPatas`**: Selector de tonos de tinte y barniz para patas de madera.
- [ ] **`SeguimientoFasesRestauracion`**: Stepper visual de etapas físicas del tapizado.
- [ ] **`SelectorEstiloCosturas`**: Selector gráfico de tipos de costura y capitoné.
- [ ] **`CalculadoraFleteMuebles`**: Calculadora basada en dimensiones y operarios necesarios.
- [ ] **`ManualCuidadoTapiceria`**: Consejos para limpiar derrames y evitar desgaste.
- [ ] **`ToggleImpermeabilizacion`**: Checkbox para agregar servicio de teflonado con garantía.

## 💆 Vertical 9: Estética, Podología y Bienestar (`wellness_podology`)
- [ ] **`HistorialClinicoPodologia`**: Formulario privado de registro de patologías, alergias y tratamientos.
- [ ] **`MapaAnatomicoPie`**: Imagen interactiva del pie para marcar zonas de dolor o lesiones.
- [ ] **`SelectorServicioCabina`**: Selector de masajes con asignación automática de cabina.
- [ ] **`SelectorProfesionalStaff`**: Filtros para seleccionar terapeuta/especialista preferido.
- [ ] **`ConsentimientoFirmaDigital`**: Firma táctil de términos antes de tratamientos.
- [ ] **`SelectorAceitesEsenciales`**: Selector de esencias para personalizar la experiencia en cabina.
- [ ] **`RegistroEsterilizacionAutoclave`**: Registro de lotes de esterilización de herramientas médicas.
- [ ] **`ProgramadorSesionesPaquete`**: Selector para agendar sesiones consecutivas de paquetes.
- [ ] **`TarjetasProductosPostCuidado`**: Carrusel de cremas y plantillas sugeridas.
- [ ] **`VisorAnalisisPisada`**: Ficha para registrar resultados del análisis de pisada.

## 🍎 Vertical 10: Minimarkets y Alimentos (`grocery_food`)
- [ ] **`SelectorCantidadGranel`**: Input y selector de peso con precio dinámico reactivo.
- [ ] **`AlertaVencimientoLotes`**: Alerta de productos próximos a vencer con rebaja de precios.
- [ ] **`BuscadorCodigoPLU`**: Campo de búsqueda de códigos rápidos (PLU) de frutas y verduras.
- [ ] **`CalculadoraCombosOfertas`**: Ficha de combos (ej. Arroz + Aceite) y cálculo de descuento.
- [ ] **`FormularioAbastecimientoGondolas`**: Registro móvil de faltantes de productos en estantes.
- [ ] **`SelectorHorariosRetiro`**: Selector de franja horaria para retiro en tienda física.
- [ ] **`AdvertenciaNutricionalAlergenos`**: Alerta con etiquetas (Sin Gluten, Vegano, Contiene Maní).
- [ ] **`FormularioMermasDesperdicios`**: Registro para dar de baja productos dañados.
- [ ] **`PlantillaComprasRecurrentes`**: Carga de compras anteriores para recargar el carrito.
- [ ] **`CuadriculaOfertasDia`**: Grilla dinámica de ofertas agrícolas del día.

## 🚜 Vertical 11: Insumos y Repuestos Agrícolas (`insumos-agricolas`)
- [ ] **`BuscadorRepuestosTractor`**: Buscador jerárquico de marca, modelo, año y repuesto compatible.
- [ ] **`CalculadoraDosificacionAgroquimicos`**: Estima químicos por hectárea según cultivo.
- [ ] **`TablaEspecificacionesSemillas`**: Comparación de germinación, pureza y tratamiento de semillas.
- [ ] **`DisenadorManguerasHidraulicas`**: Configurar mangueras por diámetro, PSI y terminales.
- [ ] **`TarjetaNeumaticosAgricolas`**: Selector de llantas por medida, taco y tracción.
- [ ] **`FormularioPedidoMayorista`**: Cotizador para toneladas de fertilizante o bultos de insumos.
- [ ] **`CalculadoraCostoSiembra`**: Calculadora interactiva de gastos de semilla, fertilizante y mano de obra.
- [ ] **`SelectorBoquillasAspersion`**: Selector gráfico según presión de agua y tamaño de gota.
- [ ] **`MonitorHumedadSemillas`**: Registro de porcentaje de humedad y pureza de granos.
- [ ] **`BuscadorFiltrosLubricantes`**: Filtros y lubricantes compatibles con motores diésel pesados.

## 🎂 Vertical 12: Alimentos Artesanales y Repostería (`alimentos-artesanales`)
- [ ] **`ConfiguradorPastelesPersonalizados`**: Asistente de porciones, sabor, cobertura y dedicatoria.
- [ ] **`ProgramadorEntregaPasteles`**: Calendario con bloqueos de producción máxima diaria.
...
- [ ] **`CotizadorMesasDulces`**: Cotizador de bocaditos, macarons y trufas para eventos.
...
- [ ] **`SelectorCalibreAlambre`**: Selector de alambres de púas o galvanizados por calibre y longitud.
