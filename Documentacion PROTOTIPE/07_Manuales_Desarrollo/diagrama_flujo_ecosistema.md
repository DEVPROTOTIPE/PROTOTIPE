...
2. Sube el logo comercial, el cual es procesado y optimizado por la API Bridge [`server.js`](file:///d:/PROTOTIPE/Prototipe-CLI/server.js) vía Jimp y guardado físicamente.
...
2. Se consulta `/api/git/status` para auditar el repositorio local seleccionado. La API Bridge bloquea el avance si se detectan archivos `.env` expuestos (fugas de credenciales).
3. El usuario escribe o autogenera el mensaje de commit e inicia el backup vía `/api/git/backup-stream`.
4. La API Bridge mata consolas de desarrollo Vite activas para liberar archivos, renombra temporalmente repositorios `.git` anidados a `.git-backup-temp` para no generar gitlinks huérfanos, y corre `git add .` y `git commit` inyectando `--no-verify` en el push para evitar bloqueos de tests Playwright durante el respaldo.
