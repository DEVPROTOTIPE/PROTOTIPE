...
import ComisionesPanel from './components/admin/ComisionesPanel'
...
import { PARTICLES_ICONS_BY_KEY } from './components/admin/particlesIcons'

/**
 * BackgroundCanvas
 * Renderizador ligero en Canvas HTML5 de partículas GPU, orbs mesh y auroras dinámicas
 */

function BackgroundCanvas({
  bgType = 'solid',
  bgMouseTracking = false,
  bgParticlesCount = 40,
  bgParticlesSpeed = 'normal',
  bgParticlesSize = 2,
  bgParticlesColor = 'primary',
  ...
  useEffect(() => {
    ...
    const brandRgb = parseColorToRgb(primaryColor);
    const secRgb = parseColorToRgb(secondaryColor);
    const bgRgb = parseColorToRgb(bgColor);
    const colors = {
      primary: [primaryColor || '#6366f1'],
      pastel: [primaryColor || '#6366f1', secondaryColor || '#a855f7', '#ec4899', '#3b82f6', '#10b981'],
      neutral: ['#ffffff', '#cbd5e1', '#94a3b8']
    }[bgParticlesColor] || [primaryColor || '#6366f1'];
    ...
    const count = Math.min(bgParticlesCount || 40, 150);
    if (bgType === 'particles') {
    ...
    const target = global ? window : canvas.parentElement;
    if (bgMouseTracking && target) {
    ...
    const animate = (now) => {
      ...
      ctx.fillStyle = bgColor || '#070a13';
      ctx.fillRect(0, 0, w, h);

      // ─── DIBUJAR MESH GRADIENT FLUÍDO ───
      if (bgType === 'mesh' && orbsRef.current.length > 0) {
        orbsRef.current.forEach(orb => {
          orb.x += orb.vx * bgOrbsSpeed * dt;
          orb.y += orb.vy * bgOrbsSpeed * dt;

          const currentRadius = orb.baseRadius * bgOrbsSize;

          if (orb.x - currentRadius < -30) { orb.x = -30 + currentRadius; orb.vx *= -1; }
          if (orb.x + currentRadius > w + 30) { orb.x = w + 30 - currentRadius; orb.vx *= -1; }
          if (orb.y - currentRadius < -30) { orb.y = -30 + currentRadius; orb.vy *= -1; }
          if (orb.y + currentRadius > h + 30) { orb.y = h + 30 - currentRadius; orb.vy *= -1; }

          const color = orb.colorIndex % 2 === 0 ? primaryColor : secondaryColor;
          const rgb = parseColorToRgb(color);
          const radG = ctx.createRadialGradient(orb.x, orb.y, 0, orb.x, orb.y, currentRadius);
          const stopMiddle = Math.min(0.99, Math.max(0.01, 0.45 * bgOrbsBlur));
          radG.addColorStop(0, `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${bgOrbsOpacity})`);
          radG.addColorStop(stopMiddle, `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${bgOrbsOpacity * 0.38})`);
      ...
      if (bgMouseTracking && (hasMouse || Math.abs(spot.x - targetSpot.x) > 0.1)) {
      ...
      // ─── DIBUJAR PARTÍCULAS ───
      if (bgType === 'particles' && particlesRef.current.length > 0) {
        let resolvedParticlesSpeed = parseFloat(bgParticlesSpeed);
        if (isNaN(resolvedParticlesSpeed)) {
          const speedStr = String(bgParticlesSpeed).toLowerCase();
        ...
        particlesRef.current.forEach(p => {
          ...
          const radius = p.baseRadius * (bgParticlesSize || 2) + 0.5;
          ...
          const alpha = p.alphaFactor * bgParticlesOpacity;

          ctx.save();
          if (bgParticlesShape === 'circle') {
            ctx.beginPath();
            ctx.arc(p.x, p.y, radius, 0, Math.PI * 2);
            ctx.fillStyle = color;
            ctx.globalAlpha = alpha;
            ctx.fill();
          } else if (bgParticlesShape === 'glow') {
            ...
            ctx.globalAlpha = alpha;
            ctx.fill();
          } else if (bgParticlesShape === 'niche') {
            const activeIcon = bgParticlesIcon && bgParticlesIcon !== 'default' ? bgParticlesIcon : niche;
    ...
    return () => {
      ...
      ro.disconnect();
      const cleanTarget = global ? window : canvas.parentElement;
      if (cleanTarget) {
        cleanTarget.removeEventListener('pointermove', handlePointerMove);
        cleanTarget.removeEventListener('pointerleave', handlePointerLeave);
      }
    };
  }, [
    ...
    bgOrbsCount, bgOrbsOpacity, primaryColor, secondaryColor, niche, bgParticlesIcon,
    global, bgOrbsSpeed, bgOrbsSize, bgOrbsBlur, bgColor
  ]);

  return (
    <div className={`absolute inset-0 pointer-events-none overflow-hidden z-0 ${global ? '' : 'rounded-[22px]'}`}>
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full block animate-fade-in" />
    </div>
  );
}
...
/**
 * InteractiveTiltCard
 * Componente que encapsula la inclinación 3D física e iluminación reflectiva glare
 */
function InteractiveTiltCard({ enabled, children, className, style, primaryColor }) {
  ...
  const borderBeamActive = className.includes('border-beam-effect');

  // Si no está habilitado, render plano de 2 capas sin rotaciones pero respetando el overflow: visible para sombras
  if (!enabled) {
    return (
      <div 
      ...
      >
        <div 
          ...
          data-border-beam={borderBeamActive ? "true" : "false"}
        >
    ...
    >
      ...
      >
        ...
        >
          ...
          <div 
            className="fx-card-glare animate-fade-in"
            style={{
              position: 'absolute',
              inset: 0,
              zIndex: 1,
...
const ONBOARD_COMP_TYPE_STYLES = {
  atom:      { icon: '⚛️', color: 'text-rose-400',    bg: 'bg-rose-500/10',   border: 'border-rose-500/20' },
  component: { icon: '🧩', color: 'text-violet-400', bg: 'bg-violet-500/10', border: 'border-violet-500/20' },
  module:    { icon: '📦', color: 'text-amber-400',   bg: 'bg-amber-500/10',  border: 'border-amber-500/20' },
  hook:      { icon: '⚡', color: 'text-sky-400',     bg: 'bg-sky-500/10',    border: 'border-sky-500/20' },
  service:   { icon: '🔌', color: 'text-emerald-400', bg: 'bg-emerald-500/10',border: 'border-emerald-500/20' },
};
...
const PALETTE_CATEGORIES = [
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Pastel Lilac', primary: '#8b5cf6', secondary: '#c084fc', bg: '#faf5ff', text: '#3b0764' },
      { name: 'Blossom Pink', primary: '#ec4899', secondary: '#f472b6', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Soft Mint', primary: '#0d9488', secondary: '#14b8a6', bg: '#f0fdfa', text: '#042f2e' },
      { name: 'Clean Linen', primary: '#3b82f6', secondary: '#60a5fa', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Sunny Peach', primary: '#f97316', secondary: '#fb923c', bg: '#fff7ed', text: '#431407' },
      { name: 'Citrus Yellow', primary: '#ca8a04', secondary: '#eab308', bg: '#fefdf0', text: '#422006' },
      { name: 'Rose Quartz', primary: '#db2777', secondary: '#fbcfe8', bg: '#fff5f7', text: '#500724' },
      { name: 'Fresh Lime', primary: '#84cc16', secondary: '#a3e635', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Light Denim', primary: '#2563eb', secondary: '#93c5fd', bg: '#f0f5ff', text: '#0f172a' },
      { name: 'Nordic Slate', primary: '#475569', secondary: '#64748b', bg: '#f1f5f9', text: '#0f172a' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Aluminium Clean', primary: '#475569', secondary: '#64748b', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Zinc Galvanized', primary: '#0284c7', secondary: '#38bdf8', bg: '#f0f9ff', text: '#032b43' },
      { name: 'Oxide Light', primary: '#ea580c', secondary: '#fdba74', bg: '#fff7ed', text: '#431407' },
      { name: 'Brass Luster', primary: '#ca8a04', secondary: '#fbbf24', bg: '#fefdf0', text: '#422006' },
      { name: 'Safety Yellow Light', primary: '#eab308', secondary: '#fde047', bg: '#fefdf5', text: '#422006' },
      { name: 'Grinding Steel', primary: '#1e293b', secondary: '#475569', bg: '#f1f5f9', text: '#0f172a' },
      { name: 'Copper Tubing', primary: '#b45309', secondary: '#d97706', bg: '#fdfaf7', text: '#451a03' },
      { name: 'Laser Green Light', primary: '#16a34a', secondary: '#4ade80', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Foundry Ash', primary: '#52525b', secondary: '#71717a', bg: '#f4f4f5', text: '#18181b' },
      { name: 'Cobalt Light', primary: '#1d4ed8', secondary: '#60a5fa', bg: '#f0f5ff', text: '#0f172a' }
    ]
  },
  {
    id: "refrigeration_ac",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Glacier Blue', primary: '#0ea5e9', secondary: '#38bdf8', bg: '#f0f9ff', text: '#032b43' },
      { name: 'Frost Mint', primary: '#0d9488', secondary: '#5eead4', bg: '#f0fdfa', text: '#042f2e' },
      { name: 'Cyan Breeze', primary: '#06b6d4', secondary: '#22d3ee', bg: '#ecfeff', text: '#083b4c' },
      { name: 'Deep Cold Light', primary: '#2563eb', secondary: '#60a5fa', bg: '#f0f5ff', text: '#0f172a' },
      { name: 'Winter Calm', primary: '#1e40af', secondary: '#93c5fd', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Eco Airflow', primary: '#10b981', secondary: '#6ee7b7', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Silver Compressor', primary: '#64748b', secondary: '#cbd5e1', bg: '#f1f5f9', text: '#0f172a' },
      { name: 'Thermal Crisp', primary: '#0284c7', secondary: '#f97316', bg: '#f0f9ff', text: '#0f172a' },
      { name: 'Polar Breeze', primary: '#06b6d4', secondary: '#34d399', bg: '#f0fdfa', text: '#0f172a' },
      { name: 'Crystal Ice', primary: '#00f0ff', secondary: '#7000ff', bg: '#fafaff', text: '#0f072a' }
    ]
  },
  {
    id: "contractors",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Safety Orange Light', primary: '#f97316', secondary: '#fdba74', bg: '#fffaf5', text: '#431407' },
      { name: 'Yellow Hat Light', primary: '#eab308', secondary: '#fef08a', bg: '#fefdf0', text: '#422006' },
      { name: 'Cement Light', primary: '#4b5563', secondary: '#9ca3af', bg: '#f9fafb', text: '#111827' },
      { name: 'Timber Soft', primary: '#15803d', secondary: '#86efac', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Brick Light', primary: '#b91c1c', secondary: '#fca5a5', bg: '#fff5f5', text: '#4c0519' },
      { name: 'Drywall White', primary: '#334155', secondary: '#475569', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Copper Plumbing', primary: '#d97706', secondary: '#fbbf24', bg: '#fefdf5', text: '#422006' },
      { name: 'Slate Soft', primary: '#334155', secondary: '#475569', bg: '#f1f5f9', text: '#0f172a' },
      { name: 'Construction Green', primary: '#84cc16', secondary: '#bef264', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Structure Blue Light', primary: '#1d4ed8', secondary: '#93c5fd', bg: '#f0f7ff', text: '#0f172a' }
    ]
  },
  {
    id: "machinery_rental",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Caterpillar Gold', primary: '#eab308', secondary: '#ca8a04', bg: '#fefdf0', text: '#422006' },
      { name: 'John Deere Light', primary: '#16a34a', secondary: '#ca8a04', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Orange Kubota Light', primary: '#ea580c', secondary: '#f97316', bg: '#fff7ed', text: '#431407' },
      { name: 'Warning Gold', primary: '#f59e0b', secondary: '#fbbf24', bg: '#fffbeb', text: '#451a03' },
      { name: 'Hydraulic Sky', primary: '#0252cf', secondary: '#2563eb', bg: '#f0f7ff', text: '#0f172a' },
      { name: 'High Vis Yellow', primary: '#a3e635', secondary: '#84cc16', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Earth Rust', primary: '#9a3412', secondary: '#c2410c', bg: '#fff7ed', text: '#431407' },
      { name: 'Industrial Slate', primary: '#374151', secondary: '#9ca3af', bg: '#f9fafb', text: '#111827' },
      { name: 'Clean Fleet', primary: '#64748b', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Contrast Amber', primary: '#d97706', secondary: '#475569', bg: '#fdfaf7', text: '#451a03' }
    ]
  },
  {
    id: "carpentry",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Cedar Soft', primary: '#854d0e', secondary: '#a16207', bg: '#fdfcf0', text: '#451a03' },
      { name: 'Mahogany Soft', primary: '#7f1d1d', secondary: '#991b1b', bg: '#fff5f5', text: '#4c0519' },
      { name: 'Pine Clean', primary: '#166534', secondary: '#15803d', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Natural Oak Light', primary: '#b45309', secondary: '#d97706', bg: '#fefaf0', text: '#451a03' },
      { name: 'Birch Light', primary: '#d97706', secondary: '#fcd34d', bg: '#fefdf5', text: '#422006' },
      { name: 'Walnut Grain', primary: '#c2410c', secondary: '#ea580c', bg: '#fff7ed', text: '#431407' },
      { name: 'Varnish Gold Light', primary: '#ca8a04', secondary: '#fbbf24', bg: '#fefdf0', text: '#422006' },
      { name: 'Nordic Birch', primary: '#9ca3af', secondary: '#4b5563', bg: '#f9fafb', text: '#111827' },
      { name: 'Forest Wood', primary: '#065f46', secondary: '#0f766e', bg: '#f2fbf9', text: '#032f2e' },
      { name: 'Modern Studio', primary: '#1e293b', secondary: '#475569', bg: '#f8fafc', text: '#0f172a' }
    ]
  },
  {
    id: "laundry",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Breeze Clean', primary: '#06b6d4', secondary: '#0891b2', bg: '#ecfeff', text: '#083b4c' },
      { name: 'Lavender Calm', primary: '#a855f7', secondary: '#c084fc', bg: '#faf5ff', text: '#310762' },
      { name: 'Oxygen Sky', primary: '#2563eb', secondary: '#60a5fa', bg: '#f0f5ff', text: '#0f172a' },
      { name: 'Fresh Mint Light', primary: '#10b981', secondary: '#a7f3d0', bg: '#ecfdf5', text: '#052e16' },
      { name: 'Bubble White', primary: '#3b82f6', secondary: '#60a5fa', bg: '#f8fafc', text: '#1e3a8a' },
      { name: 'Sunny Cotton Light', primary: '#fbbf24', secondary: '#fef08a', bg: '#fefdf0', text: '#422006' },
      { name: 'Marine Splash Light', primary: '#0ea5e9', secondary: '#67e8f9', bg: '#f0fdfa', text: '#032b43' },
      { name: 'Gentle Rose Light', primary: '#ec4899', secondary: '#fbcfe8', bg: '#fdf2f8', text: '#4c0519' },
      { name: 'Pure Linen Light', primary: '#64748b', secondary: '#cbd5e1', bg: '#f1f5f9', text: '#0f172a' },
      { name: 'Citrus Splash', primary: '#84cc16', secondary: '#a3e635', bg: '#f7fee7', text: '#1a2e05' }
    ]
  },
  {
    id: "furniture_repair",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Leather Tan', primary: '#7c2d12', secondary: '#9a3412', bg: '#fff7ed', text: '#431407' },
      { name: 'Velvet Plum Light', primary: '#701a75', secondary: '#86198f', bg: '#fdf4ff', text: '#4c0519' },
      { name: 'Walnut Clean', primary: '#5c2c16', secondary: '#78350f', bg: '#fffbeb', text: '#451a03' },
      { name: 'Brass Gold Light', primary: '#b45309', secondary: '#fbbf24', bg: '#fefdf0', text: '#422006' },
      { name: 'Emerald Weave Light', primary: '#047857', secondary: '#059669', bg: '#ecfdf5', text: '#052e16' },
      { name: 'Linen Beige Light', primary: '#ca8a04', secondary: '#fde047', bg: '#fffbeb', text: '#422006' },
      { name: 'Antique Denim', primary: '#312e81', secondary: '#4338ca', bg: '#f0f3ff', text: '#080718' },
      { name: 'Sage Weave Light', primary: '#15803d', secondary: '#4ade80', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Bronze Gold', primary: '#854d0e', secondary: '#d97706', bg: '#fffbf0', text: '#451a03' },
      { name: 'Terracotta Clay', primary: '#c2410c', secondary: '#f97316', bg: '#fffaf0', text: '#431407' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Zen Teal Light', primary: '#0d9488', secondary: '#2dd4bf', bg: '#f2fbf9', text: '#042f2e' },
      { name: 'Rose Petal Light', primary: '#ec4899', secondary: '#f472b6', bg: '#fdf2f8', text: '#4c0519' },
      { name: 'Lavender Calm Light', primary: '#8b5cf6', secondary: '#a78bfa', bg: '#f5f3ff', text: '#310762' },
      { name: 'Sakura Pink', primary: '#db2777', secondary: '#f472b6', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Eucalyptus Mint', primary: '#10b981', secondary: '#34d399', bg: '#ecfdf5', text: '#052e16' },
      { name: 'Orchid Pastel', primary: '#d946ef', secondary: '#f5d0fe', bg: '#fdf4ff', text: '#3b0764' },
      { name: 'Clay White', primary: '#475569', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Warm Peach Light', primary: '#f97316', secondary: '#fdba74', bg: '#fffaf0', text: '#431407' },
      { name: 'Soft Sky Light', primary: '#0ea5e9', secondary: '#38bdf8', bg: '#f0f9ff', text: '#032b43' },
      { name: 'Herbal Moss', primary: '#15803d', secondary: '#a3e635', bg: '#f7fee7', text: '#1a2e05' }
    ]
  },
  {
    id: "grocery_food",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Tomato Salad', primary: '#ef4444', secondary: '#f87171', bg: '#fff5f5', text: '#4c0519' },
      { name: 'Organic Mint', primary: '#16a34a', secondary: '#4ade80', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Banana Shake', primary: '#eab308', secondary: '#fde047', bg: '#fefdf0', text: '#422006' },
      { name: 'Blueberry Creamy', primary: '#1d4ed8', secondary: '#3b82f6', bg: '#f0f5ff', text: '#0f172a' },
      { name: 'Citrus Punch', primary: '#f97316', secondary: '#fb923c', bg: '#fffaf0', text: '#431407' },
      { name: 'Carrot Sweet', primary: '#ea580c', secondary: '#fdba74', bg: '#fff7ed', text: '#431407' },
      { name: 'Apple Garden', primary: '#84cc16', secondary: '#bef264', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Cacao Cream', primary: '#78350f', secondary: '#b45309', bg: '#fffbeb', text: '#451a03' },
      { name: 'Clean Milk', primary: '#0ea5e9', secondary: '#38bdf8', bg: '#f0f9ff', text: '#032b43' },
      { name: 'Cherry Red', primary: '#991b1b', secondary: '#f87171', bg: '#fff5f5', text: '#4c0519' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Green Meadow', primary: '#15803d', secondary: '#86efac', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Fertile Soil', primary: '#b45309', secondary: '#fbbf24', bg: '#fffbeb', text: '#451a03' },
      { name: 'Harvest Sun', primary: '#ca8a04', secondary: '#fef08a', bg: '#fefdf0', text: '#422006' },
      { name: 'Coffee Latte', primary: '#4f1a00', secondary: '#d97706', bg: '#fef6f0', text: '#451a03' },
      { name: 'Eucalyptus Leaf', primary: '#0f766e', secondary: '#99f6e4', bg: '#f0fdfa', text: '#042f2e' },
      { name: 'Orange Tractor', primary: '#ea580c', secondary: '#ffedd5', bg: '#fff7ed', text: '#431407' },
      { name: 'River Clay', primary: '#7c2d12', secondary: '#ffedd5', bg: '#fff8f6', text: '#431407' },
      { name: 'Steel Silo', primary: '#475569', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Moss Green', primary: '#166534', secondary: '#bbf7d0', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Bright Sun', primary: '#eab308', secondary: '#fef9c3', bg: '#fefdf0', text: '#422006' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Strawberry Milk', primary: '#db2777', secondary: '#fbcfe8', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Honey Drizzle', primary: '#d97706', secondary: '#fde047', bg: '#fefdf5', text: '#422006' },
      { name: 'Cocoa Shake', primary: '#7c2d12', secondary: '#fdba74', bg: '#fffbeb', text: '#451a03' },
      { name: 'Vanilla Custard', primary: '#ca8a04', secondary: '#fef08a', bg: '#fffbeb', text: '#422006' },
      { name: 'Sweet Strawberry', primary: '#e11d48', secondary: '#ffe4e6', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Mint Ice Cream', primary: '#0d9488', secondary: '#ccfbf1', bg: '#f2fbf9', text: '#042f2e' },
      { name: 'Caramel Flavour', primary: '#b45309', secondary: '#ffedd5', bg: '#fffcf9', text: '#451a03' },
      { name: 'Blueberry Creamy', primary: '#2563eb', secondary: '#dbeafe', bg: '#f0f5ff', text: '#0f172a' },
      { name: 'Pistachio Soft', primary: '#16a34a', secondary: '#dcfce7', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Plum Syrup', primary: '#701a75', secondary: '#fdf4ff', bg: '#fdf4ff', text: '#4c0519' }
    ]
  },
  {
    id: "ferreteria-rural",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Steel Clean', primary: '#475569', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Zinc Plated', primary: '#0284c7', secondary: '#bae6fd', bg: '#f0f9ff', text: '#032b43' },
      { name: 'Safety Orange Tile', primary: '#c2410c', secondary: '#fdba74', bg: '#fffaf0', text: '#431407' },
      { name: 'Mortar Brick', primary: '#b91c1c', secondary: '#fca5a5', bg: '#fff5f5', text: '#4c0519' },
      { name: 'Amber Yellow', primary: '#f59e0b', secondary: '#fef3c7', bg: '#fefdf0', text: '#422006' },
      { name: 'Copper Pipe Light', primary: '#ea580c', secondary: '#ffedd5', bg: '#fff7ed', text: '#431407' },
      { name: 'Zinc Sheet', primary: '#0284c7', secondary: '#bae6fd', bg: '#f0f9ff', text: '#032b43' },
      { name: 'Wood Post', primary: '#15803d', secondary: '#bbf7d0', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Clay Brick', primary: '#ca8a04', secondary: '#fef08a', bg: '#fefdf0', text: '#422006' },
      { name: 'Concrete Block', primary: '#374151', secondary: '#d1d5db', bg: '#f3f4f6', text: '#0f172a' }
    ]
  },
  {
    id: "repuestos-motos",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Moto Racing Light', primary: '#ef4444', secondary: '#fca5a5', bg: '#fff5f5', text: '#4c0519' },
      { name: 'Fiber Gray', primary: '#3f3f46', secondary: '#a1a1aa', bg: '#fafafa', text: '#18181b' },
      { name: 'Nitrous Sky', primary: '#2563eb', secondary: '#93c5fd', bg: '#f0f5ff', text: '#0f172a' },
      { name: 'Asphalt Grey', primary: '#52525b', secondary: '#d4d4d8', bg: '#f4f4f5', text: '#18181b' },
      { name: 'Neon Orange', primary: '#f97316', secondary: '#fed7aa', bg: '#fffaf0', text: '#431407' },
      { name: 'Lubricant Gold', primary: '#ca8a04', secondary: '#fef08a', bg: '#fefdf0', text: '#422006' },
      { name: 'Chassis Grey Light', primary: '#64748b', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Exhaust Laser', primary: '#db2777', secondary: '#fbcfe8', bg: '#fff5f7', text: '#500724' },
      { name: 'Yamaha Cyan Light', primary: '#06b6d4', secondary: '#a5f3fc', bg: '#ecfeff', text: '#083b4c' },
      { name: 'Chrome Polish', primary: '#475569', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Glam Pink Light', primary: '#ec4899', secondary: '#fbcfe8', bg: '#fdf2f8', text: '#4c0519' },
      { name: 'Orchid Violet Light', primary: '#a855f7', secondary: '#e9d5ff', bg: '#faf5ff', text: '#310762' },
      { name: 'Ruby Lip Light', primary: '#e11d48', secondary: '#ffe4e6', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Golden Light', primary: '#ca8a04', secondary: '#fde047', bg: '#fffdf5', text: '#422006' },
      { name: 'Lavender Calm Beauty', primary: '#c084fc', secondary: '#f3e8ff', bg: '#faf5ff', text: '#310762' },
      { name: 'Ocean Nail Light', primary: '#06b6d4', secondary: '#cffafe', bg: '#ecfeff', text: '#083b4c' },
      { name: 'Matte Nude Light', primary: '#b45309', secondary: '#ffedd5', bg: '#fffbeb', text: '#451a03' },
      { name: 'Rose Gold Cream', primary: '#fb7185', secondary: '#ffe4e6', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Emerald Polish', primary: '#0d9488', secondary: '#ccfbf1', bg: '#f2fbf9', text: '#042f2e' },
      { name: 'Blush Rose', primary: '#db2777', secondary: '#fbcfe8', bg: '#fff5f7', text: '#500724' }
    ]
  },
  {
    id: "petshops-locales",
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Happy Dog Light', primary: '#f97316', secondary: '#fed7aa', bg: '#fffaf0', text: '#431407' },
      { name: 'Pet Care Sky', primary: '#06b6d4', secondary: '#a5f3fc', bg: '#ecfeff', text: '#083b4c' },
      { name: 'Green Grass Field', primary: '#16a34a', secondary: '#bbf7d0', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Puppy Golden', primary: '#eab308', secondary: '#fef9c3', bg: '#fefdf0', text: '#422006' },
      { name: 'Cat Purr Pink', primary: '#db2777', secondary: '#fbcfe8', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Water Pool', primary: '#0ea5e9', secondary: '#bae6fd', bg: '#f0f9ff', text: '#032b43' },
      { name: 'Olive Moss', primary: '#84cc16', secondary: '#d9f99d', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Feather Lilac', primary: '#8b5cf6', secondary: '#ddd6fe', bg: '#f5f3ff', text: '#310762' },
      { name: 'Bone Chew Light', primary: '#ca8a04', secondary: '#fef08a', bg: '#fffbeb', text: '#422006' },
      { name: 'Red Leash', primary: '#ef4444', secondary: '#fca5a5', bg: '#fff5f5', text: '#4c0519' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Electric Blue Light', primary: '#2563eb', secondary: '#93c5fd', bg: '#f0f7ff', text: '#0f172a' },
      { name: 'Titanium Slate', primary: '#52525b', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Safety Amber Light', primary: '#ca8a04', secondary: '#fde047', bg: '#fefdf5', text: '#422006' },
      { name: 'Valve Mint', primary: '#0d9488', secondary: '#99f6e4', bg: '#f0fdfa', text: '#042f2e' },
      { name: 'Coil Crimson', primary: '#b91c1c', secondary: '#fca5a5', bg: '#fff5f5', text: '#4c0519' },
      { name: 'Copper Coil Light', primary: '#d97706', secondary: '#fcd34d', bg: '#fffdf5', text: '#422006' },
      { name: 'Clean White Light', primary: '#64748b', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Logic Grass', primary: '#16a34a', secondary: '#86efac', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Laser Violet', primary: '#7c3aed', secondary: '#c084fc', bg: '#f5f3ff', text: '#310762' },
      { name: 'Aqua Breeze', primary: '#06b6d4', secondary: '#a5f3fc', bg: '#ecfeff', text: '#083b4c' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Classic Tan Leather', primary: '#7c2d12', secondary: '#ea580c', bg: '#fff7ed', text: '#431407' },
      { name: 'Minimal Linen Light', primary: '#d97706', secondary: '#fcd34d', bg: '#fefdf5', text: '#422006' },
      { name: 'Rose Silk', primary: '#db2777', secondary: '#fbcfe8', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Olive Tweed Light', primary: '#65a30d', secondary: '#d9f99d', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Indigo Soft', primary: '#312e81', secondary: '#818cf8', bg: '#f0f3ff', text: '#080718' },
      { name: 'Gold Buckle Light', primary: '#ca8a04', secondary: '#fef08a', bg: '#fefdf0', text: '#422006' },
      { name: 'Sand Canvas Light', primary: '#b45309', secondary: '#ffedd5', bg: '#fffbeb', text: '#451a03' },
      { name: 'Forest Boot Light', primary: '#065f46', secondary: '#34d399', bg: '#f2fbf9', text: '#042f2e' },
      { name: 'Royal Velvet Light', primary: '#6b21a8', secondary: '#e9d5ff', bg: '#faf5ff', text: '#310762' },
      { name: 'Crimson Suede Light', primary: '#991b1b', secondary: '#fca5a5', bg: '#fff5f5', text: '#4c0519' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Avocado Lime', primary: '#65a30d', secondary: '#bef264', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Almond Honey', primary: '#d97706', secondary: '#fef08a', bg: '#fefdf0', text: '#422006' },
      { name: 'Berry Puree', primary: '#db2777', secondary: '#fbcfe8', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Matcha Latte', primary: '#0d9488', secondary: '#a7f3d0', bg: '#ecfdf5', text: '#052e16' },
      { name: 'Citrus Juice', primary: '#f97316', secondary: '#fed7aa', bg: '#fffaf0', text: '#431407' },
      { name: 'Chia Pudding', primary: '#475569', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Turmeric Milk', primary: '#eab308', secondary: '#fef9c3', bg: '#fefdf0', text: '#422006' },
      { name: 'Spinach Fresh', primary: '#16a34a', secondary: '#86efac', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Sweet Pumpkin Light', primary: '#ea580c', secondary: '#ffedd5', bg: '#fff7ed', text: '#431407' },
      { name: 'Acai Berry Bowl', primary: '#701a75', secondary: '#f5d0fe', bg: '#fdf4ff', text: '#3b0764' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Desk Slate Light', primary: '#334155', secondary: '#94a3b8', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Eco Timber', primary: '#854d0e', secondary: '#fbbf24', bg: '#fefcf0', text: '#451a03' },
      { name: 'Cyber Glow', primary: '#db2777', secondary: '#38bdf8', bg: '#fff5fc', text: '#500724' },
      { name: 'Studio White Light', primary: '#64748b', secondary: '#cbd5e1', bg: '#f1f5f9', text: '#0f172a' },
      { name: 'Minimal Steel Light', primary: '#18181b', secondary: '#a1a1aa', bg: '#f4f4f5', text: '#18181b' },
      { name: 'Active Lime Light', primary: '#84cc16', secondary: '#bef264', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Soft Leather Light', primary: '#7c2d12', secondary: '#fdba74', bg: '#fffbeb', text: '#451a03' },
      { name: 'Deep Focus Light', primary: '#1d4ed8', secondary: '#93c5fd', bg: '#f0f7ff', text: '#0f172a' },
      { name: 'Mint Workspace Light', primary: '#0d9488', secondary: '#5eead4', bg: '#f2fbf9', text: '#042f2e' },
      { name: 'Terracotta Clay Office', primary: '#c2410c', secondary: '#ffedd5', bg: '#fffbf0', text: '#431407' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Neon Punch', primary: '#a855f7', secondary: '#f472b6', bg: '#faf5ff', text: '#310762' },
      { name: 'Amber Barrel Light', primary: '#d97706', secondary: '#fbbf24', bg: '#fffbeb', text: '#451a03' },
      { name: 'Fresh Mojito', primary: '#65a30d', secondary: '#bef264', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Blue Lagoon Light', primary: '#06b6d4', secondary: '#a5f3fc', bg: '#ecfeff', text: '#083b4c' },
      { name: 'Ruby Sangria', primary: '#e11d48', secondary: '#ffe4e6', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Gold Tequila Light', primary: '#ca8a04', secondary: '#fef08a', bg: '#fefdf0', text: '#422006' },
      { name: 'Gin Fizz', primary: '#0ea5e9', secondary: '#93c5fd', bg: '#f0f9ff', text: '#032b43' },
      { name: 'Plum Syrup Cocktail', primary: '#701a75', secondary: '#f5d0fe', bg: '#fdf4ff', text: '#3b0764' },
      { name: 'Dry Tonic', primary: '#16a34a', secondary: '#86efac', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Spiced Cider', primary: '#7c2d12', secondary: '#fdba74', bg: '#fffbeb', text: '#451a03' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Geek Violet Light', primary: '#7c3aed', secondary: '#c084fc', bg: '#f5f3ff', text: '#310762' },
      { name: 'Laser Cyan Light', primary: '#06b6d4', secondary: '#22d3ee', bg: '#ecfeff', text: '#083b4c' },
      { name: 'Retro Arcade Light', primary: '#db2777', secondary: '#fde047', bg: '#fff1f5', text: '#500724' },
      { name: 'Card Border Light', primary: '#b45309', secondary: '#fbbf24', bg: '#fffbeb', text: '#451a03' },
      { name: 'Cyber Mint', primary: '#10b981', secondary: '#86efac', bg: '#ecfdf5', text: '#052e16' },
      { name: 'Fire Red Light', primary: '#e11d48', secondary: '#fda4af', bg: '#fff1f2', text: '#4c0519' },
      { name: 'Mecha Silver', primary: '#475569', secondary: '#94a3b8', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Mana Potion Light', primary: '#2563eb', secondary: '#60a5fa', bg: '#f0f5ff', text: '#0f172a' },
      { name: 'Toxic Slime Light', primary: '#84cc16', secondary: '#d9f99d', bg: '#f7fee7', text: '#1a2e05' },
      { name: 'Galaxy Pink', primary: '#c084fc', secondary: '#fbcfe8', bg: '#faf5ff', text: '#310762' }
    ]
  },
  {
    ...
    palettes: [
      ...
      // --- Light Mode & Pastel Palettes ---
      { name: 'Eco Kraft Light', primary: '#d97706', secondary: '#fbbf24', bg: '#fefdf5', text: '#422006' },
      { name: 'Pure Clean Light', primary: '#2563eb', secondary: '#38bdf8', bg: '#f0f7ff', text: '#0f172a' },
      { name: 'Hygiene Mint Light', primary: '#0d9488', secondary: '#a7f3d0', bg: '#ecfdf5', text: '#052e16' },
      { name: 'B2B Steel Light', primary: '#475569', secondary: '#cbd5e1', bg: '#f8fafc', text: '#0f172a' },
      { name: 'Citrus Clean', primary: '#f97316', secondary: '#fed7aa', bg: '#fffaf0', text: '#431407' },
      { name: 'Safety Gold', primary: '#eab308', secondary: '#fef08a', bg: '#fefdf0', text: '#422006' },
      { name: 'Eco Leaf Light', primary: '#16a34a', secondary: '#86efac', bg: '#f0fdf4', text: '#052e16' },
      { name: 'Chemical Purple Light', primary: '#7c3aed', secondary: '#ddd6fe', bg: '#f5f3ff', text: '#310762' },
      { name: 'Wheat Sack', primary: '#b45309', secondary: '#ffedd5', bg: '#fffbeb', text: '#451a03' },
      { name: 'Pack Sky', primary: '#0284c7', secondary: '#bae6fd', bg: '#f0f9ff', text: '#032b43' }
    ]
  }
];
...
// Interceptor global de fetch para inyectar X-Developer-Google-Token en peticiones al Bridge
if (typeof window !== 'undefined') {
  const originalFetch = window.fetch;
  window.fetch = async function (resource, config = {}) {
    if (typeof resource === 'string' && (resource.includes('/api/') || resource.includes(':4000/'))) {
      const googleToken = localStorage.getItem('developer_google_token');
      if (googleToken) {
        config.headers = config.headers || {};
        if (config.headers instanceof Headers) {
          config.headers.set('X-Developer-Google-Token', googleToken);
        } else if (Array.isArray(config.headers)) {
          const idx = config.headers.findIndex(h => h[0].toLowerCase() === 'x-developer-google-token');
          if (idx !== -1) {
            config.headers[idx][1] = googleToken;
          } else {
            config.headers.push(['X-Developer-Google-Token', googleToken]);
          }
        } else {
          config.headers['X-Developer-Google-Token'] = googleToken;
        }
      }
    }
...
export default function App() {
  ...
  const clientesSaas = useMemo(() => {
    return allClientesControl.filter(c => {
      ...
      if (c.tipo === 'core' || c.isCore) return false
      if (idLower.includes('smartfix') || idLower === 'ventas' || idLower.includes('-dev')) return false
      return true
    })
  }, [allClientesControl])
  ...
  const [setupFee, setSetupFee] = useState(1500000)
  const [niche, setNiche] = useState('retail_clothing')
  const [niches, setNiches] = useState([
    { id: "retail_clothing", name: "🛍️ Ropa y Retail Tradicional (retail_clothing)", emoji: "🛍️", shortName: "Ropa y Retail" },
    { id: "technical_services", name: "⚙️ Tornerías y Mecanizado de Precisión (technical_services)", emoji: "⚙️", shortName: "Tornerías y Mecanizado" },
    { id: "refrigeration_ac", name: "❄️ Refrigeración y Climatización (refrigeration_ac)", emoji: "❄️", shortName: "Refrigeración/AC" },
    { id: "contractors", name: "📐 Contratistas y Construcción (contractors)", emoji: "📐", shortName: "Contratistas" },
    { id: "machinery_rental", name: "🚜 Alquiler de Maquinaria y Equipos (machinery_rental)", emoji: "🚜", shortName: "Alquiler Maquinaria" },
    { id: "carpentry", name: "🪚 Carpinterías y Muebles (carpentry)", emoji: "🪚", shortName: "Carpinterías" },
    { id: "laundry", name: "🧺 Lavanderías y Tintorerías (laundry)", emoji: "🧺", shortName: "Lavanderías" },
    { id: "furniture_repair", name: "🛋️ Restauración y Tapicería de Muebles (furniture_repair)", emoji: "🛋️", shortName: "Restauración/Tapicería" },
    { id: "wellness_podology", name: "💆 Estética, Podología y Bienestar (wellness_podology)", emoji: "💆", shortName: "Estética/Podología" },
    { id: "grocery_food", name: "🍎 Minimarkets y Alimentos (grocery_food)", emoji: "🍎", shortName: "Minimarkets/Alimentos" },
    { id: "insumos-agricolas", name: "🚜 Insumos y Repuestos Agrícolas (insumos-agricolas)", emoji: "🚜", shortName: "Insumos y Repuestos Agrícolas" },
    { id: "alimentos-artesanales", name: "🎂 Alimentos Artesanales y Repostería (alimentos-artesanales)", emoji: "🎂", shortName: "Alimentos Artesanales" },
    { id: "ferreteria-rural", name: "🛠️ Ferretería y Construcción Rural (ferreteria-rural)", emoji: "🛠️", shortName: "Ferretería Rural" },
    { id: "repuestos-motos", name: "🏍️ Repuestos y Accesorios de Motos (repuestos-motos)", emoji: "🏍️", shortName: "Repuestos Motos" },
    { id: "distribuidoras-beauty", name: "💅 Suministros de Belleza Profesional (distribuidoras-beauty)", emoji: "💅", shortName: "Distribuidoras Belleza" },
    { id: "petshops-locales", name: "🐶 Alimentos y Accesorios para Mascotas (petshops-locales)", emoji: "🐶", shortName: "Petshops Locales" },
    { id: "repuestos-lineablanca", name: "⚙️ Repuestos de Electrodomésticos (repuestos-lineablanca)", emoji: "⚙️", shortName: "Repuestos Línea Blanca" },
    { id: "moda-local-calzado", name: "👞 Calzado y Confección Local (moda-local-calzado)", emoji: "👞", shortName: "Moda y Calzado" },
    { id: "alimentacion-saludable", name: "🥗 Alimentación Orgánica y Saludable (alimentacion-saludable)", emoji: "🥗", shortName: "Alimentación Saludable" },
    { id: "home-office-ergonomia", name: "💻 Equipamiento Home Office (home-office-ergonomia)", emoji: "💻", shortName: "Home Office/Ergonomía" },
    { id: "licores-cocteleria", name: "🍹 Bodega de Licores y Coctelería (licores-cocteleria)", emoji: "🍹", shortName: "Licores y Coctelería" },
    { id: "coleccionismo-geek", name: "🧸 Artículos Geek y Coleccionismo (coleccionismo-geek)", emoji: "🧸", shortName: "Coleccionismo Geek" },
    { id: "distribucion-horeca", name: "📦 Insumos Horeca B2B (distribucion-horeca)", emoji: "📦", shortName: "Distribución Horeca B2B" }
  ]);

  const loadNichesFromServer = () => {
    fetch(`${CLI_URL}/api/niches`)
      .then(res => res.json())
      .then(data => {
        if (data && typeof data === 'object') {
          const apiKeys = Object.keys(data);
          const updatedNiches = apiKeys.map(key => {
            const item = data[key];
            const cleanName = item.name || key.split(/[_-]/).map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            const emoji = item.emoji || "💼";
            return {
              id: key,
              name: `${emoji} ${cleanName} (${key})`,
              emoji: emoji,
              shortName: cleanName
            };
          });
      ...
      .catch(err => console.warn('Error cargando nichos dinámicos:', err));
  };

  useEffect(() => {
    loadNichesFromServer();
  }, []);
  ...
  const [editSetupFee, setEditSetupFee] = useState(1500000)
  ...
  // Design Effect Tokens (CORE-242) — shadow palette + glass + motion + radius semántico
  const [shadowStyle, setShadowStyle] = useState('soft')       // none|soft|hard|glow|neon
  ...
  const [radiusMode, setRadiusMode] = useState('rounded')       // sharp|soft|rounded|extra|pill
  // --- Personalización Avanzada de Fondos e Interactividad (Dynamic Canvas Control) ---
  const [bgType, setBgType] = useState('solid')                 // solid|mesh|aurora|grid|particles
  const [bgMouseTracking, setBgMouseTracking] = useState(false)
  const [bgParticlesCount, setBgParticlesCount] = useState(40)
  const [bgParticlesSpeed, setBgParticlesSpeed] = useState('normal') // slow|normal|fast
  const [bgParticlesSize, setBgParticlesSize] = useState(2)     // 1 to 5
  const [bgParticlesColor, setBgParticlesColor] = useState('primary') // primary|pastel|neutral
  const [bgParticlesOpacity, setBgParticlesOpacity] = useState(0.35)
  const [bgParticlesDirection, setBgParticlesDirection] = useState('random') // random|up|down|left|right
  const [bgParticlesShape, setBgParticlesShape] = useState('circle') // circle|glow|star|niche
  const [bgParticlesIcon, setBgParticlesIcon] = useState('default')  // default o un nicho específico
  const [bgOrbsCount, setBgOrbsCount] = useState(3)             // 1 to 4
  ...
  const [enableKds, setEnableKds] = useState(false)
  const [costoPorFacturaDian, setCostoPorFacturaDian] = useState(150)
  const [customRequirements, setCustomRequirements] = useState('')
  // [BRECHA-2] Campos SEO del cliente — capturados en el wizard y enviados al generador
  const [seoDescription, setSeoDescription] = useState('')
  const [seoKeywords, setSeoKeywords] = useState('')
  // [UX-2] Etapa actual del aprovisionamiento para mostrar progreso legible al usuario
  const [provisioningStageLabel, setProvisioningStageLabel] = useState('')
  const [provisioningLogs, setProvisioningLogs] = useState([])
  const [wizardTab, setWizardTab] = useState('server')
  // [UX-3] Buscador para filtrar y priorizar recomendaciones de la biblioteca en el wizard
  ...
  // --- Variables Calculadas de Branding Effects Studio ---
  const previewRadiusCss = {
    sharp: '0rem',
    soft: '0.375rem',
    rounded: '0.75rem',
    extra: '1.25rem',
  ...
  // Funciones de conversión de color en caliente para el simulador
  const hexToRgb = (hex) => {
  ...
  const primaryRgbStr = `${rgbObj.r} ${rgbObj.g} ${rgbObj.b}`;
  const primaryHslStr = `${hslObj.h} ${hslObj.s}% ${hslObj.l}%`;

  const nLightness = Math.min(68, Math.max(52, hslObj.l));
  const nSaturation = Math.min(100, Math.max(72, hslObj.s));
  const neonHslStr = `${hslObj.h} ${nSaturation}% ${nLightness}%`;

  const glowGain = hslObj.l < 45 ? 1.25 : hslObj.l > 72 ? 0.82 : 1;

  const previewShadowCss = {
    none: 'none',
    soft: `0 12px 32px hsl(${hslObj.h} ${hslObj.s}% ${hslObj.l}% / ${(0.16 * glowGain).toFixed(3)})`,
    hard: `4px 4px 0 hsl(${hslObj.h} ${hslObj.s}% ${hslObj.l}% / 1)`,
    glow: `0 0 0 1px hsl(${hslObj.h} ${hslObj.s}% ${hslObj.l}% / ${(Math.min(0.9, 0.24 * glowGain)).toFixed(3)}), 0 0 16px hsl(${hslObj.h} ${hslObj.s}% ${hslObj.l}% / ${(Math.min(0.9, 0.42 * glowGain)).toFixed(3)}), 0 0 34px hsl(${hslObj.h} ${hslObj.s}% ${hslObj.l}% / ${(Math.min(0.9, 0.28 * glowGain)).toFixed(3)}), 0 12px 34px hsl(${hslObj.h} ${hslObj.s}% ${hslObj.l}% / ${(Math.min(0.9, 0.18 * glowGain)).toFixed(3)})`,
    neon: `0 0 12px hsl(${neonHslStr} / ${(Math.min(0.9, 0.45 * glowGain)).toFixed(3)}), 0 0 24px hsl(${neonHslStr} / ${(Math.min(0.8, 0.32 * glowGain)).toFixed(3)}), 0 0 48px hsl(${neonHslStr} / ${(Math.min(0.6, 0.18 * glowGain)).toFixed(3)}), 0 0 96px hsl(${neonHslStr} / ${(Math.min(0.4, 0.08 * glowGain)).toFixed(3)})`
  }[shadowStyle] || 'none';

  const isBrandDark = (() => {
    const c = (bgColor || '#070a13').replace('#', '');
    const rgb = parseInt(c, 16) || 0;
    const r = (rgb >> 16) & 0xff;
    const g = (rgb >> 8) & 0xff;
    const b = (rgb >> 0) & 0xff;
    const luma = 0.2126 * r + 0.7152 * g + 0.0722 * b;
    return luma < 140; // Umbral estándar
  })();

  const previewBgStyle = glassmorphism
    ? (isBrandDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.04)')
    : (isBrandDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.015)');
  
  const previewBlurStyle = glassmorphism ? 'blur(16px)' : 'none';
  const previewBorderStyle = glassmorphism
    ? (isBrandDark ? '1px solid rgba(255,255,255,0.12)' : '1px solid rgba(0,0,0,0.08)')
    : (isBrandDark ? '1px solid rgba(255,255,255,0.04)' : '1px solid rgba(0,0,0,0.03)');

  const animDurationStyle = { instant: '0ms', fast: '150ms', normal: '250ms', slow: '400ms' }[animationSpeed] || '250ms';

  // Sincronizar automáticamente el modo de color del simulador al cambiar la paleta o fondo de la marca
  useEffect(() => {
    const c = (bgColor || '#070a13').replace('#', '');
    const rgb = parseInt(c, 16) || 0;
    const r = (rgb >> 16) & 0xff;
    ...
    const b = (rgb >> 0) & 0xff;
    const luma = 0.2126 * r + 0.7152 * g + 0.0722 * b;
    setMockTheme(luma < 140 ? 'dark' : 'light');
  }, [bgColor]);
  ...
  const [dashBgSettings, setDashBgSettings] = useState(() => {
    try {
      const saved = localStorage.getItem('prototipe_dash_bg_settings');
      if (saved) {
      ...
      console.error('Error loading dashboard bg settings:', e);
    }
    return {
      bgType: 'mesh',
      bgMouseTracking: true,
  ...
  const updateDashBgSetting = (key, value) => {
    setDashBgSettings(prev => {
      const updated = { ...prev, [key]: value };
      localStorage.setItem('prototipe_dash_bg_settings', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const detectNicheKeyFromSector = (sectorText) => {
    const text = (sectorText || '').toLowerCase();
    if (text.includes('ropa') || text.includes('moda') || text.includes('retail') || text.includes('calzado') || text.includes('vestir')) return 'retail_clothing';
    if (text.includes('torner') || text.includes('mecanizado') || text.includes('precisi') || text.includes('taller')) return 'technical_services';
    if (text.includes('refrig') || text.includes('aire') || text.includes('clima') || text.includes('hvac')) return 'refrigeration_ac';
    if (text.includes('contrat') || text.includes('construc') || text.includes('obra') || text.includes('ingenier')) return 'contractors';
    if (text.includes('maquinaria') || text.includes('alquiler') || text.includes('renta')) return 'machinery_rental';
    if (text.includes('carpinter') || text.includes('mueble') || text.includes('madera')) return 'carpentry';
    if (text.includes('lavander') || text.includes('tintorer')) return 'laundry';
    if (text.includes('tapicer') || text.includes('restauraci')) return 'furniture_repair';
    if (text.includes('podolog') || text.includes('estetic') || text.includes('bienestar') || text.includes('spa')) return 'wellness_podology';
    if (text.includes('minimarket') || text.includes('tienda') || text.includes('abarrote')) return 'grocery_food';
    if (text.includes('agricola') || text.includes('insumo') || text.includes('campo')) return 'insumos-agricolas';
    if (text.includes('helado') || text.includes('reposter') || text.includes('alimento') || text.includes('panader') || text.includes('pasteler')) return 'alimentos-artesanales';
    if (text.includes('ferreteria-rural') || text.includes('ferreteria')) return 'ferreteria-rural';
    if (text.includes('moto') || text.includes('repuesto') || text.includes('accesorio')) return 'repuestos-motos';
    if (text.includes('belleza') || text.includes('cosmetico') || text.includes('distribuidora')) return 'distribuidoras-beauty';
    if (text.includes('mascota') || text.includes('pet') || text.includes('perro') || text.includes('gato')) return 'petshops-locales';
    if (text.includes('licor') || text.includes('bebida') || text.includes('coctel') || text.includes('bar')) return 'licores-cocteleria';
    if (text.includes('geek') || text.includes('juego') || text.includes('coleccion')) return 'coleccionismo-geek';
    if (text.includes('horeca') || text.includes('b2b') || text.includes('distribucion')) return 'distribucion-horeca';
    return 'retail_clothing'; // Fallback por defecto
  };

  const handleImportBriefingData = (session) => {
    if (!session) return;

    // 1. Datos básicos
    if (session.nombreEmpresa) {
      setNewClientName(session.nombreEmpresa);
    }

    const form = session.form || {};

    // Mapear requerimientos a customRequirements
    if (form.seccion15 && Array.isArray(form.seccion15.obligatorios)) {
      setCustomRequirements(
        `Requerimientos del Briefing:\n` +
        form.seccion15.obligatorios.map(r => `- ${r}`).join('\n')
      );
    }

    // Mapear logo
    if (form.seccion14 && form.seccion14.logoUrl) {
      setLogoLocalPath(form.seccion14.logoUrl);
      const parts = form.seccion14.logoUrl.split('/');
      setLogoFilename(parts[parts.length - 1] || 'logo.png');
    }

    // 2. Colores y Fuentes de Marca
    if (form.seccion14) {
      if (form.seccion14.colorPrimario) setPrimaryColor(form.seccion14.colorPrimario);
      if (form.seccion14.colorSecundario) setSecondaryColor(form.seccion14.colorSecundario);
      if (form.seccion14.googleFont) setGoogleFont(form.seccion14.googleFont);
    }

    // Detección automática de nicho
    if (form.seccion3 && form.seccion3.sector) {
      const detectedNiche = detectNicheKeyFromSector(form.seccion3.sector);
      setNiche(detectedNiche);
    }

    // Mapeo de template
    if (session.coreKey) {
      setSelectedTemplate(session.coreKey === 'ventas' ? 'template-ventas' : 'template-core-seed');
    }

    const analysis = session.analysisResult || {};
    const hasAnalysis = !!session.analysisResult;
    if (hasAnalysis) {
      // 3. Mapear Feature Flags lógicas del Core
      if (analysis.recommendedFlags) {
        const flags = analysis.recommendedFlags;
        if (flags.creditsEnabled !== undefined) setEnableBilling(!!flags.creditsEnabled);
        if (flags.deliveryEnabled !== undefined) setEnableDelivery(!!flags.deliveryEnabled);
        if (flags.kdsEnabled !== undefined) setEnableKds(!!flags.kdsEnabled);
        if (flags.dianBillingEnabled !== undefined) setEnableDianBilling(!!flags.dianBillingEnabled);
      }

      // 4. Mapear Componentes Recomendados de la Biblioteca
      const normalize = (str) => (str || '').replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
      if (Array.isArray(analysis.suggestedComponents)) {
        const allComponents = libraryList.flatMap(cat => cat.components || []);
        const matched = [];

        analysis.suggestedComponents.forEach(sugg => {
          if (!sugg) return;
          const comp = allComponents.find(c => 
            normalize(c.technicalName) === normalize(sugg.technicalName) ||
            normalize(c.name) === normalize(sugg.name)
          );
          if (comp) {
            matched.push({
              name: comp.name,
              ...
              link: comp.link,
              resourceType: comp.resourceType || 'component'
            });
          }
        });
        setSelectedRecomendations(matched);
      }

      // 5. Mapear Precios sugeridos comerciales
      if (analysis.pricing) {
        if (analysis.pricing.mensualidad !== undefined) setPagoMensualFijo(analysis.pricing.mensualidad || 0);
        if (analysis.pricing.minSetup !== undefined) setSetupFee(analysis.pricing.minSetup || 0);
        if (analysis.pricing.comision) {
          setComisionPorcentaje(analysis.pricing.comision || 0);
          setBillingMode('percentage');
        } else {
          setBillingMode('flat_monthly');
        }
      }

      showToast(`Sesión de Briefing para "${session.nombreEmpresa || 'Cliente'}" importada con éxito ✓`, { type: 'success' });
    } else {
      showToast(`Datos básicos importados. Nota: Esta sesión no tiene análisis de IA completado.`, { type: 'warning' });
    }
  ...
  const loadBriefingSessions = async () => {
    const db = getCentralApp() ? getFirestore(getCentralApp()) : null;
    if (!db) return;
    setLoadingBriefings(true);
    try {
      const q = query(collection(db, 'briefings'), orderBy('fecha', 'desc'), limit(50));
      const snap = await getDocs(q);
      const list = [];
      snap.forEach(doc => {
        list.push({ id: doc.id, ...doc.data() });
      });
      setBriefingSessions(list);
    } catch (err) {
      console.error('Error cargando sesiones de briefing:', err);
      showToast('Error al cargar briefings de Firestore', { type: 'error' });
    } finally {
      setLoadingBriefings(false);
    }
  };

  const handleImportManifest = (rawJson) => {
    if (!rawJson || !rawJson.trim()) {
      showToast('Por favor introduce un manifiesto JSON válido.', { type: 'error' });
      return;
    }
    try {
      const manifest = JSON.parse(rawJson);

      // 1. Mapear vertical de negocio
      if (manifest.nicheKey) {
        setNiche(manifest.nicheKey);

        if (manifest.isNewNiche && manifest.newNicheConfig) {
          const newNicheId = manifest.nicheKey;
          const config = manifest.newNicheConfig;
          setNiches(prev => {
            if (!prev.some(n => n.id === newNicheId)) {
              return [...prev, {
                id: newNicheId,
                name: `${config.icon === 'UtensilsCrossed' ? '🍴' : '💼'} ${config.displayName} (${newNicheId})`,
                emoji: config.icon === 'UtensilsCrossed' ? '🍴' : '💼',
      ...
      if (manifest.template) {
        setSelectedTemplate(manifest.template);
      }

      // 3. Mapear Feature Flags del Core
      if (manifest.featureFlags) {
        const flags = manifest.featureFlags;
        if (flags.dianBillingEnabled !== undefined) {
          setEnableDianBilling(flags.dianBillingEnabled);
        }
        if (flags.creditsEnabled !== undefined) {
          setEnableBilling(flags.creditsEnabled);
        }
        if (flags.deliveryEnabled !== undefined) {
          setEnableDelivery(flags.deliveryEnabled);
        }
        if (flags.kdsEnabled !== undefined) {
          setEnableKds(flags.kdsEnabled);
        }
      }

      // 4. Mapear Componentes de la Biblioteca
      if (Array.isArray(manifest.libraryComponentsToInject)) {
        const allComponents = libraryList.flatMap(cat => cat.components || []);
        
        const translationMap = {
          'POS_Express_Scanner': 'POSExpressScanner',
          'Caja_Diaria_POS': 'CajaDiariaPOS',
          'Reservas_Agenda_Citas': 'ReservasAgendaCitas',
          'Creditos_y_Saldos': 'CreditosSaldos',
          'Facturacion_y_Firma_Digital': 'DeveloperBillingPanel',
          'Pantalla_Cocina_KDS': 'PantallaCocinaKDS',
          'RuletaSuerte': 'RaffleWheelOfFortune',
          'SelectorBoletasRifas': 'RaffleNumberSelector',
          'WhatsApp_Omnicanal': 'OmnicanalidadWhatsApp'
        };

        const mappedRecs = manifest.libraryComponentsToInject.map(keyOrObj => {
          if (typeof keyOrObj === 'string') {
            const mappedKey = translationMap[keyOrObj] || keyOrObj;
            
            const normalize = (str) => (str || '').replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
            const normKey = normalize(mappedKey);

            const found = allComponents.find(c => 
              normalize(c.technicalName) === normKey || 
              normalize(c.name) === normKey ||
              normalize(c.technicalName) === normalize(keyOrObj) ||
              normalize(c.name) === normalize(keyOrObj)
            );

            if (found) {
              return {
                name: found.name,
                technicalName: found.technicalName,
                link: found.link,
                resourceType: found.resourceType || 'component'
              };
            }
            return {
              name: keyOrObj,
              technicalName: keyOrObj,
              link: '',
        ...
        setSelectedRecomendations(mappedRecs);
      }

      // 5. Mapear Branding Estético y Estilo Visual Premium
      if (manifest.branding) {
        ...
        if (brand.textColor) setTextColor(brand.textColor);
        if (brand.surfaceColor) setSurfaceColor(brand.surfaceColor);
        if (brand.surface2Color) setSurface2Color(brand.surface2Color);
        if (brand.borderColor) setBorderColor(brand.borderColor);
        if (brand.textMutedColor) setTextMutedColor(brand.textMutedColor);
        if (brand.radiusBase) setRadiusBase(brand.radiusBase);
        if (brand.googleFont) setGoogleFont(brand.googleFont);
        if (brand.shadowStyle) setShadowStyle(brand.shadowStyle);
        if (brand.glassmorphism !== undefined) setGlassmorphism(brand.glassmorphism);
        if (brand.animationSpeed) setAnimationSpeed(brand.animationSpeed);
        if (brand.radiusMode) setRadiusMode(brand.radiusMode);
        if (brand.borderBeam !== undefined) setBorderBeam(brand.borderBeam);
        if (brand.tilt3d !== undefined) setTilt3d(brand.tilt3d);

        // Mapear Personalización del Lienzo y Fondo de Partículas
        if (brand.background) {
          const bg = brand.background;
          if (bg.bgType) setBgType(bg.bgType);
          if (bg.bgMouseTracking !== undefined) setBgMouseTracking(bg.bgMouseTracking);
          if (bg.bgParticlesCount !== undefined) setBgParticlesCount(bg.bgParticlesCount);
          if (bg.bgParticlesSpeed) setBgParticlesSpeed(bg.bgParticlesSpeed);
          if (bg.bgParticlesSize !== undefined) setBgParticlesSize(bg.bgParticlesSize);
          if (bg.bgParticlesColor) setBgParticlesColor(bg.bgParticlesColor);
          if (bg.bgParticlesOpacity !== undefined) setBgParticlesOpacity(bg.bgParticlesOpacity);
          if (bg.bgParticlesDirection) setBgParticlesDirection(bg.bgParticlesDirection);
          if (bg.bgParticlesShape) setBgParticlesShape(bg.bgParticlesShape);
          if (bg.bgParticlesIcon) setBgParticlesIcon(bg.bgParticlesIcon);
          if (bg.bgOrbsCount !== undefined) setBgOrbsCount(bg.bgOrbsCount);
          if (bg.bgOrbsOpacity !== undefined) setBgOrbsOpacity(bg.bgOrbsOpacity);
        }
      }

      // 6. Formatear Notas y Deltas
      let requirementsText = '';
      if (Array.isArray(manifest.customDeltasToBuild) && manifest.customDeltasToBuild.length > 0) {
        requirementsText = manifest.customDeltasToBuild.map(delta => {
          return `[Delta: ${delta.name} (${delta.type})]\n- ${delta.description}`;
        }).join('\n\n');
      }

      const cleanJson = JSON.stringify(manifest, null, 2);
      setCustomRequirements(`=== PROTOTIPE MANIFEST ===\n${cleanJson}\n==========================\n\n${requirementsText}`);

      if (manifest.isNewNiche && manifest.newNicheConfig) {
        setSeoDescription(`${manifest.newNicheConfig.displayName} — ${manifest.newNicheConfig.description}`);
      }
      if (manifest.branding && manifest.branding.seoKeywords) {
        setSeoKeywords(manifest.branding.seoKeywords);
      }

      // Construir resumen para el panel de confirmación
      const flags = manifest.featureFlags || {};
      setImportSummary({
        nicheKey: manifest.nicheKey,
        isNewNiche: manifest.isNewNiche,
        nicheDisplayName: manifest.isNewNiche && manifest.newNicheConfig
          ? manifest.newNicheConfig.displayName
          : manifest.nicheKey,
        template: manifest.template || 'template-ventas',
        flags: {
          deliveryEnabled: !!flags.deliveryEnabled,
          kdsEnabled: !!flags.kdsEnabled,
          creditsEnabled: !!flags.creditsEnabled,
          dianBillingEnabled: !!flags.dianBillingEnabled,
        },
        libraryComponents: Array.isArray(manifest.libraryComponentsToInject)
          ? manifest.libraryComponentsToInject
          : [],
        deltas: Array.isArray(manifest.customDeltasToBuild)
          ? manifest.customDeltasToBuild
          : [],
      ...
      showToast(`Error al parsear JSON: ${e.message}`, { type: 'error' });
    }
  };
  ...
  const [expandedOnboardCat, setExpandedOnboardCat] = useState(null)

  useEffect(() => {
    if (libraryList.length > 0) {
      const firstValidCat = libraryList.find(cat => cat.components && cat.components.length > 0);
      if (firstValidCat) {
        setExpandedOnboardCat(firstValidCat.typeKey || firstValidCat.name);
      }
    }
  }, [libraryList]);
  ...
  // --- SISTEMA DE PERSISTENCIA Y BORRADOR DEL ASISTENTE DE APROVISIONAMIENTO (LOCALSTORAGE) ---
  const [hasLoadedDraft, setHasLoadedDraft] = useState(false)

  const handleLoadDraft = () => {
    try {
      const raw = localStorage.getItem('prototipe_wizard_draft')
      if (raw) {
        const draft = JSON.parse(raw)
        if (draft.newClientName) setNewClientName(draft.newClientName)
        if (draft.niche) setNiche(draft.niche)
        if (draft.billingMode) setBillingMode(draft.billingMode)
        if (draft.comisionPorcentaje !== undefined) setComisionPorcentaje(draft.comisionPorcentaje)
        if (draft.montoFijoServicio !== undefined) setMontoFijoServicio(draft.montoFijoServicio)
        if (draft.pagoMensualFijo !== undefined) setPagoMensualFijo(draft.pagoMensualFijo)
        if (draft.enableDianBilling !== undefined) setEnableDianBilling(draft.enableDianBilling)
        if (draft.costoPorFacturaDian !== undefined) setCostoPorFacturaDian(draft.costoPorFacturaDian)
        if (draft.selectedTemplate) setSelectedTemplate(draft.selectedTemplate)
        if (draft.customRequirements) setCustomRequirements(draft.customRequirements)
        if (draft.adminEmail) setAdminEmail(draft.adminEmail)
        if (draft.adminPassword) setAdminPassword(draft.adminPassword)
        if (draft.customPort) setCustomPort(draft.customPort)
        if (draft.whatsappAdmin) setWhatsappAdmin(draft.whatsappAdmin)
        if (draft.storeAddress) setStoreAddress(draft.storeAddress)
        if (draft.fbApiKey) setFbApiKey(draft.fbApiKey)
        if (draft.fbAuthDomain) setFbAuthDomain(draft.fbAuthDomain)
        if (draft.fbProjectId) setFbProjectId(draft.fbProjectId)
        if (draft.fbStorageBucket) setFbStorageBucket(draft.fbStorageBucket)
        if (draft.fbMessagingSenderId) setFbMessagingSenderId(draft.fbMessagingSenderId)
        if (draft.fbAppId) setFbAppId(draft.fbAppId)
        if (draft.fbVapidKey) setFbVapidKey(draft.fbVapidKey)
        if (draft.autoProvisionFirebase !== undefined) setAutoProvisionFirebase(draft.autoProvisionFirebase)
        ...
        if (draft.surfaceColor) setSurfaceColor(draft.surfaceColor)
        if (draft.surface2Color) setSurface2Color(draft.surface2Color)
        if (draft.borderColor) setBorderColor(draft.borderColor)
        if (draft.textMutedColor) setTextMutedColor(draft.textMutedColor)
        if (draft.radiusBase) setRadiusBase(draft.radiusBase)
        if (draft.googleFont) setGoogleFont(draft.googleFont)
        if (draft.shadowStyle) setShadowStyle(draft.shadowStyle)
        if (draft.glassmorphism !== undefined) setGlassmorphism(draft.glassmorphism)
        if (draft.animationSpeed) setAnimationSpeed(draft.animationSpeed)
        if (draft.radiusMode) setRadiusMode(draft.radiusMode)
        if (draft.bgType) setBgType(draft.bgType)
        if (draft.bgMouseTracking !== undefined) setBgMouseTracking(draft.bgMouseTracking)
        if (draft.bgParticlesCount !== undefined) setBgParticlesCount(draft.bgParticlesCount)
        if (draft.bgParticlesSpeed) setBgParticlesSpeed(draft.bgParticlesSpeed)
        if (draft.bgParticlesSize !== undefined) setBgParticlesSize(draft.bgParticlesSize)
        if (draft.bgParticlesColor) setBgParticlesColor(draft.bgParticlesColor)
        if (draft.bgParticlesOpacity !== undefined) setBgParticlesOpacity(draft.bgParticlesOpacity)
        if (draft.bgParticlesDirection) setBgParticlesDirection(draft.bgParticlesDirection)
        if (draft.bgParticlesShape) setBgParticlesShape(draft.bgParticlesShape)
        if (draft.bgParticlesIcon) setBgParticlesIcon(draft.bgParticlesIcon)
        if (draft.bgOrbsCount !== undefined) setBgOrbsCount(draft.bgOrbsCount)
        if (draft.bgOrbsOpacity !== undefined) setBgOrbsOpacity(draft.bgOrbsOpacity)
        if (draft.borderBeam !== undefined) setBorderBeam(draft.borderBeam)
        ...
        if (draft.enableDelivery !== undefined) setEnableDelivery(draft.enableDelivery)
        if (draft.enableKds !== undefined) setEnableKds(draft.enableKds)
        if (draft.seoDescription) setSeoDescription(draft.seoDescription)
        if (draft.seoKeywords) setSeoKeywords(draft.seoKeywords)
        if (draft.selectedRecomendations) setSelectedRecomendations(draft.selectedRecomendations)
        
        showToast('Borrador del asistente restaurado automáticamente', { type: 'info' })
  ...
  const handleClearDraft = () => {
    localStorage.removeItem('prototipe_wizard_draft')
    setNewClientName('')
    setNiche('retail_clothing')
    setBillingMode('comision')
    setComisionPorcentaje(5)
    setMontoFijoServicio(0)
    setPagoMensualFijo(0)
    setEnableDianBilling(false)
    setCostoPorFacturaDian(0)
    setSelectedTemplate('template-core-seed')
    setCustomRequirements('')
    setAdminEmail('')
    setAdminPassword('')
    setCustomPort('')
    ...
    setFbAppId('')
    setFbVapidKey('')
    setAutoProvisionFirebase(false)
    setPrimaryColor('#6366f1')
    setSecondaryColor('#06b6d4')
    setBgColor('#0b0f19')
    setTextColor('#f8fafc')
    setSurfaceColor('#111827')
    setSurface2Color('#1f2937')
    setBorderColor('#374151')
    setTextMutedColor('#9ca3af')
    setRadiusBase('12px')
    setGoogleFont('Outfit')
    setEnableGithub(true)
    setEnableFirebaseDeploy(true)
    setEnablePwa(true)
    setEnablePush(true)
    ...
    showToast('Campos del asistente restablecidos', { type: 'success' })
  }
  ...
  // Guardar borrador automáticamente al cambiar los campos del wizard
  useEffect(() => {
    const draft = {
      newClientName,
      niche,
    ...
    if (newClientName || fbProjectId || customRequirements || primaryColor !== '#6366f1') {
      localStorage.setItem('prototipe_wizard_draft', JSON.stringify(draft))
      setHasLoadedDraft(true)
    }
  }, [
    newClientName, niche, billingMode, comisionPorcentaje, montoFijoServicio,
    pagoMensualFijo, enableDianBilling, costoPorFacturaDian, selectedTemplate,
    customRequirements, adminEmail, adminPassword, customPort, whatsappAdmin,
    storeAddress, fbApiKey, fbAuthDomain, fbProjectId, fbStorageBucket,
    fbMessagingSenderId, fbAppId, fbVapidKey, autoProvisionFirebase,
    primaryColor, secondaryColor, bgColor, textColor, surfaceColor,
    surface2Color, borderColor, textMutedColor, radiusBase, googleFont,
    shadowStyle, glassmorphism, animationSpeed, radiusMode,
    bgType, bgMouseTracking, bgParticlesCount, bgParticlesSpeed,
  ...
  const handleLoginGoogle = async () => {
    setAuthError('')
    setAuthLoading(true)
    addLog("Iniciando sesión con Google...", "info")
    
    const centralApp = getCentralApp()
    if (!centralApp) {
      setUser({ email: 'google-simulated@dev.com', uid: 'simulated-google-uid' })
      localStorage.setItem('developer_google_token', 'simulated-google-access-token')
      setAuthLoading(false)
      addLog("Sandbox Bypass: Sesión Google simulada autorizada.", "success")
      return
    }
    ...
    try {
      ...
      // Añadir scope para Cloud Platform
      provider.addScope('https://www.googleapis.com/auth/cloud-platform')
      const result = await signInWithPopup(authInstance, provider)
      
      const credential = GoogleAuthProvider.credentialFromResult(result)
      if (credential && credential.accessToken) {
        localStorage.setItem('developer_google_token', credential.accessToken)
        addLog(`Sesión Google autorizada: ${result.user.email} (Token de acceso guardado)`, "success")
      } else {
        addLog(`Sesión Google iniciada pero no se obtuvo Access Token. Se usara fallback de consola.`, "warning")
      }
    } catch (err) {
      console.error(err)
      setAuthError(`Error de autenticación Google: ${err.message}`)
      addLog("Fallo de inicio de sesión con Google.", "error")
    } finally {
      setAuthLoading(false)
    }
  }
  ...
  const handleLoginGithub = async () => {
    setAuthError('')
    setAuthLoading(true)
    addLog("Iniciando sesión con GitHub...", "info")
    
    const centralApp = getCentralApp()
    if (!centralApp) {
      setUser({ email: 'github-simulated@dev.com', uid: 'simulated-github-uid' })
      setAuthLoading(false)
      addLog("Sandbox Bypass: Sesión GitHub simulada autorizada.", "success")
      return
    }
    ...
    try {
      ...
      const result = await signInWithPopup(authInstance, provider)
      addLog(`Sesión GitHub autorizada: ${result.user.email}`, "success")
    } catch (err) {
      console.error(err)
      setAuthError(`Error de autenticación GitHub: ${err.message}`)
      addLog("Fallo de inicio de sesión con GitHub.", "error")
    } finally {
      setAuthLoading(false)
    }
  }
  ...
  const handleLogout = async () => {
    localStorage.removeItem('developer_google_token')
    const centralApp = getCentralApp()
    if (!centralApp) {
      setUser(null)
      return
  ...
  const isCrmClientCore = useMemo(() => {
    if (!selectedCrmClientId) return false;
    const client = allClientesControl.find(c => c.id.toLowerCase() === selectedCrmClientId.toLowerCase());
    if (client) {
      const idLower = (client.id || '').toLowerCase();
      return client.tipo === 'core' || client.isCore || idLower.includes('smartfix') || idLower === 'ventas' || idLower.includes('-dev');
    }
    const idLower = selectedCrmClientId.toLowerCase();
    return idLower.includes('smartfix') || idLower === 'ventas' || idLower.includes('-dev');
  }, [selectedCrmClientId, allClientesControl]);

  useEffect(() => {
    if (selectedCrmClientId && isCrmClientCore) {
      setCrmTab('config');
    }
  ...
  const handleSaveCrmConfig = async () => {
    ...
    if (isSimulated) {
      setAllClientesControl(prev => prev.map(c => c.id.toLowerCase() === selectedCrmClientId.toLowerCase() ? {
        ...c,
        niche: editNiche,
  ...
  const handleArchiveClient = async (clientId) => {
    if (isSimulated) {
      setAllClientesControl(prev => prev.map(c => c.id.toLowerCase() === clientId.toLowerCase() ? { ...c, archived: true } : c))
  ...
  const handleUnarchiveClient = async (clientId) => {
    if (isSimulated) {
      setAllClientesControl(prev => prev.map(c => c.id.toLowerCase() === clientId.toLowerCase() ? { ...c, archived: false } : c))
  ...
  const filteredPeriodReports = useMemo(() => {
    const base = selectedPeriod ? reports.filter(r => r.periodo === selectedPeriod) : reports
    
    // Set dinámico de IDs de cores detectados
    const coreIds = new Set(
      allClientesControl
        .filter(c => {
          const idLower = (c.id || '').toLowerCase()
          return c.tipo === 'core' || c.isCore || idLower.includes('smartfix') || idLower === 'ventas' || idLower.includes('-dev')
        })
        .map(c => c.id.toLowerCase())
    )

    return base.filter(r => {
      const idLower = (r.clientId || '').toLowerCase()
      if (coreIds.has(idLower)) return false
      if (idLower.includes('smartfix') || idLower === 'ventas' || idLower.includes('-dev')) return false
      return true
    })
  }, [reports, selectedPeriod, allClientesControl])
      ...
      <div className="min-h-screen relative flex items-center justify-center bg-[var(--color-bg)] px-4 font-sans overflow-hidden transition-colors duration-300">
        ...
        >
          ...
          <div className="flex items-center my-4">
            ...
            <span className="px-3 text-[10px] text-[var(--color-text-muted)] font-medium uppercase tracking-wider">o continuar con</span>
            <div className="flex-1 border-t border-[var(--color-border)]/50"></div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <button
              ...
              disabled={authLoading}
              className="h-11 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] hover:bg-[var(--color-surface-2)] text-xs font-bold text-[var(--color-text)] transition-all cursor-pointer flex items-center justify-center gap-2 active:scale-[0.98] disabled:opacity-50"
            >
  ...
  if (isOnboardingActive) {
    return (
      <div className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text)] font-sans pb-12 overflow-x-clip transition-colors duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-4">
                ...
                {hasLoadedDraft && (
                  <button
                    ...
                    className="h-8 px-3 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-450 border border-rose-500/20 text-[10px] font-bold transition-all cursor-pointer flex items-center gap-1.5"
                    title="Restablece todos los campos a su estado inicial"
                  >
                    <Trash2 size={12} />
                    Limpiar Borrador
                  </button>
              ...
              <div className="space-y-4 min-h-[350px]">
                {wizardTab === 'server' && (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-[var(--color-text-muted)] block">ID de Cliente (Auto-generado)</label>
                        <input 
                          type="text" 
                          disabled
                      ...
                      <div className="space-y-1">
                        ...
                        <CustomSelect 
                          ...
                          options={[
                            { value: 'percentage', label: 'Porcentaje de Venta' },
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-[var(--color-text-muted)] block">Token de Telemetría (Auto-generado)</label>
                        <input 
                          type="text" 
                          disabled
                      ...
                      <div className="space-y-1">
                        ...
                        <CustomSelect 
                          ...
                          options={templates.map(t => ({ value: t.id || t, label: t.name || t }))}
                        />
                      </div>
                    </div>
                ...
                {wizardTab === 'branding' && (
                  <div className="space-y-6 animate-fade-in">
                      ...
                      <div className="p-4 bg-[var(--color-surface-2)]/20 border border-[var(--color-border)] rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 select-none">
                        <div className="space-y-0.5">
                          <span className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-wider block">Paletas recomendadas</span>
                          <span className="text-[9px] text-[var(--color-text-muted)] leading-relaxed block">
                            Elige combinaciones creadas para los 23 nichos de negocio del ecosistema.
                          </span>
                        </div>
                        <button
                          type="button"
                          ...
                          className="px-4 py-2.5 bg-[var(--color-surface-3)] hover:bg-[var(--color-border)] text-[var(--color-text)] border border-[var(--color-border)] rounded-xl text-xs font-bold flex items-center gap-2 transition-all hover:scale-[1.03] active:scale-95 cursor-pointer shadow-sm shrink-0"
                        >
                          <Palette size={13} className="text-indigo-400" />
                          Explorar Paletas de Nicho
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        {/* ── Design Effects Studio (CORE-242) ── */}
                        <div className="sm:col-span-2">
                          <BrandingEffectsPanel
                            ...
                            bgType={bgType}
                            setBgType={setBgType}
                            bgMouseTracking={bgMouseTracking}
                            setBgMouseTracking={setBgMouseTracking}
                            bgParticlesCount={bgParticlesCount}
                            setBgParticlesCount={setBgParticlesCount}
                            bgParticlesSpeed={bgParticlesSpeed}
                            setBgParticlesSpeed={setBgParticlesSpeed}
                            bgParticlesSize={bgParticlesSize}
                            setBgParticlesSize={setBgParticlesSize}
                            bgParticlesColor={bgParticlesColor}
                            setBgParticlesColor={setBgParticlesColor}
                            bgParticlesOpacity={bgParticlesOpacity}
                            setBgParticlesOpacity={setBgParticlesOpacity}
                            bgParticlesDirection={bgParticlesDirection}
                            setBgParticlesDirection={setBgParticlesDirection}
                            bgParticlesShape={bgParticlesShape}
                            setBgParticlesShape={setBgParticlesShape}
                            bgParticlesIcon={bgParticlesIcon}
                            setBgParticlesIcon={setBgParticlesIcon}
                            bgOrbsCount={bgOrbsCount}
                            setBgOrbsCount={setBgOrbsCount}
                            bgOrbsOpacity={bgOrbsOpacity}
                            setBgOrbsOpacity={setBgOrbsOpacity}
                            borderBeam={borderBeam}
                            setBorderBeam={setBorderBeam}
                            tilt3d={tilt3d}
                            setTilt3d={setTilt3d}
                            niche={niche}
                          />
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-2xl space-y-4">
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <label className="flex items-center gap-2.5 cursor-pointer text-xs font-semibold text-[var(--color-text-muted)] select-none">
                          <input 
                            ...
                            className="w-4 h-4 rounded accent-indigo-600 bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] focus:ring-0 focus:outline-none"
                          />
                          Módulo de Domicilios y Despachos
                        </label>
                        ...
                        <label className="flex items-center gap-2.5 cursor-pointer text-xs font-semibold text-[var(--color-text-muted)] select-none">
                          <input 
                            type="checkbox" 
                            checked={enableKds} 
                            onChange={(e) => setEnableKds(e.target.checked)}
                            className="w-4 h-4 rounded accent-indigo-600 bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] focus:ring-0 focus:outline-none"
                          />
                      ...
                      <div className="border-t border-[var(--color-border)] pt-3.5 space-y-2">
                        ...
                        {/* [UX-3] Buscador interactivo con ordenamiento por relevancia */}
                        <div className="relative">
                          <input
                            type="text"
                            value={recommendationsSearchQuery}
                            onChange={(e) => setRecommendationsSearchQuery(e.target.value)}
                            placeholder="🔍 Buscar por cualquier palabra... (ej: agenda, scanner, auth, hook)"
                            className="bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-xl px-3 py-2 text-xs w-full text-[var(--color-text)] outline-none focus:border-indigo-500 placeholder-[var(--color-text-muted)]/40 focus:ring-1 focus:ring-indigo-500/20"
                          />
                          {recommendationsSearchQuery && (
                            <button
                              type="button"
                              onClick={() => setRecommendationsSearchQuery('')}
                              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-indigo-400 hover:text-indigo-300 font-bold cursor-pointer"
                            >
                              Limpiar
                            </button>
                          )}
                        ...
                        <div className="bg-[var(--color-bg)]/60 border border-[var(--color-border)] rounded-xl p-2.5 max-h-64 overflow-y-auto space-y-3 scrollbar-thin">
                          {libraryList.length === 0 ? (
                            ...
                            // Helper local para filtrar y ordenar por relevancia
                            const query = recommendationsSearchQuery.trim().toLowerCase();
                            if (query) {
                              const queryWords = query.split(/\s+/).filter(Boolean);
                              const matches = [];

                              libraryList.forEach((cat) => {
                                if (!cat.components) return;
                                cat.components.forEach((comp) => {
                                  let score = 0;
                                  const nameLower = comp.name.toLowerCase();
                                  const techNameLower = (comp.technicalName || '').toLowerCase();
                                  const catNameLower = cat.name.toLowerCase();

                                  queryWords.forEach((word) => {
                                    // Puntuación: coincidencia exacta al inicio > coincidencia parcial
                                    if (nameLower.startsWith(word)) score += 15;
                                    else if (nameLower.includes(word)) score += 8;

                                    if (techNameLower.startsWith(word)) score += 10;
                                    else if (techNameLower.includes(word)) score += 5;

                                    if (catNameLower.includes(word)) score += 2;
                                  });
                                  ...
                                  if (score > 0) {
                                    matches.push({
                                      ...
                                      isModule: comp.resourceType === 'module' || cat.isModule
                                    });
                                  }
                                });
                              });
                              ...
                              if (sortedMatches.length === 0) {
                                return (
                                  <div className="py-6 text-center text-[10px] text-[var(--color-text-muted)] italic">
                                    No se encontraron componentes para "{recommendationsSearchQuery}"
                                  </div>
                                );
                              }
                              ...
                              return (
                                <div className="grid grid-cols-2 gap-1.5 animate-fade-in">
                                  {sortedMatches.map((comp, compIdx) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <div className="w-full">
                                          <span className={`text-[10px] font-bold block leading-tight truncate ${isSelected ? 'text-[var(--color-text)]' : 'text-[var(--color-text-muted)]'}`}>
                                            {comp.name}
                                          </span>
                                          {comp.technicalName && (
                                          ...
                                          <span className="text-[7px] uppercase tracking-wider font-extrabold text-indigo-400 mt-1 block">
                                            📁 {comp.categoryName}
                                          </span>
                                        </div>
                                      </div>
                            ...
                            // Renderizado por categorías normal
                            return libraryList.map((cat, catIdx) => {
                              if (!cat.components || cat.components.length === 0) return null;
                              
                              const key = cat.typeKey || cat.name;
                              const isCollapsed = expandedOnboardCat !== key;
                              const style = ONBOARD_COMP_TYPE_STYLES[cat.typeKey] || ONBOARD_COMP_TYPE_STYLES.component;

                              return (
                                <div key={catIdx} className="space-y-1.5">
                                  {/* Category folder header button */}
                                  <button
                                    type="button"
                                    onClick={() => setExpandedOnboardCat(prev => prev === key ? null : key)}
                                    className={`w-full flex items-center gap-2 px-3 py-1.5 rounded-xl ${style.bg} border ${style.border} hover:opacity-90 transition-all cursor-pointer text-left`}
                                  >
                                    <ChevronRight
                                      size={10}
                                      className={`${style.color} transition-transform duration-200 shrink-0 ${isCollapsed ? '' : 'rotate-90'}`}
                                    />
                                    <span className="text-xs leading-none shrink-0">{cat.icon || style.icon}</span>
                      ...
                      <div className="border-t border-[var(--color-border)] pt-3.5 space-y-1.5">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => {
                              loadBriefingSessions();
                              setIsBriefingSelectModalOpen(true);
                            }}
                            className="text-[9px] font-bold text-emerald-500 hover:text-emerald-450 cursor-pointer flex items-center gap-1 bg-emerald-500/10 px-2 py-0.5 rounded-md hover:bg-emerald-500/20 transition-all border-none outline-none"
                          >
                            📥 Cargar desde Briefing
                          </button>
                          <button
                            type="button"
                            onClick={() => setIsImportModalOpen(true)}
                            className="text-[9px] font-bold text-indigo-500 hover:text-indigo-400 cursor-pointer flex items-center gap-1 bg-indigo-500/10 px-2 py-0.5 rounded-md hover:bg-indigo-500/20 transition-all border-none outline-none"
                          >
                            🔌 Importar Manifiesto
                          </button>
                        </div>
                      ...
                      {/* [BRECHA-2] SEO y Metadatos del Cliente */}
                      <div className="border-t border-[var(--color-border)] pt-3.5 space-y-2">
                        <label className="text-[10px] font-bold text-[var(--color-text-muted)] block uppercase tracking-wider">🔍 SEO &amp; Metadatos</label>
                        <input
                          type="text"
                          value={seoDescription}
                          onChange={(e) => setSeoDescription(e.target.value)}
                          placeholder={`Meta description — ej: "${newClientName || 'Tu negocio'} — Compras y pedidos en línea"`}
                          className="bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-xl px-3 py-2 text-xs w-full text-[var(--color-text)] outline-none focus:border-indigo-500"
                        />
                        <input
                          type="text"
                          ...
                          placeholder="Keywords SEO — ej: tienda online, pedidos, catálogo"
                          className="bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-xl px-3 py-2 text-xs w-full text-[var(--color-text)] outline-none focus:border-indigo-500"
                        />
                        <p className="text-[9px] text-[var(--color-text-muted)]">Opcionales — si se omiten, se genera descripción automática.</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="flex items-center justify-between border-t border-[var(--color-border)] pt-4">
                ...
                <button 
                  ...
                  onClick={async () => {
                    ...
                    // [UX-1] Pre-validación síncrona: detectar campos faltantes ANTES
                    // de lanzar el worker y esperar 10+ min para saber que falló.
                    const missingFields = []
                    if (!selectedTemplate) missingFields.push('Plantilla base (Core)')
                    if (!targetPath?.trim()) missingFields.push('Ruta de destino del proyecto')
                    if (!autoProvisionFirebase) {
                      if (!fbApiKey?.trim()) missingFields.push('Firebase API Key')
                      if (!fbProjectId?.trim()) missingFields.push('Firebase Project ID')
                      if (!fbAuthDomain?.trim()) missingFields.push('Firebase Auth Domain')
                      if (!fbStorageBucket?.trim()) missingFields.push('Firebase Storage Bucket')
                      if (!fbAppId?.trim()) missingFields.push('Firebase App ID')
                    }
                    if (missingFields.length > 0) {
                      alert(`⚠️ Faltan campos requeridos para el aprovisionamiento:\n\n• ${missingFields.join('\n• ')}\n\nCompleta estos datos antes de continuar.`)
                      return
                    }
                    setProvisioningStageLabel('Iniciando...')
                    setProvisioningLogs([
                    ...
                    const rawPayload = {
                      ...
                      // [BRECHA-2] SEO del cliente
                      seoTitle: newClientName.trim(),
                      seoDescription: seoDescription.trim() || `${newClientName.trim()} — Plataforma de ventas, inventario y pedidos en línea`,
                    ...
                    if (isSimulated) {
                      ...
                      setAllClientesControl(prev => [...prev, { id: clientId, niche, billingMode, comisionPorcentaje, montoFijoServicio, pagoMensualFijo, enableDianBilling, costoPorFacturaDian }])
                    ...
                    try {
                      ...
                      // 1. Llamar primero al API local /api/create-project
                      setProvisioningLogs(prev => [...prev, `[API Bridge] ⏳ Enviando parámetros de aprovisionamiento al servidor...`])
                      const cliRes = await fetch(`${CLI_URL}/api/create-project`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                      ...
                      setProvisioningLogs(prev => [
                        ...
                        `[CLI API] ⏳ Conectando canal de eventos SSE para transmisión de logs...`
                      ])
                        ...
                        eventSource.onmessage = async (event) => {
                          try {
                            ...
                            if (data.type === 'log') {
                              addLog(data.line, "info")
                              setProvisioningLogs(prev => [...prev, data.line])
                              // [UX-2] Parsear el log para detectar la etapa actual del aprovisionamiento
                              const STAGE_PATTERNS = [
                                { regex: /Copiar estructura base/i, label: '1/12 — Copiando plantilla base...' },
                                { regex: /Documentación estándar/i, label: '2/12 — Generando documentación...' },
                                { regex: /Variables de marca/i, label: '3/12 — Inyectando paleta de colores...' },
                              ...
                              // --- 3. ESCRIBIR A FIRESTORE ÚNICAMENTE SI EL APROVISIONAMIENTO FÍSICO ES EXITOSO ---
                              addLog(`[Firestore] Registrando cliente ${clientId} en la nube central...`, "info")
                              setProvisioningLogs(prev => [...prev, `[Firestore] ⏳ Registrando cliente en la nube central...`])
                              const clientRef = doc(dbInstance, 'clientes_control', clientId)
                              await setDoc(clientRef, {
                              ...
                              if (selectedRecomendations && selectedRecomendations.length > 0) {
                                addLog(`[Auto-Inject] Inyectando ${selectedRecomendations.length} componentes...`, "info")
                                setProvisioningLogs(prev => [...prev, `[Auto-Inject] ⏳ Iniciando auto-inyección de ${selectedRecomendations.length} componentes seleccionados...`])
                                for (const rec of selectedRecomendations) {
                                  ...
                                  try {
                                    ...
                                    if (injectRes.ok) {
                                      ...
                                      setProvisioningLogs(prev => [...prev, `[Auto-Inject] ✅ Componente "${rec.name}" inyectado con éxito.`])
                                    } else {
                                      const errTxt = await injectRes.text()
                                      addLog(`[Auto-Inject Warning] Falló inyección de ${rec.name}: ${errTxt}`, "warning")
                                      setProvisioningLogs(prev => [...prev, `[Auto-Inject Warning] ⚠️ Falló inyección de "${rec.name}": ${errTxt}`])
                                    }
                                  } catch (injErr) {
                                    addLog(`[Auto-Inject Warning] Error de conexión inyectando ${rec.name}: ${injErr.message}`, "warning")
                                    setProvisioningLogs(prev => [...prev, `[Auto-Inject Warning] ⚠️ Error de conexión inyectando "${rec.name}": ${injErr.message}`])
                                  }
                                }
                                addLog(`[Auto-Inject] Inyección en lote finalizada.`, "success")
                                setProvisioningLogs(prev => [...prev, `[Auto-Inject] ✅ Inyección en lote finalizada con éxito.`])
                              }
            ...
            <div className="lg:col-span-5 relative h-full mockup-preview-panel">
              ...
              <div className="flex flex-col items-center gap-3 w-full mb-3 select-none">
                  ...
                  {(() => {
                    const mockVarsStyle = {
                      ...
                      "--brand-primary-rgb": primaryRgbStr,
                      "--brand-primary-hsl": primaryHslStr,
                      "--brand-neon-hsl": neonHslStr,
                      "--shadow-default": previewShadowCss,
                      "--shadow-card": previewShadowCss,
                      "--shadow-button": previewShadowCss,
                      "--radius-card": previewRadiusCss,
                      "--radius-button": previewBtnRadius,
                      "--glass-blur": glassmorphism ? '16px' : '0px',
                      "--glass-opacity": glassmorphism ? '0.07' : '0',
                      "--glass-bg": glassmorphism ? 'rgba(255,255,255,0.07)' : 'transparent',
                      "--glass-border": glassmorphism ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0)'
                    };
                    window.__mockVarsStyle = mockVarsStyle; // Guardar temporalmente para reuso
                    return null;
                  })()}
              ...
              {mockDeviceMode === 'desktop' ? (
                ...
                <div 
                  className="w-full max-w-[340px] sm:max-w-[360px] md:max-w-[420px] h-[340px] rounded-2xl relative shadow-2xl transition-all border border-slate-700/50 flex flex-col overflow-hidden"
                  style={{ 
                    backgroundColor: isBrandDark ? (bgType === 'solid' ? bgColor : '#070a13') : (bgType === 'solid' ? bgColor : '#f8fafc'), 
                    color: isBrandDark ? textColor : '#111827',
                    fontFamily: `'${googleFont}', sans-serif`,
                    boxShadow: previewShadowCss !== 'none' ? previewShadowCss : `0 20px 40px -10px ${primaryColor}20, 0 0 2px 2px ${primaryColor}40`,
                    transition: `all ${animDurationStyle} ease`,
                    ...window.__mockVarsStyle
                  }}
                >
                  ...
                  <div className="flex-1 flex overflow-hidden relative">
                    {/* Left Sidebar Nav */}
                    <div className="w-24 bg-slate-500/[0.04] border-r border-slate-500/10 p-1.5 flex flex-col gap-3 shrink-0 relative z-10">
                    ...
                    {/* Right Content Frame */}
                    <div className="flex-1 flex flex-col overflow-hidden p-2.5 relative">
                      {/* Fondo dinámico del Design Studio en laptop */}
                      <BackgroundCanvas
                        bgType={bgType}
                        bgMouseTracking={bgMouseTracking}
                        bgParticlesCount={bgParticlesCount}
                      ...
                      <div className="flex-1 overflow-y-auto space-y-2.5 scrollbar-none pr-0.5 relative z-10">
                          ...
                          <div className="space-y-2.5">
                            <div className="grid grid-cols-2 gap-2">
                              <InteractiveTiltCard
                                enabled={tilt3d}
                                primaryColor={primaryColor}
                                className={`p-2.5 relative overflow-hidden transition-all shadow-sm border flex flex-col justify-center ${borderBeam ? 'border-beam-container border-beam-effect' : ''}`}
                                style={{ 
                                  background: previewBgStyle,
                                  backdropFilter: previewBlurStyle,
                                  WebkitBackdropFilter: previewBlurStyle,
                                  borderColor: previewBorderStyle.split(' ')[2],
                                  borderRadius: previewRadiusCss,
                                  boxShadow: previewShadowCss,
                                  transition: `all ${animDurationStyle} ease`,
                                  '--border-beam-width': borderBeam ? '1.2px' : '0px'
                                }}
                              >
                ...
                <div 
                  className="w-[240px] h-[480px] rounded-[30px] p-2 relative shadow-2xl transition-all border border-slate-700/50 flex flex-col overflow-hidden"
                  style={{ 
                    backgroundColor: isBrandDark ? (bgType === 'solid' ? bgColor : '#070a13') : (bgType === 'solid' ? bgColor : '#f8fafc'), 
                    color: isBrandDark ? textColor : '#111827',
                    fontFamily: `'${googleFont}', sans-serif`,
                ...
                >
                  ...
                  <div className="h-full w-full flex flex-col justify-between relative overflow-hidden rounded-[22px] p-3 pt-5 z-10">
                    
                    {/* Fondo dinámico del Design Studio en móvil */}
                    <BackgroundCanvas
                      bgType={bgType}
                      bgMouseTracking={bgMouseTracking}
                      bgParticlesCount={bgParticlesCount}
                    ...
                    {/* Status Bar */}
                    <div className="flex items-center justify-between text-[8px] opacity-80 mb-2 font-mono relative z-10">
                      <span>16:49</span>
                      <div className="flex items-center gap-1">
                        <span>5G</span>
                    ...
                    {/* Mock App Header */}
                    <div className="flex items-center justify-between mt-1 shrink-0 relative z-10">
                      <div className="flex items-center gap-1.5">
                        <div 
                      ...
                      <div 
                        className="w-5 h-5 rounded-full bg-slate-500/10 flex items-center justify-center cursor-pointer hover:bg-slate-500/20 transition-colors"
                    ...
                    {/* Mock Contents */}
                    <div className="flex-1 my-3 overflow-y-auto space-y-3 scrollbar-none px-2.5 relative z-10">
                      ...
                      {mockActiveTab === 'inicio' && (
                        <div className="space-y-3">
                          ...
                          <InteractiveTiltCard
                            enabled={tilt3d}
                            primaryColor={primaryColor}
                            className={`p-3 relative transition-all border ${borderBeam ? 'overflow-hidden border-beam-container border-beam-effect' : ''}`}
                            style={{ 
                              background: previewBgStyle,
                              backdropFilter: previewBlurStyle,
                              WebkitBackdropFilter: previewBlurStyle,
                        ...
                        {mockIsNewSaleOpen ? (
                          <div 
                            className="p-3 border space-y-2 animate-scale-up"
                            style={{
                              background: previewBgStyle,
                              backdropFilter: previewBlurStyle,
                              WebkitBackdropFilter: previewBlurStyle,
                          ...
                          >
                            ...
                            <input 
                              ...
                              value={mockNewSaleTitle}
                              onChange={(e) => setMockNewSaleTitle(e.target.value)}
                              className="w-full bg-slate-500/5 border border-slate-500/25 px-2 py-1 text-[9px] outline-none focus:border-indigo-500"
                              style={{ 
                                color: 'inherit',
                                borderRadius: previewBtnRadius
                              }}
                            ...
                            <button
                              ...
                              className="w-full py-1.5 text-[9px] font-bold text-white cursor-pointer hover:brightness-110 active:scale-95 transition-all"
                              style={{ 
                                backgroundColor: primaryColor,
                                borderRadius: previewBtnRadius,
                                boxShadow: previewShadowCss
                          ...
                          <button 
                            type="button"
                            onClick={() => setMockIsNewSaleOpen(true)}
                            className="w-full py-2 text-[9px] font-bold text-white active:scale-95 transition-all cursor-pointer flex items-center justify-center gap-1 hover:brightness-110"
                            style={{ 
                              backgroundColor: primaryColor,
                              borderRadius: previewBtnRadius,
                              boxShadow: previewShadowCss
                        ...
                        <div className="space-y-1.5">
                          ...
                          {mockOrders.length === 0 ? (
                            ...
                            mockOrders.slice(0, 3).map((item) => (
                              <div 
                                key={item.id} 
                                className="p-2 border flex items-center justify-between text-[8px] hover:bg-slate-500/10 transition-colors"
                                style={{
                                  background: previewBgStyle,
                                  borderColor: previewBorderStyle.split(' ')[2],
                                  borderRadius: previewRadiusCss,
                    ...
                    {mockActiveTab === 'ventas' && (
                      <div className="space-y-3">
                        ...
                        <div 
                          className="p-2 border space-y-1.5"
                          style={{
                            background: previewBgStyle,
                            borderColor: previewBorderStyle.split(' ')[2],
                            borderRadius: previewRadiusCss,
                          ...
                          <div className="space-y-1.5 max-h-[160px] overflow-y-auto pr-0.5 scrollbar-none">
                            {mockOrders.length === 0 ? (
                              ...
                              mockOrders.map((item) => (
                                <div 
                                  key={item.id} 
                                  className="p-2 border flex items-center justify-between text-[8px] hover:bg-slate-500/10 group/row transition-all duration-200"
                                  style={{
                                    background: previewBgStyle,
                                    borderColor: previewBorderStyle.split(' ')[2],
                                    borderRadius: previewRadiusCss,
                      ...
                      <div className="space-y-3 animate-fade-in">
                        ...
                        <div className="space-y-2 max-h-[260px] overflow-y-auto pr-0.5 scrollbar-none">
                          {(MOCK_CATALOG[niche] || MOCK_CATALOG.retail_clothing).map((item) => (
                            <div 
                              key={item.id} 
                              className="p-2 border flex items-center justify-between gap-2 transition-all hover:bg-slate-500/10"
                              style={{
                                background: previewBgStyle,
                                borderColor: previewBorderStyle.split(' ')[2],
                                borderRadius: previewRadiusCss,
                            ...
                            >
                              ...
                              <button
                                ...
                                className="px-2 py-1 text-[8px] font-bold text-white transition-all hover:scale-105 active:scale-95 shrink-0 cursor-pointer"
                                style={{ 
                                  backgroundColor: primaryColor,
                                  borderRadius: previewBtnRadius,
                                  boxShadow: previewShadowCss
        ...
        {/* Modal de Importación de Manifiesto de Aprovisionamiento */}
        {/* Modal Resumen de Importación de Manifiesto */}
        {isImportSummaryOpen && importSummary && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4 animate-fade-in">
            <div className="absolute inset-0" onClick={() => setIsImportSummaryOpen(false)} />
            <div className="relative w-full max-w-lg bg-[var(--color-surface)] border border-[var(--color-border)] rounded-3xl shadow-2xl animate-scale-up flex flex-col overflow-hidden">
              {/* Header */}
              <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[var(--color-border)]">
                <div className="flex items-center gap-3">
                  <span className="text-xl">✅</span>
                  <div>
                    <h3 className="text-sm font-bold text-[var(--color-text)]">Manifiesto Importado Correctamente</h3>
                    <p className="text-[10px] text-[var(--color-text-muted)]">Resumen de lo configurado automáticamente en el wizard</p>
                  </div>
                </div>
                <button onClick={() => setIsImportSummaryOpen(false)} className="text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text)] bg-transparent border-none cursor-pointer">✕</button>
              </div>
              ...
              <div className="px-6 py-5 space-y-4 overflow-y-auto max-h-[70vh]">
                ...
                <div className="flex gap-3">
                  <div className="flex-1 bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-2xl p-3">
                    <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1">Vertical de Negocio</p>
                    <p className="text-xs font-bold text-[var(--color-text)]">{importSummary.nicheDisplayName}</p>
                    <p className="text-[10px] text-[var(--color-text-muted)] font-mono mt-0.5">{importSummary.nicheKey}</p>
                    {importSummary.isNewNiche && (
                      <span className="mt-1.5 inline-block px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-400 text-[9px] font-bold">NUEVO NICHO</span>
                    )}
                  </div>
                  <div className="flex-1 bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-2xl p-3">
                    <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1">Template Base</p>
                    <p className="text-xs font-bold text-[var(--color-text)] font-mono">{importSummary.template}</p>
                    {importSummary.hasBranding && (
                      <span className="mt-1.5 inline-block px-2 py-0.5 rounded-full bg-indigo-500/15 text-indigo-400 text-[9px] font-bold">🎨 Branding aplicado</span>
                    )}
                  </div>
                </div>

                {/* Feature Flags */}
                <div className="bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-2xl p-3">
                  <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-2">Módulos del Core activados</p>
                  <div className="grid grid-cols-2 gap-1.5">
                    {[
                      { key: 'deliveryEnabled', label: '🚴 Domicilios y Despachos' },
                      { key: 'kdsEnabled', label: '🍳 Pantalla Cocina / KDS' },
                      { key: 'creditsEnabled', label: '💳 Créditos / Billing' },
                      { key: 'dianBillingEnabled', label: '🧾 Facturación DIAN' },
                    ].map(f => (
                      <div key={f.key} className={`flex items-center gap-1.5 px-2 py-1.5 rounded-xl text-[10px] font-semibold border ${
                        importSummary.flags[f.key]
                          ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'
                          : 'bg-[var(--color-surface-3)]/20 border-[var(--color-border)] text-[var(--color-text-muted)]/50'
                      }`}>
                        <span>{importSummary.flags[f.key] ? '✅' : '○'}</span>
                        <span>{f.label}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-2xl p-3">
                  <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-2">
                    Componentes de Biblioteca inyectados
                    <span className="ml-2 px-1.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400 text-[9px]">{importSummary.libraryComponents.length}</span>
                  </p>
                  {importSummary.libraryComponents.length > 0 ? (
                    <div className="flex flex-wrap gap-1.5">
                      {importSummary.libraryComponents.map(c => (
                        <span key={c} className="px-2 py-1 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[10px] font-mono font-semibold">{c}</span>
                      ))}
                    </div>
                  ) : (
                    <p className="text-[10px] text-[var(--color-text-muted)]">Ninguno seleccionado</p>
                  )}
                </div>
                ...
                <div className="bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-2xl p-3">
                  <p className="text-[9px] font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-2">
                    Deltas a programar
                    <span className="ml-2 px-1.5 py-0.5 rounded-full bg-amber-500/20 text-amber-400 text-[9px]">{importSummary.deltas.length}</span>
                  </p>
                  {importSummary.deltas.length > 0 ? (
                    <div className="space-y-1.5">
                      {importSummary.deltas.map((d, i) => (
                        <div key={i} className="flex items-start gap-2">
                          <span className={`shrink-0 mt-0.5 px-1.5 py-0.5 rounded-md text-[8px] font-bold ${
                            d.type === 'PROPUESTA DE EXTRACTOR'
                              ? 'bg-purple-500/15 text-purple-400'
                              : 'bg-amber-500/15 text-amber-400'
                          }`}>{d.type === 'PROPUESTA DE EXTRACTOR' ? 'EXTRACTOR' : 'CREAR LOCAL'}</span>
                          <span className="text-[10px] font-semibold text-[var(--color-text)]">{d.name}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-[10px] text-[var(--color-text-muted)]">Sin deltas custom</p>
                  )}
                </div>
              </div>

              <div className="px-6 pb-5 pt-3 border-t border-[var(--color-border)]">
                <button
                  onClick={() => setIsImportSummaryOpen(false)}
                  className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 !text-white rounded-xl text-xs font-bold cursor-pointer transition-all border-none"
                >
                  Entendido — Continuar con el Briefing
                </button>
              </div>
            </div>
          </div>
        ...
        {isBriefingSelectModalOpen && (
          <div className="fixed inset-0 z-[65] flex items-center justify-center bg-slate-950/75 backdrop-blur-sm p-4 animate-fade-in">
            <div className="absolute inset-0" onClick={() => setIsBriefingSelectModalOpen(false)} />
            <div className="relative w-full max-w-lg bg-[var(--color-surface)] border border-[var(--color-border)] rounded-3xl p-6 shadow-2xl animate-scale-up flex flex-col overflow-hidden max-h-[85vh]">
              <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-3 mb-4">
                <div className="flex items-center gap-2">
                  <span className="p-2 rounded-xl bg-emerald-500/10 text-emerald-450 font-bold text-sm">
                    📥
                  </span>
                  <div>
                    <h3 className="text-sm font-bold text-[var(--color-text)]">Cargar desde Briefing</h3>
                    <p className="text-[10px] text-[var(--color-text-muted)]">Elige una sesión de briefing guardada en Firestore</p>
                  </div>
                </div>
                <button 
                  onClick={() => setIsBriefingSelectModalOpen(false)} 
                  className="text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text)] cursor-pointer border-none bg-transparent"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="mb-4">
                <input 
                  ...
                  className="w-full bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl px-3 py-2 text-xs text-[var(--color-text)] outline-none focus:border-emerald-500 font-sans"
                />
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-2.5 max-h-[50vh] pr-1 scrollbar-thin">
                {loadingBriefings ? (
                  <div className="flex flex-col items-center justify-center py-12 gap-3 text-xs text-[var(--color-text-muted)]">
                    <div className="w-5 h-5 rounded-full border-2 border-emerald-500/20 border-t-emerald-500 animate-spin" />
                    <span>Cargando sesiones de Firestore...</span>
                  </div>
                ) : (() => {
                  const filtered = briefingSessions.filter(s => 
                    (s.nombreEmpresa || '').toLowerCase().includes(briefingSearch.toLowerCase())
                  );
                  
                  if (filtered.length === 0) {
                    return (
                      <div className="text-center py-12 text-xs text-[var(--color-text-muted)]">
                        No se encontraron sesiones de briefing.
                      </div>
                    );
                  }
                  ...
                  return filtered.map((session) => {
                    const hasAnalysis = !!session.analysisResult;
                    return (
                      <div key={session.id} className="p-4 bg-[var(--color-surface-2)]/60 hover:bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-2xl flex flex-col gap-2.5 transition-all">
                        <div className="flex justify-between items-start gap-4">
                          <div>
                            <h4 className="text-xs font-bold text-[var(--color-text)] leading-tight">{session.nombreEmpresa || 'Sin Nombre'}</h4>
                            <p className="text-[9px] text-[var(--color-text-muted)] mt-1 flex items-center gap-2">
                              <span>📅 {session.fecha || 'Sin fecha'}</span>
                              <span>•</span>
                              <span>Core: {session.coreKey || 'ventas'}</span>
                            </p>
                          </div>
                          
                          {/* Badge de Estado del Análisis */}
                          {hasAnalysis ? (
                            <span className="text-[9px] font-bold px-2 py-0.5 bg-emerald-500/10 text-emerald-450 border border-emerald-500/20 rounded-full select-none shrink-0 whitespace-nowrap">
                              Analizado
                            </span>
                          ) : (
                            <span className="text-[9px] font-bold px-2 py-0.5 bg-amber-500/10 text-amber-450 border border-amber-500/20 rounded-full select-none shrink-0 whitespace-nowrap">
                              Pendiente Análisis
                            </span>
                          )}
                        </div>
                        ...
                        {/* Botón de carga */}
                        <div className="flex justify-end border-t border-[var(--color-border)]/50 pt-2.5">
                          <button
                            onClick={() => {
                              handleImportBriefingData(session);
                              setIsBriefingSelectModalOpen(false);
                            }}
                            className={`px-3 py-1.5 rounded-lg text-[10px] font-bold cursor-pointer transition-all border-none ${
                              hasAnalysis
                                ? 'bg-emerald-600 hover:bg-emerald-500 !text-white'
                                : 'bg-[var(--color-surface-3)] text-[var(--color-text-muted)] hover:bg-[var(--color-border)]'
                            }`}
                          >
                            Cargar en Wizard
                          </button>
        ...
        {isImportModalOpen && (
          <div className="fixed inset-0 z-[65] flex items-center justify-center bg-slate-950/75 backdrop-blur-sm p-4 animate-fade-in">
            ...
            <div className="relative w-full max-w-lg bg-[var(--color-surface)] border border-[var(--color-border)] rounded-3xl p-6 shadow-2xl animate-scale-up flex flex-col overflow-hidden">
              <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-3 mb-4">
                <div className="flex items-center gap-2">
                  <span className="p-2 rounded-xl bg-indigo-500/10 text-indigo-400 font-bold text-sm">
                    🔌
                  </span>
                  <div>
                    <h3 className="text-sm font-bold text-[var(--color-text)]">Importar Manifiesto de Aprovisionamiento</h3>
                    <p className="text-[10px] text-[var(--color-text-muted)]">Pega el manifiesto JSON válido devuelto por la IA de descubrimiento</p>
                  </div>
                </div>
                <button 
              ...
              <div className="space-y-4">
                <textarea
                  ...
                  placeholder='{\n  "nicheKey": "grocery_food",\n  "isNewNiche": false,\n  ...\n}'
                  rows={10}
                  className="w-full bg-[var(--color-surface-2)]/30 border border-[var(--color-border)] rounded-2xl p-4 text-xs font-mono text-[var(--color-text)] outline-none focus:border-indigo-500 resize-none h-[220px]"
                />
                ...
                <div className="flex items-center justify-end gap-2 pt-2 border-t border-[var(--color-border)]">
                  <button
                    ...
                    className="px-4 py-2 text-xs font-bold text-[var(--color-text-muted)] hover:text-[var(--color-text)] cursor-pointer border-none bg-transparent"
                  >
                    Cancelar
                  </button>
                  <button
                    type="button"
                    onClick={() => handleImportManifest(manifestJsonInput)}
                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold cursor-pointer transition-all border-none"
                  >
                    🔌 Procesar e Importar
                  </button>
                </div>
        ...
        {/* Modal de Selección de Paletas de Marca */}
        {isPaletteModalOpen && (
          <div className="fixed inset-0 z-[65] flex items-center justify-center bg-slate-950/75 backdrop-blur-sm animate-fade-in p-4">
            <div className="absolute inset-0" onClick={() => setIsPaletteModalOpen(false)} />
            
            <div className="relative w-full max-w-3xl bg-[var(--color-surface)] border border-[var(--color-border)] rounded-3xl p-6 shadow-2xl animate-scale-up max-h-[85vh] flex flex-col overflow-hidden transition-colors duration-300">
              {/* Header */}
              <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-4 mb-4 shrink-0">
                <div className="flex items-center gap-2">
                  <span className="p-2 rounded-xl bg-indigo-500/10 text-indigo-400">
                  ...
                  <div>
                    <h3 className="font-extrabold text-sm text-[var(--color-text)]">Paletas por Nicho Comercial</h3>
                    <p className="text-[10px] text-[var(--color-text-muted)]">Explora y selecciona paletas optimizadas para las 23 verticales de negocio oficiales.</p>
                  </div>
                </div>
                <button 
              ...
              <div className="mb-4 shrink-0">
                <div className="flex items-center gap-2.5 bg-[var(--color-bg)] border border-[var(--color-border)] px-3.5 py-2 rounded-xl shadow-sm focus-within:border-indigo-500/50 transition-colors duration-300">
                  ...
                  <input 
                    type="text" 
                    placeholder="Buscar nicho comercial o nombre de paleta (ej: Ropa, Tornería, Calzado...)"
              ...
              {/* Listado de Acordeones con Categorías */}
              <div className="flex-1 overflow-y-auto pr-1 space-y-2.5 min-h-[300px]">
                {PALETTE_CATEGORIES
                  .filter(category => {
                    const matchesCategoryName = category.name.toLowerCase().includes(paletteSearchQuery.toLowerCase());
                    const matchesPalettes = category.palettes.some(preset => preset.name.toLowerCase().includes(paletteSearchQuery.toLowerCase()));
                    return matchesCategoryName || matchesPalettes;
                  })
                  .map((category) => {
                    const isSearching = paletteSearchQuery.trim() !== '';
                    const isOpen = isSearching || expandedPaletteCategory === category.id;
                    
                    const filteredPalettes = category.palettes.filter(preset => {
                      if (!isSearching) return true;
                      return preset.name.toLowerCase().includes(paletteSearchQuery.toLowerCase()) || 
                             category.name.toLowerCase().includes(paletteSearchQuery.toLowerCase());
                    });
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (!isSearching) {
                              setExpandedPaletteCategory(isOpen ? null : category.id);
                            }
                          }}
                          disabled={isSearching}
                          className={`w-full flex items-center justify-between px-4 py-3 bg-[var(--color-surface-2)]/30 transition-colors text-xs font-bold text-[var(--color-text)] select-none ${isSearching ? 'cursor-default' : 'cursor-pointer hover:bg-[var(--color-surface-2)]/60'}`}
                        >
                        ...
                        {isOpen && (
                          <div className="p-4 bg-[var(--color-surface)]/20 border-t border-[var(--color-border)] animate-scale-up origin-top">
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                              {filteredPalettes.map((preset, pIdx) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`p-2.5 rounded-xl border text-left transition-all duration-200 cursor-pointer ${
                                      isSelected 
                                        ? 'bg-indigo-600/20 border-indigo-500 shadow-md scale-[1.02] ring-1 ring-indigo-500' 
                                  ...
                                  >
                                    <div className="flex items-center gap-1.5 mb-1.5 justify-start">
                                      <div className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: preset.primary }} title="Primario" />
                                      <div className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: preset.secondary }} title="Secundario" />
                                      <div className="w-3 h-3 rounded-full border border-white/10 shadow-sm" style={{ backgroundColor: preset.bg }} title="Fondo" />
                                      <div className="w-3 h-3 rounded-full border border-black/10 shadow-sm" style={{ backgroundColor: preset.text }} title="Texto" />
                                    </div>
                                    <span className="text-[10px] font-bold block text-[var(--color-text)] truncate" title={preset.name}>{preset.name}</span>
                                  </button>
                                );
                              })}
                            </div>
  ...
  const SIDEBAR_GROUPS = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'comisiones', label: 'Comisiones Acumuladas', icon: TrendingUp },
  ...
  const NAV_TABS = [
    ...
    { id: 'comisiones', label: 'Comisiones Acumuladas', icon: TrendingUp, shortLabel: 'Comisiones' },
  ...
  return (
    <div className="h-screen bg-[var(--color-bg)] text-[var(--color-text)] font-sans overflow-hidden selection:bg-violet-500/30 selection:text-violet-200 transition-colors duration-300 flex flex-col md:cursor-none">
      ...
      {/* ── Fondo tecnológico premium global animado con Spotlight ── */}
      <BackgroundCanvas
        bgType={dashBgSettings.bgType}
        bgMouseTracking={dashBgSettings.bgMouseTracking}
        bgParticlesCount={dashBgSettings.bgParticlesCount}
        bgParticlesSpeed={dashBgSettings.bgParticlesSpeed}
        bgParticlesSize={dashBgSettings.bgParticlesSize}
        bgParticlesColor="primary"
        bgParticlesOpacity={dashBgSettings.bgParticlesOpacity}
        bgParticlesDirection={dashBgSettings.bgParticlesDirection}
        bgParticlesShape={dashBgSettings.bgParticlesShape}
        bgParticlesIcon="default"
        bgOrbsCount={dashBgSettings.bgOrbsCount}
        bgOrbsOpacity={dashBgSettings.bgOrbsOpacity}
        primaryColor={dashBgSettings.primaryColor}
        secondaryColor={dashBgSettings.secondaryColor}
        bgColor={dashBgSettings.bgColor}
        niche={niche}
        global={true}
        bgOrbsSpeed={dashBgSettings.bgOrbsSpeed}
        bgOrbsSize={dashBgSettings.bgOrbsSize}
        bgOrbsBlur={dashBgSettings.bgOrbsBlur}
      />
      <div aria-hidden="true" className="tech-bg-vignette" />
      ...
      <nav className="h-14 border-b border-[var(--color-border)] bg-[var(--color-surface)]/90 backdrop-blur-md pr-4 lg:pr-6 pl-0 flex items-center justify-between sticky top-0 z-50 shadow-sm transition-colors duration-300 shrink-0">
        <div className="flex items-center gap-0">
          ...
          <button
            ...
            className="lg:hidden w-8 h-8 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)]/50 hover:bg-[var(--color-surface-2)] flex items-center justify-center text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors cursor-pointer shrink-0 ml-3"
            title="Abrir menú"
          >
            <Menu size={15} />
          </button>
          ...
          <div 
            ...
            className={`flex items-center gap-3 pl-3 pr-4 transition-all duration-300 ease-out cursor-pointer hover:opacity-80 select-none ${
        ...
        <aside className={`hidden lg:flex flex-col shrink-0 border-r border-[var(--color-border)] bg-[var(--color-surface)]/80 backdrop-blur-md transition-all duration-300 ${
                    ...
                    onClick={() => {
                      if (!sidebarCollapsed) {
                        setExpandedGroups(prev => {
                          const wasExpanded = !!prev[group.id];
                          const newStates = {
                            clientes: false,
                            finanzas: false,
                            core: false,
                          ...
                          newStates[group.id] = !wasExpanded;
                          return newStates;
                        });
                      } else {
          ...
          {!sidebarCollapsed && (
            <div className="p-3 border-t border-[var(--color-border)] text-[9px] text-[var(--color-text-muted)] font-mono">
              v{new Date().getFullYear()} · MOTOR PROTOTIPE
            </div>
          )}
        </aside>
        ...
        <main className="flex-1 overflow-y-auto scrollbar-thin pb-24 lg:pb-8">
                  ...
                  {globalDriftLoading && globalDrift.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                      {globalDrift.map(drift => {
                        ...
                        const cardColor = drift.parityPercent >= 95 ? 'border-emerald-500/25 bg-emerald-500/[0.01] hover:border-emerald-500/40 hover:shadow-[0_8px_30px_rgba(16,185,129,0.03)]' :
                                          drift.parityPercent >= 80 ? 'border-amber-500/25 bg-amber-500/[0.01] hover:border-amber-500/40 hover:shadow-[0_8px_30px_rgba(245,158,11,0.03)]' :
                                          'border-red-500/25 bg-red-500/[0.01] hover:border-red-500/40 hover:shadow-[0_8px_30px_rgba(239,68,68,0.03)]';
                        const barColor = drift.parityPercent >= 95 ? 'bg-emerald-500' :
                                         drift.parityPercent >= 80 ? 'bg-amber-500' :
                                         'bg-red-500';

                        return (
                          <div key={drift.clientId} className={`p-6 md:p-7 rounded-2xl border ${cardColor} transition-all duration-300 hover:scale-[1.008] flex flex-col gap-4 relative overflow-hidden group bg-[var(--color-surface)] shadow-sm`}>
                            {/* Gradiente sutil de fondo en hover */}
                            <div className="absolute inset-0 bg-gradient-to-tr from-indigo-500/[0.01] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />

                            <div className="flex items-center justify-between gap-4 relative z-10">
                              <div className="flex items-center gap-3.5 min-w-0">
                                <div className={`w-11 h-11 rounded-xl flex items-center justify-center border shrink-0 transition-transform duration-300 group-hover:scale-105 ${
                                  drift.parityPercent >= 95 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                  drift.parityPercent >= 80 ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
                                  'bg-red-500/10 text-red-400 border-red-500/20'
                                }`}>
                                  <Store size={20} className="shrink-0" />
                                </div>
                                <div className="min-w-0">
                                  <h4 className="font-black text-sm md:text-base text-[var(--color-text)] tracking-tight truncate leading-snug">{client.name}</h4>
                                  <span className="inline-flex mt-0.5 text-[9px] text-[var(--color-text-muted)] font-mono uppercase tracking-wider bg-[var(--color-surface-2)] px-2 py-0.5 rounded border border-[var(--color-border)]/65">
                                    Core: {drift.coreId}
                                  </span>
                                </div>
                              </div>
                              <span className={`px-2.5 py-1 rounded-full text-[10px] font-black font-mono border tracking-wider shadow-sm shrink-0 ${
                                drift.parityPercent >= 95 ? 'bg-emerald-500/15 text-emerald-405 dark:text-emerald-400 border-emerald-500/25' :
                                drift.parityPercent >= 80 ? 'bg-amber-500/15 text-amber-500 border-amber-500/25' :
                                'bg-red-500/15 text-red-500 border-red-500/25'
                              }`}>
                                {drift.parityPercent}% Paridad
                              </span>
                            </div>

                            <div className="space-y-1.5 mt-1 relative z-10">
                              <div className="flex items-center justify-between text-[11px] font-bold text-[var(--color-text-muted)] tracking-wide">
                                <span>Progreso de Paridad</span>
                                <span className="font-mono">{drift.parityPercent} / 100</span>
                              </div>
                              <div className="h-2 bg-[var(--color-surface-2)] rounded-full overflow-hidden border border-[var(--color-border)]/35 shadow-inner">
                                <div className={`h-full rounded-full transition-all duration-700 ease-out ${barColor}`} style={{ width: `${drift.parityPercent}%` }} />
                              </div>
                            </div>

                            <div className="grid grid-cols-2 gap-3.5 mt-1 relative z-10">
                              <div className="p-3 bg-[var(--color-surface-2)]/50 rounded-xl border border-[var(--color-border)] flex items-center justify-between transition-all duration-300 hover:bg-[var(--color-surface-2)]/80 hover:border-amber-500/15">
                                <div className="space-y-0.5">
                                  <span className="text-[9px] font-black uppercase tracking-wider text-[var(--color-text-muted)] block">Modificados</span>
                                  <span className="text-[8px] text-[var(--color-text-muted)] block font-mono">Diferencias Core</span>
                                </div>
                                <span className="font-black text-lg text-amber-500 font-mono pr-1">{drift.modifiedCount}</span>
                              </div>
                              
                              <div className="p-3 bg-[var(--color-surface-2)]/50 rounded-xl border border-[var(--color-border)] flex items-center justify-between transition-all duration-300 hover:bg-[var(--color-surface-2)]/80 hover:border-red-500/15">
                                <div className="space-y-0.5">
                                  <span className="text-[9px] font-black uppercase tracking-wider text-[var(--color-text-muted)] block">Faltantes</span>
                                  <span className="text-[8px] text-[var(--color-text-muted)] block font-mono">Archivos Ausentes</span>
                                </div>
                                <span className="font-black text-lg text-red-500 font-mono pr-1">{drift.missingCount}</span>
                              </div>
                            </div>

                            <div className="flex items-center justify-between gap-3 border-t border-[var(--color-border)] pt-3.5 mt-2 relative z-10">
                              <div>
                                {drift.dependenciesOutOfSync ? (
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[8.5px] font-black uppercase tracking-wider bg-amber-550/10 text-amber-500 border border-amber-500/20" title="Dependencias NPM desalineadas en package.json o node_modules">
                                    <AlertCircle size={10} className="animate-pulse" />
                                    Deps Desviadas
                                  </span>
                                ) : (
                                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[8.5px] font-black uppercase tracking-wider bg-emerald-550/10 text-emerald-500 border border-emerald-500/20">
                                    <Check size={10} />
                                    Deps Alineadas
                                  </span>
                                )}
                              ...
                              <div className="flex gap-2">
                                {drift.parityPercent < 100 && (
                                  <button onClick={async () => {
                                    ...
                                    className="h-8 px-3 bg-indigo-600 hover:bg-indigo-550 active:scale-[0.97] text-white font-black rounded-xl text-[9px] uppercase tracking-wider transition-all duration-200 cursor-pointer border-none flex items-center gap-1 shadow-sm">
                                    <RefreshCw size={9} />
                                    Sincronizar
                                  </button>
                                )}
                                ...
                                <button onClick={() => {
                                  ...
                                  className="h-8 px-3 bg-[var(--color-surface-2)] hover:bg-[var(--color-border)] text-[var(--color-text-muted)] hover:text-[var(--color-text)] rounded-xl text-[9px] font-black uppercase tracking-wider border border-[var(--color-border)] transition-all duration-200 cursor-pointer flex items-center gap-1 active:scale-[0.97]">
                                  <Settings size={9} />
                                  Detalles
                                </button>
                              </div>
          ...
          {activeTab === 'comisiones' && (
            <ComisionesPanel
              ...
              showToast={showToast}
              onTogglePayment={handleTogglePayment}
              onGoToRecaudo={(clientId) => { setActiveTab('recaudo') }}
              projMetrics={{ projNewClients, projAvgSales, projRate, projMonths, projExistingMonthly, projTotalMonthly, projTotalYear }}
            />
          )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <CustomSelect
                          value={waClientId}
                          onChange={val => setWaClientId(val)}
                          options={[{ value: '', label: '-- Seleccionar --' }, ...clientesSaas.map(c => ({ value: c.id, label: c.id }))] }
                        />
                      </div>
                      <div className="space-y-1">
          ...
          {activeTab === 'skills' && (
            <SkillsRoadmapPanel 
              showToast={showToast} 
              dashBgSettings={dashBgSettings}
              updateDashBgSetting={updateDashBgSetting}
            />
          )}

          {/* ===== TAB: BRIEFING STUDIO ===== */}
          {activeTab === 'briefing' && (
            <BriefingStudioView dbInstance={getCentralApp() ? getFirestore(getCentralApp()) : null} showToast={showToast} onImportToOnboarding={handleImportBriefingData} />
          )}
          ...
          {activeTab === 'cotizador' && (
            <CotizadorView 
              ...
              showToast={showToast} 
              onImportToOnboarding={(data) => {
                if (data.projectName) setNewClientName(data.projectName);
                if (data.requirements) setCustomRequirements(data.requirements);
                if (data.branding) {
                  ...
                  if (data.branding.googleFont) setGoogleFont(data.branding.googleFont);
                }
                if (data.flags) {
                  if (data.flags.creditsEnabled !== undefined) setEnableBilling(!!data.flags.creditsEnabled);
                  if (data.flags.enableDianBilling !== undefined) setEnableDianBilling(!!data.flags.enableDianBilling);
                }
                if (data.setupFee !== undefined) setSetupFee(data.setupFee);
                if (data.mensualidad !== undefined) setPagoMensualFijo(data.mensualidad);
                if (data.comision !== undefined) setComisionPorcentaje(data.comision);

                // Establecer modelo de facturación
                if (data.comision > 0) {
                  setBillingMode('percentage');
                } else {
                  setBillingMode('flat_monthly');
          ...
          {/* ===== TAB: GESTIÓN DE NICHOS ===== */}
          {activeTab === 'niches' && (
            <NichesManagerPanel showToast={showToast} onNichesUpdated={loadNichesFromServer} cliUrl={CLI_URL} />
          )}
                        ...
                        <tbody className="divide-y divide-[var(--color-border)] text-xs">
                          {paginatedProvisionings.length === 0 ? (
                            ...
                            paginatedProvisionings.map(client => {
                              const foundNiche = niches.find(n => n.id === client.niche);
                              const nicheData = foundNiche 
                                ? { emoji: foundNiche.emoji, name: foundNiche.shortName }
                                : { emoji: "📦", name: client.niche || "Desconocido" };
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl px-2.5 h-9">
                      ...
                      <CustomSelect
                        ...
                        onChange={(val) => {
                          setSelectedErrorClientFilter(val)
                          setErrorsPage(1)
                        }}
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl px-2.5 h-9">
                      ...
                      <CustomSelect
                        ...
                        onChange={(val) => {
                          setSelectedErrorTypeFilter(val);
                          setErrorsPage(1);
                        }}
                        options={[
          ...
          {activeTab === 'cores' && (
            <CoreManagerPanel 
              ...
              allClientesControl={allClientesControl}
              failures={failures}
              onRequestTelemetry={handleRequestClientTelemetry}
              onManageClient={(clientId) => { setSelectedCrmClientId(clientId); setActiveMetricModal('clientes'); }}
              localServers={localServers}
              onStartLocalServer={handleStartLocalServer}
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          {/* Drawer content */}
          <aside className="relative flex flex-col w-[260px] h-full bg-[var(--color-surface)] border-r border-[var(--color-border)] shadow-2xl animate-slide-right overflow-y-auto pt-4 pb-safe select-none z-10">
            {/* Header / Brand */}
            <div className="flex items-center justify-between px-4 pb-4 border-b border-[var(--color-border)]">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl overflow-hidden bg-gradient-to-tr from-violet-500/20 to-cyan-500/20 border border-violet-500/30 flex items-center justify-center shrink-0">
                  <img src="/logo.png?v=3" className="w-5.5 h-5.5 object-contain rounded-full" alt="Logo" />
                </div>
                <span className="font-extrabold text-sm tracking-wide text-[var(--color-text)]">PROTOTIPE</span>
              </div>
              <button 
                onClick={() => setMobileMenuOpen(false)}
                className="w-7 h-7 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-2)]/50 hover:bg-[var(--color-surface-2)] flex items-center justify-center text-[var(--color-text-muted)] cursor-pointer"
              >
                <X size={14} />
              </button>
            </div>
            ...
            {/* Menu Groups */}
            <div className="flex flex-col gap-1.5 p-3 flex-1 overflow-y-auto scrollbar-thin">
              {SIDEBAR_GROUPS.map((group, groupIdx) => {
                const GroupIcon = group.icon;
                const isGroupActive = group.items.some(item => activeTab === item.id);
                const isExpanded = expandedGroups[group.id];
                ...
                if (group.id === 'inicio') {
                  ...
                  return (
                    <div key={item.id} className="relative w-full shrink-0">
                      <button
                        id={`mobile-sidebar-tab-${item.id}`}
                        onClick={() => {
                          setActiveTab(item.id);
                          setMobileMenuOpen(false);
                        }}
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer border w-full text-left relative sidebar-tab-item ${
                          isActive
                            ? 'sidebar-item-active text-violet-400 border-violet-500/30 sidebar-tab-active'
                            : 'text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-surface-2)]/50 border-transparent'
                        }`}
                      >
                        <span className="sidebar-indicator" />
                        <Icon size={15} className={isActive ? 'text-violet-400 sidebar-icon-glow' : 'text-slate-500'} />
                        <span className="truncate">{item.label}</span>
                        {isActive && <ChevronRight size={10} className="ml-auto text-violet-400/60" />}
                      </button>
                    </div>
                  );
                }
                ...
                return (
                  <div key={group.id} className="flex flex-col gap-0.5 relative shrink-0">
                    {groupIdx > 1 && <div className="sidebar-separator" />}
                    
                    <button
                      onClick={() => {
                        setExpandedGroups(prev => {
                          const wasExpanded = !!prev[group.id];
                          const newStates = { clientes: false, finanzas: false, core: false, monitoreo: false };
                          newStates[group.id] = !wasExpanded;
                          return newStates;
                        });
                      }}
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer border w-full text-left group/hdr ${
                        isGroupActive
                          ? 'text-violet-400 border-transparent'
                          : 'text-[var(--color-text-muted)] hover:text-[var(--color-text)] border-transparent'
                      }`}
                    >
                      <GroupIcon size={15} className={isGroupActive ? 'text-violet-400 sidebar-icon-glow' : 'text-slate-500'} />
                      <span className="truncate uppercase tracking-[0.12em] text-[8.5px] font-black text-slate-500 dark:text-slate-400/65">{group.label}</span>
                      {isExpanded ? (
                        <ChevronDown size={11} className="ml-auto opacity-45" />
                      ) : (
                        <ChevronRight size={11} className="ml-auto opacity-45" />
                      )}
                    </button>
                        ...
                        {group.items.map(item => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all duration-150 cursor-pointer w-full text-left relative sidebar-tab-item ${
                                isActive
                                  ? 'text-violet-400 font-extrabold sidebar-tab-active'
                                  : 'text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-surface-2)]/45'
                              }`}
                            >
                              <span className="sidebar-indicator" />
                              <Icon size={12} className={isActive ? 'text-violet-400 sidebar-icon-glow' : 'text-slate-500'} />
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-[var(--color-border)] bg-[var(--color-surface)]/95 backdrop-blur-xl pb-safe animate-slide-up">
        <div className="grid grid-cols-5 items-center justify-items-center px-1 py-1.5">
          {NAV_TABS.filter(tab => ['dashboard', 'crm', 'onboarding', 'library'].includes(tab.id)).map(tab => {
            const Icon = tab.icon
          ...
          {/* Menú Button (5º Elemento para abrir Sidebar móvil) */}
          <button
            type="button"
            onClick={() => setMobileMenuOpen(true)}
            className="flex flex-col items-center gap-0.5 py-2 rounded-xl transition-all duration-200 cursor-pointer w-full text-[var(--color-text-muted)] hover:text-indigo-400"
          >
            <div className="relative p-1.5 rounded-lg transition-all duration-200">
              <Menu size={18} />
            </div>
            <span className="text-[9px] font-bold tracking-wide">Menú</span>
          </button>
        </div>
      </nav>
