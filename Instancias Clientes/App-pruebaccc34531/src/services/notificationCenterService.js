...
/**
 * Se suscribe en tiempo real SOLO al lote más reciente de notificaciones.
 * Permite detectar nuevas notificaciones para sonidos/badge sin descargar
 ...
 */
export function subscribeToCentralNotifications(recipientId, recipientRole, onUpdate, pageSize = 15) {
...
/**
 * Carga una página de notificaciones históricas bajo demanda (one-shot, sin listener).
 * Usar para paginación infinita al hacer scroll en el tray.
 *
 * @param {string} recipientId
 * @param {string} recipientRole
 * @param {number} pageSize      - Cuántos documentos cargar en este lote
 * @param {DocumentSnapshot|null} lastDoc - Cursor del último doc de la página anterior
 * @returns {Promise<{ items: Array, lastDoc: DocumentSnapshot|null, hasMore: boolean }>}
 */
export async function fetchNotificationsPage(recipientId, recipientRole, pageSize = 15, lastDoc = null) {
  if (!recipientId && !recipientRole) return { items: [], lastDoc: null, hasMore: false }

  const isClient = recipientRole === 'client'

  try {
    const constraints = isClient && recipientId
      ? [
          where('recipientId', '==', recipientId),
          where('recipientRole', '==', 'client'),
          orderBy('createdAt', 'desc'),
    ...
    const docsToReturn = hasMore ? rawDocs.slice(0, pageSize) : rawDocs

    const items = docsToReturn
      .map(d => ({ id: d.id, _doc: d, ...d.data() }))
      .filter(n => n.status !== 'archived')
...
/** Archiva una notificación (la elimina totalmente de la base de datos) */
export async function archiveNotification(notificationId) {
  if (!notificationId) return
  try {
    await deleteDoc(doc(db, COL, notificationId))
  } catch (error) {
    console.error('[NC] Error al eliminar notificación:', error)
  }
}

/** Archiva todas las notificaciones de un recipiente (las elimina totalmente de la base de datos) */
export async function archiveAll(recipientId, recipientRole) {
  ...
  try {
    ...
    console.error('[NC] Error al vaciar notificaciones:', error)
  }
}
