            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Sello Flotante de Descuento ("Sticker" Glassmorphism Premium en la Esquina Superior Derecha) */}
                  {ad.type === 'inventory' && linkedProduct && (
                    <div 
                      className="absolute top-4 right-4 z-30 text-white px-3 py-2 rounded-2xl flex flex-col items-center justify-center shadow-lg border border-white/20 backdrop-blur-md hover:scale-105 transition-all duration-300 select-none max-w-[85px] text-center"
                      style={{ 
                        background: 'linear-gradient(135deg, color-mix(in srgb, var(--color-accent) 40%, rgba(0, 0, 0, 0.15)), color-mix(in srgb, var(--color-primary) 20%, rgba(0, 0, 0, 0.25)))',
                        boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.25)'
                      }}
                    >
                      <span className="leading-none text-[8.5px] uppercase tracking-wider opacity-85 font-extrabold text-neutral-200">Ahorras</span>
                      <span className="leading-none mt-1 text-[13px] font-black tracking-tight drop-shadow-sm font-mono text-white">
                        {ad.discountType === 'percentage' 
                          ? `${ad.discountValue}%` 
                          : `-${formatCurrency(ad.discountValue)}`}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="relative z-30 px-6 sm:px-10 md:px-12 py-4 sm:py-5 flex flex-col justify-center gap-y-3.5 h-full text-left max-w-sm sm:max-w-md md:max-w-xl">
                    ...
                    <div className="flex items-center gap-2.5">
                      <button
                        ...
                        className="px-5 h-9 bg-white text-black font-extrabold text-[10px] uppercase tracking-wider rounded-xl active:scale-95 transition-all duration-300 hover:bg-neutral-100 shadow-md border-none cursor-pointer flex items-center justify-center"
                      >
