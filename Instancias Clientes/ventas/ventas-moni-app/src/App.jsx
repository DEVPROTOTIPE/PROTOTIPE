...
import { syncOfflineSales } from './features/sales'
...
function ThemeApplier() {
  ...
  const contrastTextClass = useColorContrast('--color-primary')
  ...
  useLayoutEffect(() => {
    ...
    const finalActionColor = actionColor || activeColors['--color-primary'] || 'var(--color-primary)'
    root.style.setProperty('--color-action', finalActionColor)
...
const RemoteAlertModal = ({ sistemaAlerta, isAlertDismissed, handleDismissAlert }) => {
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 999999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }}>
          ...
          >
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border ${
              ...
              {sistemaAlerta.type === 'error' || sistemaAlerta.type === 'warning' ? (
...
const TelemetryReportModal = ({ telemetryReportedPeriod, setTelemetryReportedPeriod }) => {
  const location = useLocation()
  const isAdminPath = location.pathname.startsWith('/admin')
...
const PingRequestModal = ({ activePingRequest, setActivePingRequest }) => {
  const location = useLocation()
  const isAdminPath = location.pathname.startsWith('/admin')
...
export default function App() {
  ...
  const [dismissedAlertKey, setDismissedAlertKey] = useState(() => {
    try {
      return localStorage.getItem('dismissed_remote_alert') || ''
    } catch (e) {
      return ''
    }
  })
  ...
  const handleDismissAlert = () => {
    if (sistemaAlerta) {
      const key = getAlertDismissKey(sistemaAlerta)
      try {
        localStorage.setItem('dismissed_remote_alert', key)
      } catch (e) {}
  ...
  const isAlertDismissed = sistemaAlerta ? dismissedAlertKey === getAlertDismissKey(sistemaAlerta) : false
  ...
  if (isFirstLoad) {
    return (
      <div 
        className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[var(--color-surface)]"
        style={{
  ...
  if (deactivated) {
    return (
      ...
      >
        <div className="relative overflow-hidden bg-[var(--color-surface-2)]/40 backdrop-blur-xl border border-red-500/20 rounded-3xl p-8 sm:p-12 max-w-lg w-full text-center shadow-2xl flex flex-col items-center gap-6">
          <div className="w-20 h-20 rounded-2xl bg-red-500/10 flex items-center justify-center text-red-500 border border-red-500/20 animate-pulse">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm font-semibold leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
              {deactivationReason || 'Esta aplicación se encuentra inactiva temporalmente por motivos administrativos.'}
            </p>
          </div>
          <div className="w-full h-px bg-[var(--color-border)]" />
          <p className="text-[10px] font-bold tracking-wider uppercase text-red-500/80">Código de Restricción: ACC-LOCKOUT</p>
        </div>
      </div>
    )
  }
      ...
      >
        <div className="relative overflow-hidden bg-[var(--color-surface-2)]/40 backdrop-blur-xl border border-[var(--color-border)] rounded-3xl p-8 sm:p-12 max-w-lg w-full text-center shadow-2xl flex flex-col items-center gap-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full justify-center">
            <button
              onClick={() => window.location.reload()}
              className={`py-3 px-6 h-11 bg-[var(--color-primary)] hover:opacity-90 ${contrastTextClass} text-sm font-black uppercase tracking-wider rounded-2xl active:scale-95 transition-all cursor-pointer shadow-md border-none flex items-center justify-center gap-2`}
            >
        ...
        <MotionConfig reducedMotion={animationsEnabled ? "user" : "always"}>
          <AlertConfirmProvider>
            <BrowserRouter>
              ...
              <TelemetryReportModal
                telemetryReportedPeriod={telemetryReportedPeriod}
                setTelemetryReportedPeriod={setTelemetryReportedPeriod}
              />

              {/* Modal de Alerta Remota / Bloqueo (Spam de Pago) */}
              <RemoteAlertModal
                sistemaAlerta={sistemaAlerta}
              ...
              <PingRequestModal
                activePingRequest={activePingRequest}
                setActivePingRequest={setActivePingRequest}
              />
