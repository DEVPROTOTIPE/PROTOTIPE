...
export default function ClientView({ tenantId: propTenantId, customerId: propCustomerId }) {
  const { user } = useAuthStore();
  const tenantId = propTenantId || user?.tenantId || 'demo';
  const customerId = propCustomerId || user?.uid || user?.id || '';
  ...
  useEffect(() => {
    if (qrToken) {
      QRCode.toDataURL(`LOYALTY:${qrToken}`, {
        margin: 1,
        width: 256,
        color: {
          dark: '#0f172a',
        ...
        .catch(err => console.error('[ClientView] Error generando QR:', err));
    } else {
      setQrDataUrl('');
    }
  }, [qrToken]);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6 lg:space-y-0 lg:grid lg:grid-cols-12 lg:gap-8">
      {error && (
        <div className="lg:col-span-12 p-3.5 bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl text-sm">
          {error}
        </div>
      )}

      {/* Columna Izquierda: Tarjetas (Club de Puntos + QR) */}
      <div className="lg:col-span-5 space-y-6 flex flex-col justify-start">
        {/* Tarjeta de Fidelización Premium */}
        <div className={`relative p-6 rounded-3xl bg-gradient-to-br ${getLevelColor(account?.level)} shadow-xl overflow-hidden min-h-[160px] flex flex-col justify-between`}>
        ...
        <div className="p-6 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-3xl shadow-sm flex flex-col items-center text-center space-y-4">
          ...
          <div className="p-4 bg-white rounded-2xl shadow-inner relative flex items-center justify-center">
            {qrDataUrl ? (
              <img src={qrDataUrl} alt="Código QR de fidelización" className="w-[160px] h-[160px] object-contain" />
            ) : (
      ...
      <div className="lg:col-span-7 space-y-3 flex flex-col h-full">
        ...
        {transactions.length === 0 ? (
          <div className="p-6 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-3xl text-center flex-1 flex items-center justify-center min-h-[200px]">
            <span className="text-xs text-[var(--color-text-muted)]">No tienes transacciones registradas</span>
          </div>
        ) : (
          <div className="p-5 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-3xl shadow-sm space-y-3 flex-1 flex flex-col">
            <div className="space-y-2 max-h-[300px] lg:max-h-[460px] overflow-y-auto pr-1 scrollbar-thin flex-1">
              {transactions.map((tx) => (
                <div 
                  key={tx.transactionId} 
                  className="flex items-center justify-between p-3.5 bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-2xl text-sm"
                >
