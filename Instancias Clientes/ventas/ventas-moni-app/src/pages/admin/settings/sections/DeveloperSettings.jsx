...
import { 
  ...
  Save, Loader2, CheckCircle, Activity, Paintbrush, LayoutGrid, Truck, Database
} from 'lucide-react'
...
export default function DeveloperSettings({ 
  ...
  if (!isDevAuthenticated) {
    return (
      ...
      >
        ...
        <form onSubmit={(e) => {
          e.preventDefault()
          if (DEV_PIN !== null && devPinInput === DEV_PIN) {
  ...
  return (
    <div className="text-left">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            {[
              ...
              {
                id: 'dev-migracion',
                label: 'Migración de Base de Datos',
                description: 'Migra pedidos existentes a la nueva estructura de tracking público segura (Fase 1)',
                icon: Database,
                iconBg: 'bg-indigo-500/10 hover:bg-indigo-500/15',
                iconColor: 'text-indigo-500'
              },
      ...
      {/* 3.5. Subsección: Migración de Pedidos */}
      {activeSubSection === 'dev-migracion' && (
        <div className="bg-surface rounded-3xl border border-indigo-500/20 shadow-sm overflow-hidden text-left">
          <div className="p-5 sm:p-6 bg-indigo-500/5 border-b border-app">
            <h3 className="text-base font-black text-app flex items-center gap-2">
              <Database size={18} className="text-indigo-500" /> Migración de Pedidos a Tracking Seguro
            </h3>
            <p className="text-xs text-muted mt-1 leading-relaxed">
              Crea documentos en <code>order_tracking</code> y el índice privado <code>user_order_index</code> para todos los pedidos históricos existentes. Esto evita romper el historial y seguimiento de pedidos anteriores al hardening de seguridad.
            </p>
          </div>
          <div className="p-5 sm:p-6">
            {message && (
            ...
            <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-2xl p-5 mb-6">
              <p className="text-sm text-app/80 mb-4 leading-relaxed">
                Este proceso es seguro, idempotente (no duplicará registros si ya se migraron) y no interrumpe el servicio activo. Se procesa en lotes de hasta 400 documentos para cumplir con los límites de escritura transaccional de Firestore.
              </p>
              
              <button
                id="btnExecuteMigration"
                onClick={async () => {
                  setLoading(true)
                  setMessage({ type: 'info', text: 'Iniciando migración de pedidos històricos...' })
                  try {
                    const { migrateOrdersToTracking } = await import('../../../../features/orders')
                    const res = await migrateOrdersToTracking()
                    setMessage({
                      type: 'success',
                      text: `Migración completada. Nuevos documentos de tracking indexados: ${res.migrated}. Pedidos omitidos (ya migrados o sin token): ${res.skipped}. Errores: ${res.errors}.`
                    })
                  } catch (err) {
                    setMessage({ type: 'error', text: `Ocurrió un error: ${err.message}` })
                  } finally {
                    setLoading(false)
                  }
                }}
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 py-3 px-4 min-h-12 bg-primary text-white rounded-xl font-bold text-sm hover:opacity-90 active:scale-95 transition-all disabled:opacity-30 text-center border-none cursor-pointer"
              >
                {loading ? <Loader2 size={16} className="shrink-0 animate-spin" /> : <Database size={16} className="shrink-0" />}
                <span>Iniciar Migración a Tracking Seguro</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {activeSubSection === 'dev-modulos' && (
        <div className="bg-surface rounded-3xl border border-app overflow-hidden">
          ...
          <div className="p-5 sm:p-6 space-y-5">
            ...
            {/* Switch Pasarela de Pagos en Línea */}
            <div className="p-4 bg-surface-2 rounded-2xl border border-app space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-bold text-app">Pasarela de Pagos en Línea</p>
                  <p className="text-xs text-muted mt-0.5">Permite a los clientes pagar de forma segura en el checkout con tarjetas de crédito, PSE o QR local.</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
                  <input type="checkbox" className="sr-only peer"
                    checked={formData.deliverySettings?.onlinePayment?.enabled || false}
                    onChange={(e) => setFormData({ 
                      ...formData, 
                      deliverySettings: { 
                        ...(formData.deliverySettings || {}),
                        onlinePayment: { 
                          ...(formData.deliverySettings?.onlinePayment || {}), 
                          enabled: e.target.checked 
                        } 
                      } 
                    })} />
              ...
              {formData.deliverySettings?.onlinePayment?.enabled && (
                <div className="pt-2 border-t border-app/50 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-wider text-muted">Procesador de Pago</label>
                    <select
                      value={formData.deliverySettings?.onlinePayment?.processor || 'bold'}
                      onChange={(e) => setFormData({
                        ...formData,
                        deliverySettings: {
                          ...(formData.deliverySettings || {}),
                      ...
                      className="w-full h-10 px-3 rounded-xl bg-surface-3 border border-app text-xs text-app focus:outline-none focus:border-primary"
                    >
                      <option value="bold">Bold.co (Colombia)</option>
