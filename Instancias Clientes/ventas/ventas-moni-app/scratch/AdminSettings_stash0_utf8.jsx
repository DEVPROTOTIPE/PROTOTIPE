...
// ÔöÇÔöÇÔöÇ DATOS FICTICIOS (SEEDS) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
const SEED_CATEGORIES = [
  { id: 'cat-camisetas', nombre: 'Camisetas', descripcion: 'Camisetas de algod├│n' },
...
const SEED_PRODUCTS = [
  {
    id: 'prod-1',
    nombre: 'Camiseta B├ísica Negra',
    descripcion: 'Camiseta de algod├│n peinado 100%. Ajuste perfecto.',
    categoria: 'Camisetas',
    precioBase: 45000,
    costoBase: 20000,
    genero: 'unisex',
  ...
  },
  {
    ...
    descripcion: 'Pantal├│n con m├║ltiples bolsillos, ideal para el d├¡a a d├¡a.',
  ...
  {
    id: 'prod-3',
    nombre: 'Gorra Cl├ísica Vintage',
    descripcion: 'Gorra con dise├▒o retro y correa ajustable.',
    ...
    variantes: [
      { id: 'v5', talla: '├Ünica', color: 'Azul Marino', stock: 20, sku: 'CP-BLU-U' }
    ]
  }
]

const SEED_USERS = [
  { id: '3001234567', celular: '3001234567', nombre: 'Juan P├®rez' },
  { id: '3109876543', celular: '3109876543', nombre: 'Mar├¡a G├│mez' }
]
...
const SEED_ORDERS = [
  {
    ...
    cliente: { celular: '3001234567', nombre: 'Juan P├®rez' },
    items: [
      { productId: 'prod-1', variantId: 'v1', nombre: 'Camiseta B├ísica Negra', talla: 'M', color: 'Negro', cantidad: 2, precio: 45000 }
    ],
    total: 90000,
  ...
  },
  {
    ...
    cliente: { celular: '3109876543', nombre: 'Mar├¡a G├│mez' },
    items: [
      { productId: 'prod-2', variantId: 'v4', nombre: 'Pantal├│n Cargo Urbano', talla: '34', color: 'Verde Oliva', cantidad: 1, precio: 120000 },
      { productId: 'prod-3', variantId: 'v5', nombre: 'Gorra Cl├ísica Vintage', talla: '├Ünica', color: 'Azul Marino', cantidad: 1, precio: 35000 }
    ],
    total: 155000,
...
// ÔöÇÔöÇÔöÇ CUSTOM DATE PICKER COMPONENT ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// ÔöÇÔöÇÔöÇ CUSTOM SELECT COMPONENT ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// Componente helper para bloquear el scroll en el body cuando un modal est├í abierto
...
export default function AdminSettings() {
  ...
  // Navegaci├│n de secciones (null = men├║ principal)
  ...
  // Reporte autom├ítico de telemetr├¡a de facturaci├│n al panel central del desarrollador
  useEffect(() => {
  ...
  // ÔöÇÔöÇÔöÇ FIRMA Y FACTURACI├ôN DESARROLLADOR ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // Escuchar evento personalizado de la barra de navegaci├│n para regresar al men├║ de configuraci├│n principal
  useEffect(() => {
    const handleReset = () => {
  ...
  // States para Configuraci├│n del Negocio
  ...
  // ÔöÇÔöÇÔöÇ MODAL CONFIRMACI├ôN CR├ìTICA (Fase 5) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇÔöÇ PUBLICIDAD Y ANUNCIOS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  const handleSaveAd = () => {
    ...
    // Validar m├íximo 5 anuncios activos
    if (adFormData.active) {
      const activeCount = ads.filter(a => a.active && a.id !== editingAdId).length
      if (activeCount >= 5) {
        alert('L├¡mite alcanzado: S├│lo puedes tener un m├íximo de 5 publicidades activas de forma simult├ínea. Desactiva otra publicidad para poder activar esta.')
        return
      }
    }
    ...
    if (adFormData.type === 'inventory') {
      ...
      payload.ctaText = adFormData.ctaText || 'Ver promoci├│n'
  ...
  // ÔöÇÔöÇÔöÇ GESTI├ôN DE CUPONES DE DESCUENTO ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // Mapa de usos de cupones: { 'CODIGO': n├║mero_de_pedidos }
  ...
  const handleSaveCoupon = () => {
    if (!couponFormData.code) {
      alert('Ingresa el c├│digo del cup├│n')
      return
    }
    if (!couponFormData.value || Number(couponFormData.value) <= 0) {
      alert('Ingresa un valor de descuento v├ílido mayor a 0')
      return
    }
    ...
    if (config.isLoaded) {
      setFormData({
        ...
        deliverySettings: config.deliverySettings || {
          ...
          digital: { enabled: false, instructions: 'Entrega digital o prestaci├│n de servicio presencial.' }
        },
  ...
  useEffect(() => {
    // Si formData.theme a├║n no est├í listo, no hacer nada
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      // Guardar filtros de cat├ílogo en su documento independiente
      if (formData.catalogFilters) {
        await updateCatalogFilters(formData.catalogFilters)
      }

      config.setConfig(formData)
      setSaveMessage({ type: 'success', text: 'Configuraciones guardadas y aplicadas a toda la aplicaci├│n.' })
      setTimeout(() => setSaveMessage(null), 4000)
    } catch (error) {
      console.error(error)
      setSaveMessage({ type: 'error', text: 'Ocurri├│ un error al guardar las configuraciones.' })
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const handleUpdateCredentials = async (e) => {
    e.preventDefault()
    if (!currentPassword) {
      setAuthMessage({ type: 'error', text: 'Debes ingresar tu contrase├▒a actual por seguridad.' })
      return
    }
    if (!newEmail && !newPassword) {
      setAuthMessage({ type: 'error', text: 'Ingresa un nuevo correo o contrase├▒a para actualizar.' })
      return
    }
    ...
    try {
      ...
      if (err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
        setAuthMessage({ type: 'error', text: 'La contrase├▒a actual es incorrecta.' })
      } else if (err.code === 'auth/email-already-in-use') {
        setAuthMessage({ type: 'error', text: 'El nuevo correo ya est├í registrado.' })
      } else if (err.code === 'auth/invalid-email') {
        setAuthMessage({ type: 'error', text: 'El formato del nuevo correo es inv├ílido.' })
      } else {
  ...
  // Developer Zone handlers - Restauraci├│n real de la aplicaci├│n a cero
  const handleFullReset = async () => {
    if (confirmRestoreText !== 'RESTAURAR') return
    if (!window.confirm('┬┐Est├ís COMPLETAMENTE SEGURO de restaurar la aplicaci├│n? Se eliminar├ín de forma REAL y permanente todos los productos, categor├¡as, pedidos, cr├®ditos, cupones, anuncios y usuarios (excepto tu cuenta de administrador actual). Esta acci├│n no se puede deshacer.')) return
    ...
    try {
      ...
      // 1. Borrar colecciones est├índar
      ...
      userSnapshot.docs.forEach((userDoc) => {
        ...
        // Mantener al usuario administrador actual (usualmente marcado con role: 'admin' o con su email espec├¡fico)
      ...
      setMessage({ type: 'success', text: `┬íRestauraci├│n exitosa! Se eliminaron un total de ${deletedCount} registros de negocio de forma permanente. La aplicaci├│n est├í lista en cero.` })
    } catch (error) {
      console.error(error)
      setMessage({ type: 'error', text: 'Error al restaurar la aplicaci├│n: ' + error.message })
    } finally {
      setLoading(false)
    }
  }

  // ÔöÇÔöÇÔöÇ Definici├│n de secciones del men├║ ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  const MENU_SECTIONS = [
    {
      id: 'cupones',
      label: 'Cupones de Descuento',
      description: 'Genera c├│digos promocionales y ofertas',
      icon: Tag,
    ...
    },
    {
      ...
      description: 'Gestiona banners y promociones h├¡bridas',
      icon: Megaphone,
    ...
    },
    {
      ...
      description: 'Configura empleados, vendedores y el registro de ventas en el POS',
      icon: User,
    ...
    {
      id: 'temporada',
      label: 'Eventos por Temporada',
      description: 'Activa temas visuales especiales (Navidad, Halloween, D├¡a de la Madre/Padre)',
      icon: Calendar,
    ...
    },
    {
      ...
      description: 'Cambiar correo o contrase├▒a',
      icon: Lock,
    ...
    },
    {
      ...
      description: 'Facturaci├│n, restauraci├│n y filtros avanzados del cat├ílogo',
      icon: Settings,
    ...
    {
      id: 'laboratorio',
      label: 'Laboratorio de Innovaci├│n',
      description: 'Prueba funciones experimentales, IA y m├│dulos industriales',
      icon: Sparkles,
      iconBg: 'bg-purple-500/10',
      iconColor: 'text-purple-500',
    },
  ...
  // ÔöÇÔöÇÔöÇ Control de Acceso del Desarrollador (Gate) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
      ...
      >
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-muted mt-1 leading-relaxed">
            Ingresa la contrase├▒a maestra del desarrollador para acceder a la facturaci├│n y herramientas internas.
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-bold text-app mb-1.5 block">Contrase├▒a Maestra</label>
            <input
              type="password"
            ...
            />
            {devPasswordError && (
              <p className="text-[11px] text-red-500 font-semibold mt-1">La contrase├▒a es incorrecta.</p>
            )}
          </div>
          ...
          >
            <KeyRound size={16} /> Desbloquear Secci├│n
          </button>
        </form>
      </motion.div>
    )
  ...
  // ÔöÇÔöÇÔöÇ Previsualizador de Dispositivo M├│vil en Tiempo Real ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  const renderMobilePreview = () => {
    ...
    return (
      ...
      >
        {/* Enlace para cargar la tipograf├¡a din├ímica de Google Fonts para la vista previa */}
        ...
        <div className="w-[270px] h-[480px] rounded-[40px] border-[8px] border-slate-800 bg-app shadow-2xl relative overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-app select-none">
            ...
            <div className="h-24 rounded-xl bg-gradient-to-r from-primary/20 to-primary-focus/10 border border-primary/10 p-3 flex flex-col justify-center relative overflow-hidden">
              ...
              <p className="text-[10px] font-bold text-primary uppercase tracking-wide">Colecci├│n Nueva</p>
              <p className="text-xs font-black text-app mt-0.5 leading-tight">┬íEnv├¡os gratis en compras hoy!</p>
            </div>
          ...
          {/* 3. Barra Navegaci├│n Inferior Simulada (Con el bot├│n de ofertas del cliente animado) */}
          <div className="h-12 bg-surface border-t border-app flex items-center justify-around px-2 z-40 shrink-0 select-none pb-1">
            <div className="flex-1 flex flex-col items-center justify-center opacity-40">
              ...
              <span className="text-[8px] font-medium mt-0.5 scale-90">Cat├ílogo</span>
            </div>
            <div className="flex-1 flex flex-col items-center justify-center opacity-40">
            ...
            <div className="flex-1 flex flex-col items-center justify-center opacity-40">
              ...
              <span className="text-[8px] font-medium mt-0.5 scale-90">Cr├®ditos</span>
            </div>
          </div>
        </div>
      </div>
  ...
  // ÔöÇÔöÇÔöÇ Header compartido (Men├║ o Secci├│n) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  return (
    ...
    >
      {/* Toast de guardado renderizado en Portal para evitar brincos de animaci├│n */}
      ...
      {/* ÔöÇÔöÇÔöÇ HEADER ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
          ...
          <p className="text-sm text-muted">
            {activeSection 
              ? (activeSubSection === 'empleados' 
                  ...
                  : activeSubSection === 'entregas' 
                    ? 'Opciones de entrega y retiro f├¡sico o digital' 
                    : activeSubSection === 'temporada'
                      ? 'Activa decoraciones y colores especiales'
                      : activeSubSection === 'mayorista'
                        ? 'Configura valores m├¡nimos y cotizaciones de mayoreo'
                        : activeSubSection === 'garantias'
                          ? 'Administra pol├¡ticas de cambios e inicios de reclamo'
                          : activeSubSection === 'seguimiento'
                            ? 'Permite a los clientes ver el tracking en tiempo real'
      ...
      {/* ÔöÇÔöÇÔöÇ VISTA: MEN├Ü PRINCIPAL ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {!activeSection && (
        <div className="flex flex-col gap-4">

          {/* ÔöÇÔöÇ NIVEL 1: MI NEGOCIO ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
          <div className="bg-surface rounded-3xl shadow-sm overflow-hidden border border-app">
            <div className="bg-primary/5 px-5 py-3 border-b border-app">
              <p className="text-xs font-black text-primary uppercase tracking-widest">Nivel 1: Mi Negocio</p>
              <p className="text-[10px] text-muted mt-0.5">Configuraciones b├ísicas de identidad, contacto, apariencia y PWA.</p>
            </div>
            {['marca', 'ventas', 'apariencia', 'pwa'].map((id) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center gap-4 px-5 py-4 hover:bg-surface-2 active:bg-primary/5 border-b border-app last:border-0 transition-colors text-left"
                >
          ...
          {/* ÔöÇÔöÇ NIVEL 2: HERRAMIENTAS OPERATIVAS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
          <div className="bg-surface rounded-3xl shadow-sm overflow-hidden border border-app">
            <div className="bg-emerald-500/5 px-5 py-3 border-b border-app">
              <p className="text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Nivel 2: Herramientas Administrativas</p>
              <p className="text-[10px] text-muted mt-0.5">Gesti├│n operativa comercial, promociones, personal y eventos estacionales.</p>
            </div>
            {['cupones', 'publicidad', 'empleados', 'temporada'].map((id) => {
              const section = MENU_SECTIONS.find(s => s.id === id)
              if (!section) return null
              const Icon = section.icon
              return (
          ...
          {/* ÔöÇÔöÇ NIVEL 3: PANEL MAESTRO ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
          <div className="bg-surface rounded-3xl shadow-sm overflow-hidden border border-app">
            <div className="bg-red-500/5 px-5 py-3 border-b border-app">
              <p className="text-xs font-black text-red-500 uppercase tracking-widest">Nivel 3: Panel Maestro</p>
              <p className="text-[10px] text-muted mt-0.5">Zona de desarrollador: arquitectura, facturaci├│n y telemetr├¡a avanzada.</p>
            </div>
            {['developer'].map((id) => {
              const section = MENU_SECTIONS.find(s => s.id === id)
              if (!section) return null
          ...
          {/* ÔöÇÔöÇ NIVEL 4: LABORATORIO ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
          <div className="bg-surface rounded-3xl shadow-sm overflow-hidden border border-app">
            <div className="bg-purple-500/5 px-5 py-3 border-b border-app">
              <p className="text-xs font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest">Nivel 4: Laboratorio</p>
              <p className="text-[10px] text-muted mt-0.5">Espacio de innovaci├│n: accesos r├ípidos a portales especializados e industrias.</p>
            </div>
            {['laboratorio'].map((id) => {
              const section = MENU_SECTIONS.find(s => s.id === id)
              if (!section) return null
          ...
          {/* ÔöÇÔöÇ NIVEL 5: SISTEMA ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
          <div className="bg-surface rounded-3xl shadow-sm overflow-hidden border border-app">
            <div className="bg-slate-500/5 px-5 py-3 border-b border-app">
              <p className="text-xs font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">Nivel 5: Sistema</p>
              <p className="text-[10px] text-muted mt-0.5">Configuraciones t├®cnicas internas de la cuenta y accesos de seguridad.</p>
            </div>
            {['seguridad'].map((id) => {
              const section = MENU_SECTIONS.find(s => s.id === id)
              if (!section) return null
      ...
      {/* Bot├│n Cerrar Sesi├│n (solo en men├║ principal) */}
      {!activeSection && (
      ...
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {/* ÔöÇÔöÇÔöÇ VISTA: IDENTIDAD DE MARCA ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {activeSection === 'marca' && (
            ...
            <div className="p-5 sm:p-6 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ej. Lencer├¡a y Accesorios para Ti"
                  className="w-full h-12 px-4 rounded-xl bg-surface-2 border border-app text-app focus:outline-none focus:border-primary transition-colors"
                />
              </div>
              <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app md:col-span-2">
                <div>
                  ...
                  <p className="text-xs text-muted mt-0.5">Efecto sonar animado detr├ís de tu logotipo en pantallas de bienvenida</p>
                </div>
              ...
              {/* ÔöÇÔöÇÔöÇ CONFIGURACI├ôN DE APLICACI├ôN M├ôVIL (PWA) ÔöÇÔöÇÔöÇ */}
              <div className="border-t border-app pt-5 md:col-span-2 space-y-4">
                <h3 className="text-sm font-black text-primary tracking-wider uppercase">
                  Configuraci├│n de Aplicaci├│n M├│vil (PWA)
                </h3>
                <p className="text-xs text-muted leading-relaxed">
                  Define el nombre y el icono con el que se instalar├í la aplicaci├│n en la pantalla de inicio del celular de tus clientes.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app">
                      <div>
                        <p className="text-sm font-bold text-app">Usar logo de la tienda como ├¡cono de la app</p>
                        <p className="text-xs text-muted mt-0.5">Se colocar├í el logo de tu tienda centrado sobre un fondo con el color principal de la marca</p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
                        <input type="checkbox" className="sr-only peer"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-app mb-2 uppercase tracking-wider">Nombre de la App M├│vil</label>
                    <input
                      type="text"
                  ...
                  <div>
                    {!formData.pwaUseBrandIcon ? (
                      <>
                        <label className="block text-xs font-bold text-app mb-2 uppercase tracking-wider">URL del Icono de Instalaci├│n (PWA)</label>
                        <div className="flex gap-3 items-center">
                          <div className="relative flex-1">
                      ...
                      <>
                        ...
                        <div className="flex gap-4 items-center p-3 bg-surface-2 rounded-xl border border-app">
                          ...
                          <div>
                            <p className="text-xs font-bold text-app">{formData.pwaAppName || formData.appName || 'Mi Aplicaci├│n'}</p>
                            <p className="text-[10px] text-muted">Generado din├ímicamente</p>
                          </div>
                        </div>
                      </>
                    )}
      ...
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {/* ÔöÇÔöÇÔöÇ VISTA: APARIENCIA Y COLORES ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {activeSection === 'apariencia' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="bg-surface rounded-3xl shadow-sm overflow-hidden lg:col-span-7 flex flex-col">
            <div className="p-5 sm:p-6 flex flex-col gap-5">
              {/* ÔöÇÔöÇ Modo Oscuro ÔöÇÔöÇ */}
              <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app">
                <div>
              ...
              {/* ÔöÇÔöÇ Sistema de Compra Guiada ÔöÇÔöÇ */}
              <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app">
                <div>
              ...
              {/* ÔöÇÔöÇ Tipograf├¡a ÔöÇÔöÇ */}
              <div className="p-4 bg-surface-2 rounded-2xl border border-app">
                <div className="flex items-center gap-2 mb-1">
                  <Type size={16} className="text-primary shrink-0" />
                  <p className="text-sm font-bold text-app">Tipograf├¡a</p>
                </div>
                <p className="text-xs text-muted mb-4">Fuente principal de toda la aplicaci├│n ÔÇö {FONTS[formData.appFont]?.name || 'Inter'} ({FONTS[formData.appFont]?.category || 'Modernas'})</p>
              ...
              {/* ÔöÇÔöÇ Modo de Vista del Cat├ílogo ÔöÇÔöÇ */}
              <div className="p-4 bg-surface-2 rounded-2xl border border-app">
                <p className="text-sm font-bold text-app mb-1">Dise├▒o del Cat├ílogo</p>
      ...
      {/* ÔöÇÔöÇÔöÇ VISTA: PUBLICIDAD Y PROMOCIONES ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {activeSection === 'publicidad' && (
        <div className="space-y-6">
          {/* BOT├ôN AGREGAR Y HEADER */}
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-muted">Crea y gestiona tus campa├▒as h├¡bridas y de inventario</p>
            </div>
            {!showAdForm && (
              <button
                onClick={() => {
            ...
            <div className="bg-surface rounded-3xl shadow-sm overflow-hidden">
              <div className="p-5 sm:p-6 border-b border-app bg-surface-2/30 flex justify-between items-center">
                <p className="font-bold text-app text-sm">
                  {editingAdId ? 'Editar Anuncio' : 'Nuevo Anuncio / Promoci├│n'}
                </p>
                <button
              ...
              <div className="p-5 sm:p-6 space-y-4">
                ...
                {adFormData.type === 'inventory' && (
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onBlur={(e) => {
                            // Al salir del campo, convertir a n├║mero limpio
                    ...
                    <div>
                      <label className="text-xs font-bold text-app mb-1 block">T├¡tulo Personalizado (Opcional)</label>
                      <input
                        type="text"
                        placeholder="Dejar vac├¡o para usar el nombre del producto"
                    ...
                    <div className="flex items-center justify-between p-3 rounded-xl bg-surface-2 border border-app">
                      <div>
                        ...
                        <p className="text-[10px] text-muted">A├▒ade un brillo animado premium a la tarjeta del producto</p>
                      </div>
                      <input
                        type="checkbox"
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="flex items-center justify-between p-3 rounded-xl bg-surface-2 border border-app">
                      <div>
                        ...
                        <p className="text-[10px] text-muted">Permite vender un combo o producto que no est├í en el inventario real</p>
                      </div>
                      <input
                        type="checkbox"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-bold text-app mb-1 block">T├¡tulo</label>
                        <input
                          type="text"
                          required
                      ...
                      <div>
                        <label className="text-xs font-bold text-app mb-1 block">Categor├¡a Visual (ej: Combos, Ofertas)</label>
                        <input
                          type="text"
                    ...
                    <div>
                      <label className="text-xs font-bold text-app mb-1 block">Descripci├│n</label>
                      <textarea
                        rows={2}
                    ...
                    {adFormData.isTemporalProduct && (
                      <div className="grid grid-cols-2 gap-4 bg-surface-2 p-4 rounded-2xl border border-app shadow-inner">
                        ...
                        <div>
                          <label className="text-xs font-bold text-app mb-1 block">Precio Promoci├│n ($)</label>
                          <input
                            type="number"
                    ...
                    {/* ÔöÇÔöÇ Fondo y Color ÔöÇÔöÇ */}
                    <div className="space-y-3">
                      <p className="text-xs font-bold text-app">Colores del Anuncio</p>

                      {/* Presets de degradados */}
                      <div>
                        <p className="text-[10px] text-muted uppercase tracking-wider font-bold mb-2">Fondo (Degradado o Color S├│lido)</p>
                        <div className="grid grid-cols-4 gap-2 mb-3">
                          {[
                            { label: 'Rosa', bg: 'linear-gradient(135deg, #e91e8c, #ff4081)' },
                            { label: 'P├║rpura', bg: 'linear-gradient(135deg, #7c3aed, #c026d3)' },
                            ...
                            >
                              {adFormData.colors.bg === bg && (
                                <span className="text-white text-xs font-bold drop-shadow">Ô£ô</span>
                              )}
                            </button>
                          ))}
                        </div>
                        {/* Color s├│lido personalizado */}
                        <div className="flex items-center gap-3 p-3 rounded-xl bg-surface border border-app">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-muted">Selecciona un color s├│lido exacto</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 p-3 rounded-xl bg-surface border border-app">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-[10px] text-muted">Color de t├¡tulos y descripci├│n sobre el fondo</p>
                        </div>
                        {/* Botones r├ípidos Blanco/Negro */}
                    ...
                    {/* ÔöÇÔöÇ Texto del Bot├│n CTA ÔöÇÔöÇ */}
                    <div>
                      <label className="text-xs font-bold text-app mb-1 block">Texto del Bot├│n CTA</label>
                      <input
                        type="text"
                        value={adFormData.ctaText}
                        onChange={(e) => setAdFormData({ ...adFormData, ctaText: e.target.value })}
                        className="w-full h-11 px-3 rounded-xl bg-surface border border-app text-sm text-app focus:outline-none focus:border-primary transition-colors"
                        placeholder="Ver promoci├│n"
                      />
                    </div>

                    {/* ÔöÇÔöÇ Comportamiento CTA ÔöÇÔöÇ */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-bold text-app mb-1 block">Comportamiento Clic (Acci├│n)</label>
                        <CustomSelect
                          ...
                          options={[
                            ...
                            { value: 'category', label: 'Filtrar por Categor├¡a' },
                          ]}
                        />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-app mb-1 block">Valor de la Acci├│n</label>
                        <input
                          type="text"
                          ...
                          placeholder={
                            ...
                            adFormData.ctaAction === 'category' ? 'Nombre o ID Categor├¡a' :
                    ...
                    {/* ÔöÇÔöÇ Efecto Glow (igual que inventario) ÔöÇÔöÇ */}
                    <div className="flex items-center justify-between p-3 rounded-xl bg-surface-2 border border-app">
                      <div>
                        <p className="text-xs font-bold text-app">Efecto Brillo (Glow visual)</p>
                        <p className="text-[10px] text-muted">A├▒ade un brillo animado premium a la tarjeta del anuncio</p>
                      </div>
                      <input
                        type="checkbox"
                        checked={adFormData.glowEffect}
            ...
            <div className="bg-surface rounded-3xl shadow-sm overflow-hidden">
              {isLoadingAds ? (
                ...
                <div className="p-8 text-center text-muted text-sm">
                  No hay anuncios creados. ┬íCrea el primero usando el bot├│n de arriba!
                </div>
              ) : (
                          ...
                          <button
                            onClick={() => {
                              ...
                              setAdFormData({
                                ...
                                ctaText: ad.ctaText || 'Ver promoci├│n',
          ...
          {/* MODAL DE CONFIRMACI├ôN DE ELIMINACI├ôN PREMIUM */}
          {adToDelete && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                ...
                <h3 className="text-base font-bold text-app mb-2">┬┐Eliminar promoci├│n?</h3>
                
                <p className="text-xs text-muted leading-relaxed mb-6">
                  Esta acci├│n es permanente. El anuncio seleccionado se retirar├í de la tienda del cliente inmediatamente.
                </p>

                {/* Previsualizaci├│n del elemento a borrar */}
                <div className="bg-surface-2 p-3 rounded-2xl border border-app text-left mb-6">
                  ...
                  <p className="text-xs font-extrabold text-app mt-0.5 truncate">
                    {adToDelete.type === 'inventory' 
                      ...
                      : (adToDelete.title || 'Promoci├│n Personalizada')
                    }
                  </p>
                </div>
      ...
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {/* ÔöÇÔöÇÔöÇ VISTA: CUPONES DE DESCUENTO ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
          ...
          <div className="bg-surface rounded-3xl border border-app shadow-sm p-6 relative overflow-hidden">
            ...
            {isLoadingCoupons ? (
              ...
              <div className="py-12 text-center bg-surface-2 rounded-2xl border border-dashed border-app">
                ...
                <p className="text-xs text-muted font-medium">No has creado cupones de descuento a├║n.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {coupons.map(coupon => {
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between gap-2 mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            {/* M├®trica de usos en Badge premium */}
                        ...
                        {coupon.minPurchase > 0 && (
                          <p className="text-[11px] text-muted mt-0.5">
                            Compra m├¡nima: <strong className="text-app">{formatCurrency(coupon.minPurchase)}</strong>
                          </p>
                        )}
                      ...
                      <div className="flex items-center justify-between mt-4.5 border-t border-app pt-3.5">
                        <button
                          onClick={() => {
                            if (!coupon.active) {
                              // Validar que no est├® expirado antes de reactivar por comodidad
                              if (today > coupon.endDate) {
                                alert('Este cup├│n est├í expirado. Modifica la fecha de fin si deseas activarlo.')
                                return
                              }
                            }
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                >
                  ...
                  <h3 className="text-lg font-bold text-app mb-1">
                    {editingCouponId ? 'Editar Cup├│n' : 'Nuevo Cup├│n de Descuento'}
                  </h3>
                  <p className="text-xs text-muted mb-5">Configura las opciones y restricciones del cup├│n</p>

                  <div className="space-y-4">
                    {/* C├│digo con Autogenerador */}
                    <div>
                      <label className="block text-xs font-bold text-muted uppercase tracking-wider mb-2">C├│digo del Cup├│n *</label>
                    ...
                    {/* Plantillas de Ofertas R├ípidas */}
                    <div className="p-3 bg-surface-2 rounded-2xl border border-app">
                      <p className="text-[10px] font-bold text-muted uppercase tracking-wider mb-2 flex items-center gap-1">
                        <Sparkles size={11} className="text-primary" /> Plantillas de Configuraci├│n R├ípida
                      </p>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="block text-xs font-bold text-muted uppercase tracking-wider mb-2">Compra M├¡nima ($)</label>
                        <input
                          type="number"
                    ...
                    <div className="flex items-center gap-2.5 pt-2">
                      ...
                      <label htmlFor="couponActiveCheckbox" className="text-xs font-bold text-app select-none cursor-pointer">
                        Habilitar cup├│n inmediatamente
                      </label>
                    </div>
          ...
          {couponToDelete && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <div className="relative w-full max-w-sm bg-surface rounded-3xl shadow-2xl p-6 border border-app z-10 text-center">
                ...
                <h4 className="text-lg font-bold text-app mb-1">┬┐Eliminar Cup├│n?</h4>
                <p className="text-xs text-muted mb-6">
                  Esta acci├│n es permanente. ┬┐Seguro de eliminar el cup├│n <strong>{couponToDelete.code}</strong>?
                </p>
      ...
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {/* ÔöÇÔöÇÔöÇ VISTA: VENTAS Y TRANSFERENCIAS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
        ...
        <div className="bg-surface rounded-3xl shadow-sm overflow-hidden">
          <div className="p-5 sm:p-6 space-y-6">
            ...
            <div className="pt-4 border-t border-app space-y-4">
              ...
              {/* ÔöÇÔöÇ Cuenta Principal ÔöÇÔöÇ */}
              <div className="rounded-2xl border-2 border-app bg-surface-2/50 overflow-hidden">
                ...
                <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-xs font-bold text-muted mb-1.5">N├║mero de Cuenta</label>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-muted mb-1.5">C├®dula / NIT (Opcional)</label>
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-xs font-bold text-muted mb-1.5">URL del C├│digo QR de Pago</label>
              ...
              {/* ÔöÇÔöÇ Cuenta Secundaria ÔöÇÔöÇ */}
              <div className={`rounded-2xl border-2 overflow-hidden transition-all duration-300 ${formData.bankInfo2.activa ? 'border-emerald-500/40 bg-emerald-500/5' : 'border-dashed border-app bg-surface-2/30'}`}>
                <div className="flex items-center gap-3 px-4 py-3 border-b border-app/50">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-muted">El cliente podr├í elegir entre las dos cuentas</p>
                  </div>
          ...
          <div className="p-5 sm:p-6 border-t border-app bg-surface-2/30 space-y-3">
            ...
            <div className="flex items-start gap-2 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
              ...
              <p className="text-xs text-amber-500/90 leading-relaxed">
                <strong>Dato cr├¡tico:</strong> El WhatsApp y la cuenta bancaria son visibles para tus clientes en el checkout. Verifica la informaci├│n antes de guardar.
              </p>
            </div>
            <button onClick={() => {
              setCriticalConfirmText('')
              setCriticalConfirmModal({
                title: 'ÔÜá´©Å Cambio en Datos de Pago',
                desc: 'Est├ís a punto de modificar el n├║mero de WhatsApp y/o la cuenta bancaria. Tus clientes usar├ín estos datos para contactarte y realizar pagos. Un dato incorrecto puede causar p├®rdidas.\n\nEscribe CONFIRMAR para continuar.',
              ...
              {isSaving ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Lock size={18} className="shrink-0" /> Guardar Datos Cr├¡ticos</>}
            </button>
          </div>
        </div>
      )}
      ...
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {/* ÔöÇÔöÇÔöÇ VISTA: FILTROS DEL CAT├üLOGO ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
            ...
            {formData.catalogFilters && (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    { key: 'categories', label: 'Categor├¡as', desc: 'Permite filtrar por categor├¡as en el inicio.' },
                  ...
                  <div className="space-y-3">
                    ...
                    {(!formData.catalogFilters.customAttributes || formData.catalogFilters.customAttributes.length === 0) && (
                      <div className="text-center py-6 text-muted text-sm border border-dashed border-app rounded-xl">
                        No has creado ning├║n atributo personalizado a├║n.
                      </div>
                    )}
                  </div>
                </div>
      ...
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {/* ÔöÇÔöÇÔöÇ VISTA: HERRAMIENTAS DE PRUEBAS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {activeSection === 'developer' && (
        !isDevAuthenticated ? (
          ...
          <div>
            {/* SUBSECCI├ôN MENU: Listado de Herramientas de Desarrollador */}
            {activeSubSection === null && (
              <div className="space-y-4">
                {/* Cabecera superior simple y limpia */}
                <div className="flex items-center justify-between px-1 py-1 flex-wrap gap-3">
                  <p className="text-xs font-bold uppercase tracking-widest text-muted">M├│dulos de desarrollo</p>
                  <button
                    onClick={() => {
                ...
                <div className="grid grid-cols-1 gap-4">
                  {[
                    {
                      id: 'entregas',
                      label: 'M├®todos de Entrega',
                      description: 'Establece retiros en local, env├¡os a domicilio, costos y entregas digitales.',
                      icon: Truck,
                      iconBg: 'bg-primary/10 hover:bg-primary/15',
                      iconColor: 'text-primary'
                    },
                    {
                      id: 'mayorista',
                      label: 'Ventas al por Mayor',
                      description: 'Configura la cantidad m├¡nima, el tipo de descuento y el valor de mayoreo.',
                      icon: Percent,
                      iconBg: 'bg-yellow-500/10 hover:bg-yellow-500/15',
                      iconColor: 'text-yellow-500'
                    },
                    {
                      id: 'garantias',
                      label: 'Garant├¡as y Reclamos',
                      description: 'Permite a tus clientes iniciar reclamos o solicitar cambios sobre pedidos completados.',
                      icon: Shield,
                    ...
                    },
                    {
                      ...
                      description: 'Permite a tus clientes seguir sus pedidos en tiempo real por WhatsApp sin iniciar sesi├│n.',
                      icon: Package,
                      iconBg: 'bg-pink-500/10 hover:bg-pink-500/15',
                      iconColor: 'text-pink-500'
                    },
                    {
                      id: 'dev-filtros',
                      label: 'Filtros del Cat├ílogo',
                    ...
                    },
                    {
                      ...
                      description: 'Configura y activa/desactiva los m├│dulos globales del negocio (Cr├®dito, Reclamos, Cupones, Mayoreo, Empleados, Descartes)',
                      icon: ToggleLeft,
                    ...
                    },
                    {
                      id: 'dev-facturacion',
                      label: 'Facturaci├│n',
                      description: 'Comisiones y m├®tricas de ventas en tiempo real',
                      icon: Receipt,
                    ...
                    {
                      id: 'dev-restauracion',
                      label: 'Restauraci├│n de la aplicaci├│n',
            ...
            {/* 1. Subsecci├│n: Filtros del Cat├ílogo */}
            ...
            {/* 1.5. Subsecci├│n: M├│dulos Activos */}
            {activeSubSection === 'dev-modulos' && (
              <>
                <div className="bg-surface rounded-3xl shadow-sm overflow-hidden p-5 sm:p-6 space-y-5 border border-app">
                  <p className="text-xs text-muted">Habilita o deshabilita los m├│dulos globales del negocio. Los cambios se aplicar├ín en tiempo real para clientes y administradores.</p>
                  
                  {/* Switch Cr├®dito */}
                  <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app">
                    <div>
                      <p className="text-sm font-bold text-app">M├│dulo de Cr├®dito y Cuentas por Cobrar</p>
                      <p className="text-xs text-muted mt-0.5">Permite a los clientes seleccionar "Cr├®dito" como forma de pago y habilita el control de fiados.</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
                      <input type="checkbox" className="sr-only peer"
                  ...
                  <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app">
                    <div>
                      <p className="text-sm font-bold text-app">M├│dulo de Cupones y Ofertas Flash</p>
                      <p className="text-xs text-muted mt-0.5">Habilita cupones promocionales y barra de inserci├│n de c├│digos de descuento en checkout.</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
                      <input type="checkbox" className="sr-only peer"
                  ...
                  <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app">
                    <div>
                      <p className="text-sm font-bold text-app">M├│dulo de Garant├¡as y Reclamos</p>
                      <p className="text-xs text-muted mt-0.5">Permite a los clientes iniciar solicitudes de cambio o reclamo de garant├¡a para pedidos completados.</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
                      <input type="checkbox" className="sr-only peer"
                  ...
                  <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app">
                    <div>
                      <p className="text-sm font-bold text-app">M├│dulo de Ventas al por Mayor</p>
                      <p className="text-xs text-muted mt-0.5">Permite aplicar descuentos autom├íticos por volumen de compra mayorista configurado.</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
                      <input type="checkbox" className="sr-only peer"
                  ...
                  <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app">
                    <div>
                      <p className="text-sm font-bold text-app">M├│dulo de M├║ltiples Empleados</p>
                      <p className="text-xs text-muted mt-0.5">Activa esta opci├│n si tu negocio cuenta con personal de ventas o despachadores en el POS.</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
                      <input type="checkbox" className="sr-only peer"
                  ...
                  {/* Switch Ajustes de Inventario */}
                  <div className="flex items-center justify-between p-4 bg-surface-2 rounded-2xl border border-app">
                    <div>
                      <p className="text-sm font-bold text-app">M├│dulo de Ajustes de Inventario y Descartes</p>
                      <p className="text-xs text-muted mt-0.5">Permite registrar regalos, aver├¡as y consumos internos en el POS.</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
                      <input type="checkbox" className="sr-only peer"
                        checked={formData.inventoryAdjustmentsEnabled}
                        onChange={(e) => setFormData({ ...formData, inventoryAdjustmentsEnabled: e.target.checked })} />
                      <div className="w-11 h-6 bg-app/20 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary shadow-inner"></div>
                    </label>
                  </div>
                </div>
                    ...
                    onClick={async () => {
                      try {
                        const payload = {
                          ...
                          hasMultipleEmployees: formData.hasMultipleEmployees ?? false,
                          inventoryAdjustmentsEnabled: formData.inventoryAdjustmentsEnabled ?? false
                        }
                        await updateAppConfig(payload)
                        config.setConfig(payload)
                        setSaveMessage({ type: 'success', text: 'M├│dulos actualizados correctamente.' })
                        setTimeout(() => setSaveMessage(null), 3000)
                      } catch (e) {
                        setSaveMessage({ type: 'error', text: 'Error al guardar configuraci├│n de m├│dulos.' })
                      }
                    }}
                    className="w-full h-12 bg-primary text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:opacity-90 active:scale-95 transition-all shadow-sm"
                  >
                    <Save size={18} /> Guardar Configuraci├│n de M├│dulos
                  </button>
                </div>
              </>
            )}

            {/* 2. Subsecci├│n: Facturaci├│n */}
                ...
                <div className="space-y-4">
                  <div className="relative overflow-hidden rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 via-surface to-teal-500/5 p-5">
                    ...
                    <div className="relative flex items-start gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-app mb-1">M├│dulo de Facturaci├│n</p>
                        <p className="text-xs text-muted leading-relaxed">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="bg-surface-2 border border-emerald-500/20 rounded-2xl p-4">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-xs text-muted font-medium">Mi comisi├│n del mes</p>
                        </div>
                      ...
                      <div className="bg-surface-2 border border-app rounded-2xl p-4">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-xs text-muted font-medium">Comisi├│n acumulada</p>
                        </div>
                        <p className="text-xl font-black text-app">{fmt(billingMetrics?.comisionHistorica)}</p>
                        <p className="text-xs text-muted mt-0.5">hist├│rico total</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-surface rounded-2xl border border-app overflow-hidden">
                    <div className="px-5 pt-5 pb-4">
                      <p className="text-sm font-bold text-app mb-1">Porcentaje de comisi├│n</p>
                      <p className="text-xs text-muted mb-4">Se aplica sobre el total de cada pedido completado.</p>
                      <div className="flex gap-3 items-end">
                        <div className="flex-1">
                          <label className="text-xs font-bold text-app mb-1.5 block">Comisi├│n por venta (%)</label>
                          <div className="relative">
                            <input
                              type="text"
                              inputMode="decimal"
                          ...
                          onClick={async () => {
                            ...
                            if (!isNaN(num) && num >= 0) {
                              try {
                                ...
                                setSaveMessage({ type: 'success', text: `Comisi├│n actualizada al ${num}%` })
                                setTimeout(() => setSaveMessage(null), 3000)
                              } catch (error) {
                                setSaveMessage({ type: 'error', text: 'Error al actualizar la comisi├│n.' })
                                setTimeout(() => setSaveMessage(null), 3000)
                              }
                            }
                          }}
                      ...
                      <div className="bg-surface rounded-2xl border border-app overflow-hidden">
                        ...
                        <div className="divide-y divide-app">
                          {[
                            ...
                            { label: 'Comisi├│n del mes', value: fmt(billingMetrics.comisionMes), highlight: true },
                            { label: 'Total ventas hist├│rico', value: fmt(billingMetrics.totalHistorico), sub: 'Todos los tiempos' },
                            { label: 'Comisi├│n hist├│rica acumulada', value: fmt(billingMetrics.comisionHistorica), highlight: true },
                          ].map((row, i) => (
                            ...
                            >
                              <div className="flex items-center justify-between border-b border-primary/5 pb-3">
                                <div>
                                  ...
                                  <p className="text-[10px] text-muted">Dibuja la firma t├íctil del cliente en el recuadro</p>
                                </div>
                                <button
                                  onClick={() => setIsSignatureModalOpen(false)}
            ...
            {/* 3. Subsecci├│n: Restauraci├│n de la aplicaci├│n */}
            {activeSubSection === 'dev-restauracion' && (
              <div className="bg-surface rounded-3xl border border-red-500/20 shadow-sm overflow-hidden">
                <div className="p-5 sm:p-6 bg-red-500/5">
                  <p className="text-sm text-app/70">Restaura la aplicaci├│n a sus valores iniciales eliminando todos los datos de negocio de forma real.</p>
                </div>
                <div className="p-5 sm:p-6">
                  {message && (
                  ...
                  <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-5 mb-6">
                    <h3 className="font-bold text-red-500 mb-2 flex items-center gap-2">
                      <AlertTriangle size={18} /> ┬íADVERTENCIA DE ACCI├ôN DESTRUCTIVA!
                    </h3>
                    <p className="text-sm text-app/80 mb-4">
                      Esta acci├│n eliminar├í de forma <strong>permanente y real</strong> todos los productos, categor├¡as, cupones, anuncios, pedidos, cr├®ditos y usuarios (excepto tu cuenta de administrador actual). Esto dejar├í la base de datos totalmente vac├¡a para poder entregar o replicar la aplicaci├│n en otro cliente.
                    </p>
                    ...
                    >
                      <Trash2 size={16} className="shrink-0" /> <span>Restaurar Aplicaci├│n a Cero (Borrado Real)</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            {/* 4. Subsecci├│n: Contacto del Desarrollador */}
                ...
                <div className="p-5 sm:p-6 space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-muted mt-2">
                      Este n├║mero se usar├í para que los clientes te contacten si est├ín interesados en una aplicaci├│n para su propio negocio.
                    </p>
                  </div>
                </div>
                <div className="p-5 border-t border-app bg-surface">
      ...
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
      {/* ÔöÇÔöÇÔöÇ VISTA: SEGURIDAD Y ACCESOS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {/* ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ */}
              ...
              <form onSubmit={handleUpdateCredentials} className="space-y-6">
                <div>
                  <label className="block text-sm font-bold text-app mb-2 flex items-center gap-2">
                    ...
                    Contrase├▒a Actual (Requerida por seguridad)
                  </label>
                  <div className="relative">
                    <input type={showCurrentPassword ? 'text' : 'password'} value={currentPassword}
                      onChange={(e) => setCurrentPassword(e.target.value)} placeholder="Tu contrase├▒a actual"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-app mb-2 flex items-center gap-2">
                      <Lock size={16} className="text-primary" /> Nueva Contrase├▒a (Opcional)
                    </label>
                    <div className="relative">
                      <input type={showNewPassword ? 'text' : 'password'} value={newPassword}
                        onChange={(e) => setNewPassword(e.target.value)} placeholder="M├¡nimo 6 caracteres"
      ...
      {/* ÔöÇÔöÇÔöÇ VISTA: DESCARGAR APLICACI├ôN (PWA) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {activeSection === 'pwa' && (
        <div className="bg-surface rounded-3xl shadow-sm overflow-hidden">
          <div className="p-6 text-center flex flex-col items-center">
            ...
            <p className="text-sm text-muted max-w-sm mb-6 leading-relaxed">
              Instala esta aplicaci├│n en tu dispositivo para disfrutar de un acceso r├ípido, mejor rendimiento y una experiencia de pantalla completa sin barra de navegaci├│n.
            </p>
            ...
            {!isStandalone ? (
              <div className="w-full flex flex-col items-center gap-4">
                <button
                  onClick={rawInstallable ? handleInstall : () => {
                    if (isIOS) {
                      alert("­ƒô▒ Para instalar en iOS:\n\n1. Pulsa el bot├│n de Compartir (Ôåæ) en la barra inferior de Safari.\n2. Selecciona 'Agregar a la pantalla de inicio'.")
                    } else {
                      alert("­ƒôÑ Para instalar en Android/Chrome:\n\n1. Toca los tres puntos (Ôï«) del men├║.\n2. Elige 'Instalar aplicaci├│n' o 'Agregar a la pantalla principal'.")
                    }
                  }}
                ...
                {/* Mostrar instrucciones secundarias de apoyo abajo del bot├│n */}
                <div className="w-full p-4 rounded-2xl bg-surface-2 border border-app text-sm text-muted">
                  {isIOS ? (
                    <div className="text-xs text-muted leading-relaxed text-left max-w-md">
                      ­ƒô▒ <strong>Instrucciones para iPhone/iPad:</strong> pulsa el bot├│n de <strong>Compartir</strong> <span className="text-primary font-bold">Ôåæ</span> en la barra inferior de Safari y luego selecciona <strong>"Agregar a la pantalla de inicio"</strong>.
                    </div>
                  ) : (
                    <div className="text-left space-y-2.5">
                      ...
                      <ol className="list-decimal list-inside space-y-1.5 text-xs text-muted leading-relaxed">
                        <li>Toca el men├║ de <strong>tres puntos</strong> <strong className="text-app">(Ôï«)</strong> en la parte superior derecha de tu navegador Chrome.</li>
                        <li>Selecciona la opci├│n <strong>"Instalar aplicaci├│n"</strong> o <strong>"Agregar a la pantalla principal"</strong>.</li>
                        <li>Confirma en la ventana emergente y se a├▒adir├í el ├¡cono de acceso r├ípido.</li>
                      </ol>
                    </div>
                  )}
                </div>
              ...
              <div className="w-full p-4 rounded-2xl bg-surface-2 border border-app text-sm text-muted">
                <p className="font-semibold text-primary flex items-center justify-center gap-2">
                  <CheckCircle size={18} />
                  ┬íYa est├ís ejecutando la aplicaci├│n instalada!
                </p>
              </div>
            )}
          </div>
      ...
      {/* ÔöÇÔöÇÔöÇ VISTA: LABORATORIO DE INNOVACI├ôN ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {activeSection === 'laboratorio' && (
        <div className="bg-surface rounded-3xl shadow-sm overflow-hidden border border-app p-6 space-y-6">
          <div className="flex items-center gap-3 border-b border-app pb-4">
            <div className="w-10 h-10 rounded-xl bg-purple-500/10 text-purple-500 flex items-center justify-center shrink-0">
              <Sparkles size={20} />
            </div>
            <div>
              <h2 className="text-base font-black text-app">Nivel 4: Laboratorio de Innovaci├│n</h2>
              <p className="text-xs text-muted mt-0.5">Zona exclusiva para probar prototipos y m├│dulos por industria antes de su liberaci├│n oficial.</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Restaurante */}
            <div className="p-5 rounded-2xl border border-app bg-surface-2 space-y-3 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2.5">
                  <p className="text-sm font-bold text-app">­ƒì¢´©Å M├│dulo Restaurante & Cocina</p>
                  <span className="px-2 py-0.5 bg-purple-500/10 text-purple-500 rounded-md text-[9px] font-bold uppercase tracking-wider">Beta</span>
                </div>
                <p className="text-xs text-muted leading-relaxed mt-2">
                  Habilita el panel de turnero interactivo y cola de preparaci├│n de platos en tiempo real.
                </p>
              </div>
              <button 
                onClick={() => alert("M├│dulo Restaurante inicializado en pruebas de laboratorio.")}
                className="mt-3 w-full py-2 bg-primary/10 hover:bg-primary/20 text-primary text-xs font-bold rounded-xl transition-colors"
              >
                Configurar Cola de Platos
              </button>
            </div>

            {/* Barber├¡a */}
            <div className="p-5 rounded-2xl border border-app bg-surface-2 space-y-3 opacity-60 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2.5">
                  <p className="text-sm font-bold text-app">­ƒÆê Barber├¡a & Agendamiento</p>
                  <span className="px-2 py-0.5 bg-amber-500/10 text-amber-500 rounded-md text-[9px] font-bold uppercase tracking-wider">Pr├│ximamente</span>
                </div>
                <p className="text-xs text-muted leading-relaxed mt-2">
                  Agenda inteligente, turnos y asignaci├│n de barberos.
                </p>
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl border border-app bg-surface-2 space-y-3 opacity-60 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2.5">
                  <p className="text-sm font-bold text-app">­ƒöº Ferreter├¡a & Cotizaciones</p>
                  <span className="px-2 py-0.5 bg-amber-500/10 text-amber-500 rounded-md text-[9px] font-bold uppercase tracking-wider">Pr├│ximamente</span>
                </div>
                <p className="text-xs text-muted leading-relaxed mt-2">
                  Gesti├│n de cotizaciones r├ípidas para proyectos y construcci├│n.
                </p>
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl border border-app bg-surface-2 space-y-3 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2.5">
                  <p className="text-sm font-bold text-app">­ƒºá Asistente Inteligente IA</p>
                  <span className="px-2 py-0.5 bg-purple-500/10 text-purple-500 rounded-md text-[9px] font-bold uppercase tracking-wider">Experimental</span>
                </div>
                <p className="text-xs text-muted leading-relaxed mt-2">
                  Generaci├│n autom├ítica de descripciones y variantes de productos con IA.
                </p>
              </div>
              <button 
                onClick={() => alert("Asistente IA activado. Visita el creador de productos de Inventario para usar la IA.")}
                className="mt-3 w-full py-2 bg-primary/10 hover:bg-primary/20 text-primary text-xs font-bold rounded-xl transition-colors"
              >
                Probar Asistente
              </button>
      ...
      {/* ÔöÇÔöÇÔöÇ VISTA: FACTURACI├ôN ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {activeSection === 'facturacion' && (
        !isDevAuthenticated ? (
          ...
          return (
            <div className="space-y-4">
              {/* Bot├│n flotante superior de bloqueo */}
      ...
      {/* ÔöÇÔöÇÔöÇ MODAL DE CONFIRMACI├ôN CR├ìTICA (Fase 5) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      <AnimatePresence>
          ...
          >
            ...
            >
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  <label className="text-xs font-bold text-app mb-2 block tracking-wide uppercase">Confirmaci├│n de seguridad</label>
                  <input
                    type="text"
                  ...
                  {criticalConfirmText.trim().toUpperCase() === 'CONFIRMAR' && (
                    <p className="text-xs text-emerald-500 mt-1.5 flex items-center gap-1">
                      <CheckCircle size={12} /> ┬íListo! Puedes continuar.
                    </p>
                  )}
                </div>
              </div>
      ...
      {/* ÔöÇÔöÇÔöÇ MODAL DEL SELECTOR DE TEMAS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      <AnimatePresence>
        {isThemeModalOpen && (
          <ThemeModalLock>
              ...
              >
                ...
                <div className="p-5 sm:p-6 overflow-y-auto flex-1 overscroll-y-contain">
                  <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <p className="text-sm text-muted">Selecciona una paleta predefinida o crea tu propia combinaci├│n exacta.</p>
                    <button 
                    ...
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-6 bg-surface-2 rounded-2xl border border-app shadow-inner">
                       <div className="flex items-center gap-2 mb-6 p-3 rounded-xl bg-surface border border-app">
                         ...
                         <p className="text-sm font-medium text-app">
                           Est├ís editando la paleta para el modo <strong className="text-primary">{config.isDarkMode ? 'Oscuro' : 'Claro'}</strong>.
                         </p>
                       </div>
