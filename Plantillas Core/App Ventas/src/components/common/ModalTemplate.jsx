 ...
 *
 * CORRECCIÓN: Usa un <div> portal dedicado (ref estable) en lugar de document.body
 * directamente, eliminando el error "removeChild: node is not a child of this node"
 * causado por colisiones entre el ciclo de vida del portal y el reconciliador de React.
 *
 * @param {object} props
...
export default function ModalTemplate({ 
  ...
  // ─── Portal Container Dedicado ──────────────────────────────────────────────
  // Creamos un <div> propiedad exclusiva de esta instancia del modal.
  // React monta y desmonta sobre este nodo aislado → nunca toca document.body
  // directamente, eliminando la colisión de removeChild en el reconciliador.
  ...
  useEffect(() => {
    ...
    return () => {
      // Cleanup seguro: solo removemos si aún es hijo de body
  ...
  // Aguardar a que el portal esté montado en el DOM
  ...
  return ReactDOM.createPortal(modalDOM, portalRef.current)
}
