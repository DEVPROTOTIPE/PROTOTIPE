...
import { User, LogOut, Package, CreditCard, Sparkles, ChevronRight, Info, Edit2, Download, CheckCircle, Smartphone, Moon, Sun } from 'lucide-react'
        ...
        <motion.div variants={itemVariants} className="bg-surface rounded-2xl p-4 border border-app shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-app text-sm leading-tight">Modo Oscuro</h3>
                <p className="text-xs text-muted mt-1 max-w-[200px]">
            ...
            <button 
              ...
              className={`w-14 h-8 rounded-full flex items-center px-1 transition-colors duration-300 shrink-0 ${isDarkMode ? 'bg-primary' : 'bg-surface-2 border border-app'}`}
              aria-label="Alternar modo oscuro"
            >
              <motion.div 
                layout
                className={`w-6 h-6 rounded-full bg-white shadow-sm ${!isDarkMode && 'bg-muted'}`}
                initial={false}
