...
import { useProduct, useCategories, useProducts } from '../../features/inventory'
...
export default function ProductPublicDetail() {
  ...
  const handleMouseLeave = () => {
    setZoomState(prev => ({ ...prev, display: 'none' }))
  }

  const handleTouchMove = (e) => {
    if (!containerRef.current || !isMobileZoomActive) return
  ...
  const toggleMobileZoom = () => {
    const nextState = !isMobileZoomActive
    setIsMobileZoomActive(nextState)
    if (nextState) {
      setZoomState({
        display: 'block',
        left: '-50%',
    ...
    <div className="min-h-screen bg-app pb-32">
      ...
      <main className="max-w-md mx-auto pt-20 px-4 space-y-3.5">
        ...
        <div 
          ...
          className={`relative w-full aspect-square bg-surface border border-neutral-200/80 dark:border-neutral-800/80 rounded-3xl overflow-hidden shadow-sm hover:shadow-[0_20px_50px_rgba(0,0,0,0.14)] dark:hover:shadow-[0_20px_50px_rgba(0,0,0,0.35)] group cursor-crosshair transition-shadow duration-500 ${
            isMobileZoomActive ? 'touch-none' : ''
          }`}
        >
          {imageLoading && (
            ...
            <>
              <AnimatePresence mode="wait">
                <motion.img 
                  ...
                  drag={isMobileZoomActive ? false : "x"}
              ...
              <div className={`absolute inset-0 pointer-events-none rounded-3xl overflow-hidden transition-opacity duration-200 shadow-[inset_0_0_20px_rgba(0,0,0,0.06)] z-15 ${
                isMobileZoomActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
              }`}>
                <img
                  src={imageGallery[activeImageIndex]}
                  style={{
                    display: zoomState.display,
                    left: zoomState.left,
                    top: zoomState.top,
                    width: '200%',
                    height: '200%',
              ...
              {/* Reflejo de Cristal en Paralaje */}
              <div 
                className={`absolute inset-0 pointer-events-none rounded-3xl overflow-hidden transition-opacity duration-300 z-16 ${
                  isMobileZoomActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                }`}
                style={{
                  background: 'linear-gradient(135deg, rgba(255,255,255,0) 30%, rgba(255,255,255,0.12) 50%, rgba(255,255,255,0) 70%)',
                  transform: `translateX(${(zoomState.x - 50) * 0.4}%) translateY(${(zoomState.y - 50) * 0.4}%)`,
                  mixBlendMode: 'overlay'
                }}
              />

              {/* Indicador Flotante de Acción (Desktop) */}
              <div className="hidden md:flex absolute bottom-3 left-3 bg-white/75 dark:bg-black/75 backdrop-blur-md border border-[var(--color-border)] text-[10px] font-bold text-black dark:text-white px-2.5 py-1 rounded-lg pointer-events-none items-center gap-1.5 shadow-sm opacity-100 group-hover:opacity-0 transition-opacity z-20">
                <svg className="w-3.5 h-3.5 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                Pasa el cursor para ampliar detalles
              </div>

              {/* Botón Lupa de Zoom Interactivo (Móvil) */}
              <div className="absolute bottom-3 left-3 z-20 md:hidden">
                <button
                  ...
                  className={`h-9 px-3.5 rounded-xl font-bold text-[10px] uppercase tracking-wider flex items-center gap-2 shadow-lg transition-all duration-300 active:scale-95 cursor-pointer border ${
                    isMobileZoomActive 
                      ? 'bg-primary text-white border-primary scale-105' 
                      : 'bg-white/95 dark:bg-neutral-900/95 text-app border-neutral-200 dark:border-neutral-800'
                  }`}
                >
                  <motion.div
                    animate={isMobileZoomActive ? { scale: [1, 1.2, 1] } : {}}
                    transition={{ repeat: Infinity, duration: 1.5 }}
                  >
                    <ZoomIn size={14} className="stroke-[2.5]" />
                  </motion.div>
                  <span>{isMobileZoomActive ? 'Zoom Activo' : 'Activar Lupa'}</span>
                </button>
              </div>
            </>
          ) : (
          ...
          <div className="space-y-4 pt-1">
            ...
            {currentVariant && currentVariant.stock > 0 && currentVariant.stock <= (commercialOptimization?.tools?.smartTags?.lastUnit?.threshold || 3) && (
              <div className="flex pt-1 animate-pulse">
                <span className="inline-flex items-center gap-1.5 bg-[var(--color-primary)] text-white text-[10px] font-black uppercase px-2.5 py-1 rounded-lg shadow-sm border border-black/10">
