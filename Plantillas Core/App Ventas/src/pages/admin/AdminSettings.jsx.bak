...
export default function AdminSettings() {
  ...
  const handleExportDeveloperReceiptPDF = () => {
    try {
      ...
      showAlert({ title: 'Error al generar PDF', message: 'Error al generar el PDF: ' + error.message, variant: 'error' })
    }
  }
              ...
              <div className="w-full flex flex-col items-center gap-4">
                <button
                  onClick={rawInstallable ? handleInstall : () => {
                    if (isIOS) {
                      showAlert({ title: 'Instalar en iOS', message: '1. Pulsa el botón Compartir (↑) en la barra inferior de Safari.\n2. Selecciona "Agregar a la pantalla de inicio".', variant: 'info' })
                    } else {
                      showAlert({ title: 'Instalar en Android/Chrome', message: '1. Toca los tres puntos (⋮) del menú.\n2. Elige "Instalar aplicación" o "Agregar a la pantalla principal".', variant: 'info' })
                    }
                  }}
