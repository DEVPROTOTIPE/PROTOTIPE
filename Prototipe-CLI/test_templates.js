...
/**
 * Audita la configuración de empaquetado de vite.config.js para asegurar el Code Splitting.
 * @param {string} destino Dir del template
 * @param {string} name Nombre del template
 * @returns {string[]} Lista de errores de optimización
 */
function auditarViteConfig(destino, name) {
  const errors = [];
  const configPath = path.join(destino, 'vite.config.js');
  
  if (!fs.existsSync(configPath)) {
    errors.push(`vite.config.js no existe en "${name}". Todas las plantillas Core de PROTOTIPE deben definir un archivo vite.config.js.`);
    return errors;
  }
  ...
  try {
    ...
    if (!content.includes('manualChunks')) {
      errors.push(`vite.config.js en "${name}" no contiene la propiedad "manualChunks" para code splitting.`);
    }
    
    if (content.includes('manualChunks') && !content.includes('firebase-firestore')) {
      errors.push(`vite.config.js en "${name}" no fragmenta el SDK de Firebase en submódulos (ej: firebase-firestore). Causará bundles pesados en producción.`);
    }
    
    if (content.includes('manualChunks') && !content.includes('react-router')) {
...
async function testPlantilla(name, config, opts) {
  ...
  // ── Paso 2.7: Auditar estándares de empaquetado (Vite) ────────────────────
  printSection('Auditando estándares de empaquetado y Code Splitting (Vite)...');
  const viteErrors = auditarViteConfig(destino, name);
  if (viteErrors.length > 0) {
    ...
    console.log(pc.green('    ✓ vite.config.js cumple con los estándares de Code Splitting.'));
  }
