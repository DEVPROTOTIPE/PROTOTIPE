  ...
  const handleSave = async () => {
    try {
      ...
      if (currentUser) {
        ...
        const trimmedName = formData.sellerName.trim()
        
        // Guard: no sobreescribir nombre con el valor de fábrica genérico ('Vendedor')
        // ni con cadena vacía. Solo actualizamos si el admin ingresó un nombre real.
        const FACTORY_DEFAULTS = ['Vendedor', 'Administrador']
        const namePayload = (trimmedName && !FACTORY_DEFAULTS.includes(trimmedName))
          ? { nombre: trimmedName }
          : {}
