...
export function useCustomerLoyalty(tenantId) {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    setError(null);
    try {
      const records = await CustomerLoyaltyService.getRecords(tenantId);
      setData(records);
    } catch (err) {
  ...
  const addRecord = useCallback(async (recordData) => {
    ...
    setError(null);
    try {
      const id = await CustomerLoyaltyService.createRecord(tenantId, recordData);
      await fetchRecords();
      return id;
    } catch (err) {
