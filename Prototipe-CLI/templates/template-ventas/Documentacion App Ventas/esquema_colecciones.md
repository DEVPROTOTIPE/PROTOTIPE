...
### 18. `order_tracking`
* **Propósito**: Seguimiento público del estado de pedidos en tiempo real (sin PII sensible).
* **Función**:
  * Contiene una copia filtrada y no confidencial de los pedidos (estado, total, ítems, dirección de entrega sin nombres completos ni teléfonos).
  * Indexado mediante `trackingToken` para acceso público sin autenticación en la ruta `/pedido/status`.

### 19. `user_order_index`
* **Propósito**: Índice privado de referencias de pedidos por cliente, organizado por hash celular.
* **Función**:
  * Indexa los tokens de seguimiento asociados a un cliente bajo la ruta `user_order_index/{hash(celular)}/orders/{trackingToken}`.
  * Permite listar el historial del cliente en `ClientOrders.jsx` de forma rápida y segura sin exponer el número telefónico ni requerir Firebase Auth.
