...
- **Regla del Gatekeeper para Features:** Si el componente es de tipo `Feature` o `Logic` (por ejemplo, vive bajo `src/features/[featureName]/`), verifica la existencia de `src/features/[featureName]/index.js`. Si no existe, créalo; si existe, actualízalo para exportar de manera explícita el componente/hook/servicio inyectado. Queda terminantemente prohibido omitir este entrypoint, garantizando que el resto del código del cliente acceda a la feature únicamente a través de él.
...
- Ejecuta `cmd /c npm run validate` en el proyecto `[PROYECTO_ACTIVO]` (si está definido en su `package.json`) para auditar la integridad del Core, la transaccionalidad de base de datos y que no haya violaciones críticas del Design Integrity Guard en el componente portado.
- Ejecuta `cmd /c npm run build` en el proyecto `[PROYECTO_ACTIVO]` para asegurar que compile sin advertencias ni errores.
- No reportes éxito hasta que la validación y el build pasen limpios.
...
- **Repository (`api/`):** Conexión física directa con Firebase SDK.
- **Service (`services/`):** Dominio y reglas de negocio, validando esquemas (Zod).
- **Hook / Store (`hooks/`):** Estado de UI, interactividad y carga/error.
