...
```
   ...
   > **Actualización Automatizada:** Para modificar el archivo `.env.local` de forma segura, realiza una petición `POST http://localhost:3001/api/project/env` con el body: `{ "clientId": "[PROYECTO_ACTIVO]", "variables": { ... } }`.
   > **Sincronización Firestore:** Al arrancar el servidor local con `npm run dev` o ingresar por primera vez al panel de administración del cliente, el módulo de inicialización de la app propagará automáticamente estas variables a la colección `/config/settings` de Firestore.

4. **Validación de Estilos y Buenas Prácticas:**
   - Ejecuta `cmd /c npm run validate` y `cmd /c npm run build` en `[PROYECTO_ACTIVO]` para validar que los contrastes, fuentes y variables de marca HSL no rompan ninguna regla del Design Integrity Guard ni la compilación.
   - Validar las paletas de colores cromáticas ingresadas con el estándar WCAG 2.1 (contraste AAA/AA) y registrar la tipografía de marca en la guía de estilos del cliente.
   - Asegurar que al inicializar el proyecto del cliente se configure Prettier en package.json y se inyecte el archivo `.prettierrc` para garantizar el estilo de código consistente.
