...
## 🪜 Secuencia de Inyección de Archivos (Feature-Sliced Design)
1. **[UI/Atom] `CustomSelect`** -> Destino: `src/components/ui/CustomSelect.jsx`
2. **[Repository] `ordersRepository`** -> Destino: `src/features/orders/api/ordersRepository.js`
3. **[Service] `ordersService`** -> Destino: `src/features/orders/services/ordersService.js`
4. **[Hook] `useOrders`** -> Destino: `src/features/orders/hooks/useOrders.js`
5. **[Component] `OrdersPanel`** -> Destino: `src/features/orders/components/OrdersPanel.jsx`
6. **[Entrypoint] `orders/index.js`** -> Destino: `src/features/orders/index.js`

## ⚙️ Post-condiciones Obligatorias
Una vez ejecutada la inyección, compilar y validar:
```bash
